...
- The loopback listener answers only to Hosts naming its own address: loopback literals, `*.localhost`, or an allowlisted origin's host (`hostGuard` in `backend/internal/httpd/cors.go`). This is what stops DNS rebinding, and no origin check substitutes for it — after a rebind the request is GENUINELY same-origin and a same-origin GET carries no `Origin` header at all. The LAN listener is exempt: it is credential-gated, and enforcing bound-address equality there would break MagicDNS and `ao-phone-proxy`.
- **The LAN listener is exempt from `hostGuard` BECAUSE every route or surface on it is credential-gated. Adding an unauthenticated data-returning or state-changing route or surface there breaks the exemption.** A rebinding page reaching the LAN socket gets nothing only because it carries no Bearer and no `ao_conn` cookie (the browser scopes that cookie to the host the daemon actually served). One ungated route that returns data or has a side effect silently retires that argument, on a socket bound to the network. `TestEveryLANRouteIsCredentialGated` fails the build if it happens; a public, data-free app shell may be declared in `unauthenticatedLANRoutes`, anything reading or changing state may not. Today the only such declaration is the embedded web UI bundle (`backend/internal/httpd/webui`, routed by `webUIBypass` in `lan_listener.go`) — it is the password prompt itself, and a browser cannot send a credential it has not been asked for. **"Surface", not just "route", is deliberate:** that bundle is served by the router's `NotFound` fallback, so `chi.Walk` cannot yield it and `TestEveryLANRouteIsCredentialGated` cannot see it — the two tests meet at that seam. `TestWebUIBypassFailsClosedForRegisteredRoutes` and `TestWebUIBypassDisabledForUnknownHandler` cover the other side: the bypass treats a request as UI only when the router has **no** handler for it, so a new daemon route is credential-gated the day it is written, and a handler that cannot report its routes disables the bypass entirely rather than skipping the check. `webui.daemonPrefixes` is defense-in-depth behind that, not the sole guard.
