...
> Integrated with the fork on 2026-09-04; implementation notes below reflect develop.

## Context

Before remote-host support, the desktop app talked to exactly one daemon: the
loopback one it started. A user running AO on more than one machine — a laptop and
a workstation, say — could only watch one at a time, and switching meant pointing
the app somewhere else and reloading.
...
`~/.ao/remotes.json`. The missing piece was a desktop client that could hold several
...
1. **Local is a host.** `LOCAL_HOST` is a host id like any other — the one whose
   requests skip the proxy. Target selection uses a host id rather than a global
   active-host switch.
...
2. **Host-qualified refs are the target addressing rule.** Ids are never rewritten;
   ...
   still accept bare ids within an already host-scoped connection. Editor handoff
   carries the host and resolves remote workspaces through that host's connection;
   opening them locally requires a configured SSH destination.
...
6. **Saved credential containment.** Add and edit forms necessarily hold the password
   ...
   `{label, url, sshDestination?}`, and connecting returns the loopback proxy base.
...
| Severity | Finding |
| --- | --- |
| High | A renderer-supplied path could change the request origin and disclose the credential. The final request origin is now checked against the saved host before sending. |
...
store, and access to the desktop that holds the connected remotes. Revisit if AO targets
...
- The renderer is now host-aware end to end: reads, writes, routes, caches, event
  ...
  session or project by bare id outside an already host-scoped boundary risks
  misrouting; Ref types and same-id-across-host regression tests guard this.
...
- The feature is off unless Remote hosts is explicitly enabled and a host is saved
  and connected. With no remote host the fan-out is a loop of one and the app issues
  exactly the requests it did before.
