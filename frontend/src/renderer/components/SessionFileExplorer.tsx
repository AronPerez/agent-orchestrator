  ...
  useEffect(() => {
    ...
    const routeDiffWheel = (event: WheelEvent) => {
      ...
      if (
        ...
        !target.closest(".session-files-diff-scrollbar")
      )
        return;
      const scrollRoot = root.querySelector<HTMLElement>(
    ...
    root.addEventListener("wheel", routeDiffWheel, {
      capture: true,
      passive: false,
    });
    return () =>
      root.removeEventListener("wheel", routeDiffWheel, { capture: true });
  }, []);
