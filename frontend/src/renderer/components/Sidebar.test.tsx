...
vi.mock("../lib/cloud-orchestrator", () => ({
  spawnCloudOrchestrator: spawnCloudMock,
}));
vi.mock("../lib/cloud-session", () => ({
  useCloudSession: () => cloudSessionState,
}));
vi.mock("../hooks/useCloudGate", () => ({
  useCloudGate: () => cloudGateState,
}));
...
function sidebarPR(
  overrides: Partial<WorkspaceSession["prs"][number]> = {},
): WorkspaceSession["prs"][number] {
  return {
...
type RemoveProjectHandler = (project: {
  host: string;
  id: string;
}) => Promise<void>;
...
function renderSidebar({
  ...
  onCreateProject = vi
    .fn()
    .mockResolvedValue(undefined) as CreateProjectHandler,
  onInitializeProject = vi
    .fn()
    .mockResolvedValue(undefined) as InitializeProjectHandler,
  onRemoveProject = vi
    .fn()
    .mockResolvedValue(undefined) as RemoveProjectHandler,
  seedAgents = true,
  ...
  // Most sidebar tests exercise session rows and assume their fixture project
  // was previously open. Disclosure-specific tests can provide an empty list.
...
function codedError(
  message: string,
  code: "NOT_A_GIT_REPO" | "PROJECT_UNBORN",
) {
...
async function openCreateProjectDialog(
  ...
  await user.click(
    screen.getByRole("button", { name: /^Open local repository$/i }),
  );
  await screen.findByText(path);
  await chooseOption(
    screen.getByRole("combobox", { name: "Worker agent" }),
    "Codex",
  );
  await chooseOption(
    screen.getByRole("combobox", { name: "Orchestrator agent" }),
    "Claude Code",
  );
  return user;
}
...
beforeEach(() => {
  ...
  spawnCloudMock.mockReset().mockResolvedValue("cloud-orchestrator");
  spawnMock.mockReset();
...
describe("destructive actions name the remote host", () => {
  it("names the host on the session kill control", () => {
    ...
    renderSidebar({
      workspaces: [{ ...workspace, host, sessions: [{ ...session, host }] }],
    });

    expect(
      screen.getByRole("button", { name: "Kill session on workbox" }),
    ).toBeInTheDocument();
  });
  ...
  it("names the host in the remove-project confirmation", async () => {
    const user = userEvent.setup();
    renderSidebar({
      workspaces: [{ ...workspace, host: "http://192.0.2.1:3011" }],
    });
  ...
  it("says nothing extra while local is the active host", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Kill session" }),
    ).toBeInTheDocument();
...
describe("Sidebar — one tree across hosts", () => {
  it("labels remote rows and keys colliding projects by host", () => {
    ...
    const rows = [workspace, remote].map((project) =>
      document.querySelector<HTMLElement>(
        `[data-project-ref="${refKey(project)}"] [data-sidebar="menu-button"]`,
      ),
    );
    expect(
      screen.getByText("Project One", { selector: "[data-project-label]" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Project One · workbox", {
        selector: "[data-project-label]",
      }),
    ).toBeInTheDocument();
  ...
  it("highlights only the routed host's row when two hosts pin the same session id", () => {
      ...
      hostSections: [
        {
          ...
          label: "Local",
          status: "ready",
          workspaces: [pinnedOn(LOCAL_HOST, "local work")],
          failure: null,
        },
        {
          host: remoteHost,
          label: "workbox",
          status: "ready",
          workspaces: [pinnedOn(remoteHost, "remote work")],
          failure: null,
        },
      ],
    });
    ...
    expect(
      within(pinned).getByRole("button", { name: "Open local work" }),
    ).not.toHaveAttribute("aria-current");
    expect(
      within(pinned).getByRole("button", { name: "Open remote work" }),
  ...
  it("renders one row per host for the same project id", () => {
    ...
    expect(consoleError).not.toHaveBeenCalled();
    expect(
      screen.getByText(id, { selector: "[data-project-label]" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(`${id} \u00b7 workbox`, {
        selector: "[data-project-label]",
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(`${id} \u00b7 buildbox`, {
        selector: "[data-project-label]",
      }),
    ).toBeInTheDocument();
    expect(rows[0]).not.toHaveAttribute("data-active", "true");
  ...
  it("opens a remote project in place without reloading", async () => {
    ...
    const locationImpl = Object.getOwnPropertySymbols(window.location)
      .map(
        (symbol) =>
          (
            window.location as unknown as Record<
              symbol,
              { reload?: () => void }
            >
          )[symbol],
  ...
  it("navigates colliding project ids with distinct host-qualified routes", async () => {
    const remoteHost = "http://192.0.2.1:3011";
    const local = { ...workspace, id: "shared-project" };
    const remote = { ...workspace, host: remoteHost, id: "shared-project" };
    ...
    expect(navigateMock).toHaveBeenNthCalledWith(1, {
      to: "/host/$hostId/project/$projectId",
      params: { hostId: LOCAL_HOST, projectId: "shared-project" },
    });
    expect(navigateMock).toHaveBeenNthCalledWith(2, {
      to: "/host/$hostId/project/$projectId",
      params: { hostId: remoteHost, projectId: "shared-project" },
    });
  });

  it("spawns cloud and remote orchestrators through their selected transports", async () => {
    const remoteHost = "http://192.0.2.1:3011";
    const remote = {
      ...workspace,
      host: remoteHost,
    ...
    spawnMock.mockResolvedValue("remote-orchestrator");
    renderSidebar({ workspaces: [remote, cloud] });

    await userEvent.click(
      screen.getByRole("button", {
        name: "Spawn Remote Project orchestrator",
      }),
    );
    await waitFor(() =>
    ...
    expect(navigateMock).toHaveBeenCalledWith({
      ...
      params: { hostId: remoteHost, sessionId: "remote-orchestrator" },
    });

    await userEvent.click(
      screen.getByRole("button", {
        name: "Spawn Cloud Project orchestrator",
      }),
    );
    await waitFor(() =>
      expect(spawnCloudMock).toHaveBeenCalledWith(
    ...
    expect(navigateMock).toHaveBeenCalledWith({
      to: "/host/$hostId/session/$sessionId",
      params: { hostId: cloudHost("org-1"), sessionId: "cloud-orchestrator" },
    });
  });
  ...
  it("filters the tree without changing host routing", async () => {
    ...
    expect(
      screen.getByRole("option", { name: "All hosts" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: "This Mac" }),
    ).toBeInTheDocument();
    await userEvent.click(screen.getByRole("option", { name: "workbox" }));

    expect(
      screen.queryByText("Local Project", { selector: "[data-project-label]" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByText("Remote Project · workbox", {
        selector: "[data-project-label]",
      }),
    ).toBeInTheDocument();
  ...
  it("keeps ready rows visible beside a failed host and retries only that host", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Retry on workbox" }),
    );
    expect(invalidate).toHaveBeenCalledWith({
      queryKey: workspaceHostQueryKey(remoteHost),
    });
  });
  ...
  it("says a host stopped sending live updates while still showing its projects", () => {
    ...
    expect(
      screen.getByText("Not receiving live updates from workbox"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Remote Project · workbox", {
        selector: "[data-project-label]",
    ...
    expect(
      screen.queryByText("Not receiving live updates from This Mac"),
    ).not.toBeInTheDocument();
  });
  ...
  it("stays quiet for a host that never opened a stream", () => {
    ...
    expect(
      screen.queryByText(/Not receiving live updates/),
    ).not.toBeInTheDocument();
  });
});
...
describe("Sidebar", () => {
  it("shows the cloud sign-in entry point while signed out", () => {
    ...
    const activeControl = signInControls.find(
  ...
  it("keeps cloud account controls visible while signed in", () => {
    ...
    expect(
      screen.getAllByLabelText("Signed in as user@example.com"),
    ).toHaveLength(2);
  });
  ...
  it("hides cloud account controls when the daemon reports the cloud offering off", () => {
    ...
    expect(
      screen.queryByLabelText("Signed in as user@example.com"),
    ).not.toBeInTheDocument();
  });
  ...
  it("keeps the Settings footer flush with the bottom edge", () => {
    ...
    expect(screen.getAllByRole("button", { name: "Settings" })[0]).toHaveClass(
      "h-[42px]",
    );
  ...
  it("keeps only the expanded Settings control keyboard-accessible while expanded", () => {
    ...
    const expandedButton = settingsButtons.find((button) =>
      button.textContent?.includes("Settings"),
    );
    const collapsedButton = settingsButtons.find(
    ...
    expect(
      collapsedButton?.closest('[aria-hidden="true"]'),
    ).toBeInTheDocument();
  });
  ...
  it("opens project settings instead of spawning when no orchestrator agent is configured", async () => {
    const user = userEvent.setup();
    renderSidebar({
      workspaces: [{ ...workspace, orchestratorAgent: undefined }],
    });

    await user.click(
      screen.getByRole("button", { name: "Spawn Project One orchestrator" }),
    );
  ...
  it("dismisses project removal immediately and shows progress outside the modal", async () => {
    ...
    expect(
      screen.queryByRole("dialog", { name: "Remove project" }),
    ).not.toBeInTheDocument();
    expect(navigateMock).toHaveBeenCalledWith({ to: "/" });
    expect(screen.getByRole("status")).toHaveTextContent(
      "Removing Project One",
    );
  ...
  it("keeps the removal dialog dismissed and surfaces failures in the sidebar", async () => {
    ...
    expect(
      await screen.findByText("Failed to remove project"),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("dialog", { name: "Remove project" }),
    ).not.toBeInTheDocument();
  ...
  it("opens the create-project flow when the no-project shortcut signal arrives", async () => {
    ...
    expect(
      await screen.findByRole("dialog", {
        name: "Add code to Agent Orchestrator",
      }),
    ).toBeInTheDocument();
  });
  ...
  it("reveals orchestrator and kebab buttons on the project row (no dashboard button)", () => {
    ...
    expect(
      screen.queryByLabelText("Open Project One dashboard"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByLabelText("Spawn Project One orchestrator"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("Project actions for Project One"),
    ).toBeInTheDocument();
  });
  ...
  it("keeps project disclosure and row actions in the keyboard tab order", () => {
    ...
    const disclosure = screen.getByRole("button", {
      name: "Toggle Project One sessions",
    });
    ...
    expect(
      screen.queryByRole("button", { name: "Rename fix login" }),
    ).not.toBeInTheDocument();
  ...
  it("fades the message age out in favor of the overlaid hover actions", () => {
    ...
    renderSidebar({
      workspaces: [
        { ...workspace, sessions: [{ ...session, lastUserMessageAt }] },
      ],
    });
    ...
    const actions = screen
      .getByLabelText("Pin session")
      .closest("[data-session-actions]");
  ...
  it("toggles project sessions from the folder icon without selecting the project first", async () => {
    ...
    const folder = screen.getByRole("button", {
      name: "Toggle Project Two sessions",
    });
  ...
  it("lists worker sessions by updated time, newest first", () => {
    ...
    const sessionButtons = Array.from(
      document.querySelectorAll<HTMLButtonElement>(
        '[data-session-row] button[aria-label^="Open "]',
      ),
    );
    expect(
      sessionButtons.map((button) => button.getAttribute("aria-label")),
    ).toEqual([
  ...
  it("returns to the project board from an orchestrator session without collapsing", async () => {
    ...
    expect(screen.getByLabelText("Open fix login")).toBeInTheDocument();
    expect(screen.getByText("Project One").closest("button")).toHaveAttribute(
      "aria-expanded",
      "true",
    );
  });
  ...
  it("expands a collapsed project when opening its orchestrator", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Toggle Project One sessions" }),
    );
    expect(screen.queryByLabelText("Open fix login")).not.toBeInTheDocument();
    expect(screen.getByText("Project One").closest("button")).toHaveAttribute(
      "aria-expanded",
    ...
    await user.click(
      screen.getByRole("button", { name: "Open Project One orchestrator" }),
    );
  ...
  it("defaults worker and orchestrator agents when creating a project", async () => {
    ...
    await user.click(screen.getByLabelText("New project"));
    expect(
      screen.getByRole("dialog", { name: "Add code to Agent Orchestrator" }),
    ).toBeInTheDocument();
  ...
  it("clones a Git URL into the selected folder before starting agents", async () => {
    const user = userEvent.setup();
    const onCloneProject = vi
      .fn()
      .mockResolvedValue(undefined) as CloneProjectHandler;
    ...
    expect(
      await screen.findByRole("dialog", { name: "Clone a Git repository" }),
    ).toBeInTheDocument();
  ...
  it("creates the selected local repository after backing out of a clone", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: "Back to clone details" }),
    );
    await user.click(
      await screen.findByRole("button", { name: "Back to code source" }),
    );
    await user.click(
      await screen.findByRole("button", { name: /^Open local repository$/i }),
    );
  ...
  it("prioritizes authorized project agents by preferred agent order", async () => {
    ...
    ).toEqual(["Cursor", "OpenCode", "Aider", "Devin", "Goose"]);
  ...
  it("explains Git setup before creating a non-git project", async () => {
    ...
    const user = await openCreateProjectDialog("/repo/new-project", {
      path: "/repo/new-project",
      repos: [],
    });

    expect(
      await screen.findByText(/If this folder needs Git setup/i),
    ).toBeInTheDocument();
  ...
  it("warns before initializing a plain project folder nested inside a parent repo", async () => {
    ...
    window.ao!.app.chooseDirectory = vi
      .fn()
      .mockResolvedValue("/repo/parent/universe");
    window.ao!.app.scanImportFolder = vi.fn().mockResolvedValue({
    ...
    expect(
      screen.getByText(/If this folder needs Git setup/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/inside an existing Git repository at \/repo\/parent/i),
    ).toBeInTheDocument();
    expect(onInitializeProject).not.toHaveBeenCalled();
    ...
    await user.click(screen.getByRole("button", { name: "Create and start" }));
    await waitFor(() =>
      expect(onInitializeProject).toHaveBeenCalledWith("/repo/parent/universe"),
    );
    await waitFor(() => expect(onCreateProject).toHaveBeenCalledTimes(1));
  });
  ...
  it("shows repository initialization recovery for git repos with no commits", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Create and start" }));
    await waitFor(() =>
      expect(onInitializeProject).toHaveBeenCalledWith("/repo/unborn"),
    );
    await waitFor(() => expect(onCreateProject).toHaveBeenCalledTimes(1));
  });
  ...
  it("does not initialize Git when the project creation is cancelled", async () => {
    ...
    expect(onInitializeProject).not.toHaveBeenCalled();
    expect(
      screen.queryByRole("dialog", { name: "Project agents" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("can create a workspace project from the project add flow", async () => {
    ...
    window.ao!.app.chooseDirectory = vi
      .fn()
      .mockResolvedValue("/repo/workspace");
    renderSidebar({ onCreateProject });

    await user.click(screen.getByLabelText("New project"));
    await user.click(
      screen.getByRole("button", { name: /^Add a workspace folder$/i }),
    );
    ...
    expect(
      screen.getByRole("dialog", { name: "Workspace agents" }),
    ).toBeInTheDocument();
    await chooseOption(
      screen.getByRole("combobox", { name: "Worker agent" }),
      "Codex",
    ...
    await user.click(
      screen.getByRole("button", { name: "Create workspace and start" }),
    );
  ...
  it("does not run single-repo Git setup recovery for workspace imports", async () => {
    ...
    window.ao!.app.scanImportFolder = vi
      .fn()
      .mockResolvedValue({ path: "/repo/workspace", repos: [] });
    renderSidebar({ onCreateProject, onInitializeProject });
    ...
    expect(onInitializeProject).not.toHaveBeenCalled();
    expect(
      await screen.findByText(/Import failed · workspace not registered/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Review the error above or choose a different folder"),
    ).toBeInTheDocument();
    expect(window.ao!.app.checkAncestorRepo).toHaveBeenCalledWith(
      "/repo/workspace",
    );
  ...
  it("shows detected repository validation when workspace import fails", async () => {
    ...
    const onCreateProject = vi
      .fn()
      .mockRejectedValue(
        new Error("workspace not registered"),
      ) as CreateProjectHandler;
    window.ao!.app.chooseDirectory = vi
      .fn()
      .mockResolvedValue("/Users/test/dev/acme");
    window.ao!.app.checkAncestorRepo = vi.fn().mockResolvedValue(undefined);
    window.ao!.app.scanImportFolder = vi.fn().mockResolvedValue({
    ...
    expect(
      screen.getByText("Repository name is reserved by AO."),
    ).toBeInTheDocument();
    expect(screen.getByText("api")).toBeInTheDocument();
    expect(screen.getByText("main github.com/acme/api")).toBeInTheDocument();
    expect(
      screen.getByText("Resolve 1 failed repository to continue"),
    ).toBeInTheDocument();
    expect(window.ao!.app.checkAncestorRepo).toHaveBeenCalledWith(
      "/Users/test/dev/acme",
    );
  ...
  it("shows non-git child repos as needs git init in the valid list", async () => {
    ...
    expect(
      screen.queryByText(/Origin remote is required/),
    ).not.toBeInTheDocument();
  });
  ...
  it("does not rescan folders for non-validation create failures", async () => {
    ...
    const onCreateProject = vi
      .fn()
      .mockRejectedValue(
        new Error("AO daemon is not ready."),
      ) as CreateProjectHandler;
    window.ao!.app.chooseDirectory = vi
      .fn()
      .mockResolvedValue("/repo/workspace");
  ...
  it("shows needs-auth agents as unavailable while keeping authorized agents selectable", async () => {
    ...
    await waitFor(() =>
      expect(onCreateProject).toHaveBeenCalledWith(
        expect.objectContaining({ orchestratorAgent: "claude-code" }),
      ),
    );
  });
  ...
  it("opens the Mobile settings page from the footer", async () => {
    ...
    renderSidebar();
    await user.click(
      (await screen.findAllByRole("button", { name: "Connect Mobile" }))[0],
    );
    expect(useUiStore.getState().settingsModal).toEqual({
      scope: "global",
      section: "mobile",
  ...
  it("always shows Connect Mobile", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Connect Mobile" }),
    ).toBeVisible();
  });
  ...
  it("caps the inline rename input at 20 characters", async () => {
    ...
    await user.dblClick(screen.getByText("fix login"));
    expect(screen.getByLabelText("Rename fix login")).toHaveAttribute(
      "maxlength",
      "20",
    );
  });
  ...
  it("always shows action icons and reserves padding for them", () => {
    ...
    const actionCluster = screen.getByLabelText(
    ...
    expect(
      within(actionCluster as HTMLElement).getAllByRole("button"),
    ).toHaveLength(2);
    expect(
      screen.getByLabelText("Project actions for Project One"),
  ...
  it("scales project actions with the row without scaling for action-button presses", () => {
    ...
    const projectRow = screen
      .getByText("Project One")
      .closest('button, [role="button"]');
    const pressSurface = projectRow?.closest<HTMLElement>(
      "[data-project-press]",
    );
    const projectActions = screen.getByLabelText(
      "Project actions for Project One",
    );

    if (!projectRow || !pressSurface)
      throw new Error("Project press surface not found");
  ...
  it("optically aligns the project folder and label with its action icons", () => {
    ...
    const projectRow = screen
      ...
      .closest('button, [role="button"]');
    expect(
      projectRow?.querySelector("[data-project-folder-visual]"),
    ).toHaveClass("translate-y-px");
    expect(projectRow?.querySelector("[data-project-label]")).toHaveClass(
      "translate-y-px",
    );
  });
  ...
  it("flushes any queued rAF frame on pointer-up and persists the clamped width", async () => {
    ...
    try {
      ...
      expect(window.localStorage.getItem("ao-sidebar-w")).toBe(
        String(SIDEBAR_MIN_WIDTH + 5),
      );
  ...
  it("paints the dot from its board section while activity drives the pulse", () => {
    ...
    expect(sessionDot("ci failed task")).toHaveClass(
      "bg-status-needs-you",
      "animate-status-pulse",
    );
    expect(sessionDot("review task")).toHaveClass(
      "bg-status-in-review",
      "animate-status-pulse",
    );
    expect(sessionDot("ready task")).toHaveClass(
      "bg-status-ready",
      "animate-status-pulse",
    );
    expect(sessionDot("merged task")).toHaveClass(
      "bg-status-merged",
      "animate-status-pulse",
    );
  });
  ...
  it("holds the dot still for idle activity and keeps its PR tone", async () => {
    ...
    const idleDraftDot = screen
      .getByLabelText("Open idle draft task")
      .querySelector<HTMLElement>("span.rounded-full");
  ...
  it("restores only the projects saved as expanded and persists toggles", async () => {
    ...
    expect(
      JSON.parse(
        window.localStorage.getItem("ao.sidebar.expanded-projects") ?? "null",
      ),
    ).toEqual([]);
  });
      ...
      workspaces: [
        {
          ...
          sessions: [
            session,
            { ...session, id: "proj-1-2", title: "second task" },
          ],
        },
      ],
    });
  ...
  it("keeps merged sessions in the list until they are terminated", async () => {
    ...
    expect(
      screen.queryByLabelText("Open merged terminated task"),
    ).not.toBeInTheDocument();
  });
  ...
  it("keeps a staged build's restart action ahead of the failing-checks retry", async () => {
    ...
    expect(
      await screen.findAllByLabelText("Restart to install update v9.9.9"),
    ).not.toHaveLength(0);
    expect(
      screen.queryByLabelText("Retry update check"),
    ).not.toBeInTheDocument();
  });
  ...
  it("renders the restart-to-update row with the working-orange treatment when escalated", async () => {
    ...
    // Both footer variants (expanded row and collapsed rail icon) are mounted.
    const buttons = await screen.findAllByLabelText(
      "Restart to install update v9.9.9",
    );
    expect(buttons.length).toBeGreaterThan(0);
  ...
  it("commits a project drop", () => {
    ...
    act(() =>
      dragEnds.get(PROJECT_DND_ID)?.({
        active: { id: refKey({ host: LOCAL_HOST, id: "bravo" }) },
        over: { id: refKey({ host: LOCAL_HOST, id: "alpha" }) },
      }),
    );

    expect(
      Array.from(
        document.querySelectorAll("[data-project-label]"),
  ...
  it("commits a session drop within its project", () => {
    ...
    act(() =>
      dragEnds.get(sessionDndId(workspace))?.({
        active: { id: refKey({ host: LOCAL_HOST, id: "second" }) },
        over: { id: refKey({ host: LOCAL_HOST, id: "first" }) },
      }),
    );
    ...
    expect(
      Array.from(
        document.querySelectorAll(
          '[data-testid="session-list-proj-1"] button[aria-label^="Open "]',
        ),
        (node) => node.getAttribute("aria-label"),
      ),
    ).toEqual(["Open Second", "Open First"]);
  });
  ...
  it("does not toggle disclosure from the click synthesized after a folder drag", () => {
    ...
    try {
      renderSidebar({
        workspaces: [{ ...workspace, id: "alpha", name: "Alpha" }],
      });
      ...
      act(() =>
        dragEnds.get(PROJECT_DND_ID)?.({
          active: { id: refKey({ host: LOCAL_HOST, id: "alpha" }) },
          over: null,
        }),
      );
      act(() =>
        fireEvent.click(
          screen.getByRole("button", { name: "Toggle Alpha sessions" }),
        ),
      );
  ...
  it("keeps reordered sessions in an expanded project drag preview", () => {
    ...
    act(() =>
      dragStarts.get(PROJECT_DND_ID)?.({ active: { id: refKey(workspace) } }),
    );

    expect(
      document.querySelector("[data-project-drag-overlay]"),
    ).toHaveTextContent(/Project One.*Second.*First/);
  });
  ...
  it.each(["light", "dark"] as const)(
    "uses a visible project drop indicator in the %s theme",
    (theme) => {
      ...
      try {
        ...
        act(() =>
          dragStarts.get(PROJECT_DND_ID)?.({
            active: { id: refKey({ host: LOCAL_HOST, id: "bravo" }) },
          }),
        );
        act(() =>
          dragOvers.get(PROJECT_DND_ID)?.({
            active: {
            ...
            delta: { x: 0, y: 0 },
            over: {
              id: refKey({ host: LOCAL_HOST, id: "alpha" }),
              rect: { height: 32, top: 0 },
            },
          }),
        );
