...
function TestProjectSettings({
  ...
  return (
    <>
      ...
      {saveState.replacementError && (
        <span>{`Orchestrator restart failed: ${saveState.replacementError}`}</span>
      )}
    </>
  );
}
...
describe("ProjectSettingsForm", () => {
  ...
  it("shows the project name as text with a pencil until edit is clicked", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "Edit Project name" }),
    ).toHaveTextContent("Project One");
    expect(
      screen.queryByRole("textbox", { name: "Project name" }),
    ).not.toBeInTheDocument();

    await userEvent.click(
      screen.getByRole("button", { name: "Edit Project name" }),
    );
    expect(screen.getByRole("textbox", { name: "Project name" })).toHaveValue(
      "Project One",
    );
    expect(
      screen.queryByRole("button", { name: "Edit Project name" }),
    ).not.toBeInTheDocument();
  ...
  it("atomically saves the project display name and config without changing its stable ID", async () => {
    ...
    expect(
      screen.getByText("tg_content_factory_5863f66be3"),
    ).toBeInTheDocument();
  });
  ...
  it("renders git scp-style remotes as clickable https links", async () => {
    ...
    const repoLink = await screen.findByRole("link", {
      name: "git@github.com:acme/project-one.git",
    });
  ...
  it("renders ssh remotes as clickable https links", async () => {
    ...
    const repoLink = await screen.findByRole("link", {
      name: "ssh://git@github.com/acme/project-one.git",
    });
    expect(repoLink).toHaveAttribute(
      "href",
      "https://github.com/acme/project-one",
  ...
  it("loads agents fields and saves without dropping hidden workflow config", async () => {
    ...
    const workerAgent = screen.getByRole("button", {
      name: "Default worker agent",
    });
    const orchestratorAgent = screen.getByRole("button", {
      name: "Default orchestrator agent",
    });
    const permissionMode = screen.getByRole("button", {
    ...
    await userEvent.type(
      screen.getByLabelText("Worker model"),
      "openai/gpt-5.4",
    );
    await userEvent.type(
      screen.getByLabelText("Orchestrator model"),
      "anthropic/claude-sonnet",
    );
    await userEvent.click(permissionMode);
    await userEvent.click(
      await screen.findByRole("menuitem", { name: "Bypass permissions" }),
    );
  ...
  it("loads workflow fields correctly", async () => {
    ...
    const reviewerAgent = screen.getByRole("button", {
  ...
  it("shows the full model catalog again after selecting a model", async () => {
    ...
    const workerModel = await screen.findByRole("button", {
      name: "Worker model",
    });
    ...
    expect(
      screen.queryByRole("searchbox", { name: "Search worker model" }),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.getByRole("menuitem", { name: /GPT-5\.4/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("menuitem", { name: "Custom model…" }),
    ).toBeInTheDocument();
  });
  ...
  it("shows a warning when refreshing a cached model catalog fails", async () => {
    ...
    postMock.mockResolvedValue({
      data: undefined,
      error: { message: "model refresh unavailable" },
    });

    renderSettings("proj-1", undefined, "agents");

    await userEvent.click(
      await screen.findByRole("button", { name: "Refresh worker model list" }),
    );
    expect(
      await screen.findByText("model refresh unavailable"),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Worker model" }),
    ).toHaveTextContent("Agent default");
  });
  ...
  it("shows cached models immediately and deduplicates background revalidation", async () => {
    ...
    getMock.mockImplementation(async (path: string) => {
      if (path === "/api/v1/agents") return agentCatalogResponse;
      if (path === "/api/v1/agents/{agent}/models")
        return { data: cachedCatalog, error: undefined };
      return {
        data: {
          status: "ok",
          project: {
  ...
  it("requires worker and orchestrator agents for existing projects missing role config", async () => {
    ...
    expect(
      await screen.findByText("Worker and orchestrator agents are required."),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Default worker agent" }),
    ).toHaveTextContent("Select worker agent");
    expect(
      screen.getByRole("button", { name: "Default orchestrator agent" }),
    ).toHaveTextContent("Select orchestrator agent");
  ...
  it("uses the localized default label for the project reviewer picker", async () => {
    ...
    const reviewerAgent = await screen.findByRole("button", {
      name: "Default reviewer agent",
    });
    ...
    expect(
      await screen.findByRole("menuitem", { name: "Project default" }),
    ).toBeInTheDocument();
  });
  ...
  it("offers Muse Code as a reviewer", async () => {
    ...
    expect(
      await screen.findByRole("menuitem", { name: /Muse Code/ }),
    ).toBeInTheDocument();
  });
  ...
  it("orders reviewers using the default agent priority", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "Default reviewer agent" }),
    );
    ...
    getMock.mockImplementation(async (path: string) => {
      if (path === "/api/v1/agents") {
        return {
          data: {
            supported: [
              ...agentCatalogResponse.data.supported,
              qwen,
              devin,
              droid,
              kimi,
            ...
            installed: [
              ...agentCatalogResponse.data.installed,
              qwen,
              devin,
              droid,
              kimi,
  ...
  it("warns when an experimental reviewer is selected", async () => {
    ...
    expect(screen.getByRole("status")).toHaveTextContent(
      "Experimental host-trusted reviewer",
    );
  });
  ...
  it("shows unknown-auth agents as selectable with a warning in project settings", async () => {
    ...
    const workerAgent = await screen.findByRole("button", {
      name: "Default worker agent",
    });
  ...
  it("shows Copilot as a reviewer option and saves it in the reviewers payload", async () => {
    ...
    await userEvent.click(reviewer);
    const copilot = await screen.findByRole("menuitem", {
      name: "GitHub Copilot",
    });
  ...
  it("offers Kilo Code as a configured reviewer", async () => {
    ...
    await userEvent.click(reviewer);
    expect(
      await screen.findByRole("menuitem", { name: "Kilo Code" }),
    ).toBeEnabled();
  });
  ...
  it("shows scratch identity and saves only scratch-supported settings", async () => {
    ...
    const kindRow = (await screen.findByText("Type")).closest(
      ".settings-row-bar",
    );
  ...
  it("saves GitHub tracker intake settings, deriving the repo from the project's git origin", async () => {
    ...
    expect(
      screen.getByRole("link", { name: "acme/project-one" }),
    ).toHaveAttribute("href", "https://github.com/acme/project-one");
  ...
  it("blocks save when intake is enabled with no assignee", async () => {
    ...
    expect(
      await screen.findAllByText("Enabling intake requires an assignee."),
    ).toHaveLength(2);
    expect(putMock).not.toHaveBeenCalled();
  });
  ...
  it("restarts when the saved orchestrator agent already differs from the running orchestrator", async () => {
    ...
    const orchestratorAgent = await screen.findByRole("button", {
      name: "Default orchestrator agent",
    });
  ...
  it("keeps the config save successful when orchestrator replacement fails", async () => {
    ...
    expect(
      await screen.findByText(
        "Orchestrator restart failed: missing goose binary",
      ),
    ).toBeInTheDocument();
