...
export interface ChatWorkspaceProps {
  ...
  /** Agent-session actions on the primary chat tab. */
  sessionTabAction?: ReactNode;
  /** Pinned beside the tab strip, before workspace actions. */
...
export function ChatWorkspace({
  ...
  const shellActive = Boolean(shellTarget && session);
	...
	const reorderAuxiliaryTabs = useCallback(
		(nextKeys: string[]) => {
			...
			const next = nextKeys.filter(
				(key, index) => available.has(key) && nextKeys.indexOf(key) === index,
			);
	...
	useEffect(() => {
		if (auxiliaryTabOrder) {
			...
			if (
				!next.every((key, index) => key === auxiliaryTabOrder[index]) ||
				next.length !== auxiliaryTabOrder.length
			) {
		...
		setTabOrderBySession((current) => {
			...
			if (
				...
				next.every((key, index) => key === currentOrder[index])
			)
				return current;
			if (next.length === 0) {
  ...
  const handleCancelQueuedTurn = useCallback(
    async (turnId: string) => {
      ...
      setQueueEdit((current) =>
        current?.turnId === turnId ? undefined : current,
      );
    },
    [onCancelQueuedTurn, stableCancelQueuedTurn],
  );
  ...
  const steer = useCallback(
    async (text: string) => stableSteer(text),
    [stableSteer],
  );
  ...
  const rollbackTarget =
    onRollback && !turn && !newWorkDisabled
            ...
            >
              ...
              >
                ...
                <ChatComposer
                  ...
                  onInterrupt={
                    turn && !newWorkDisabled ? stableInterrupt : undefined
                  }
                  ...
                  disabled={
                    snapshot.controller.state === "stopped" || newWorkDisabled
                  }
                  skills={skills}
                  filePaths={filePaths}
                  filePathsTruncated={filePathsTruncated}
                  onStageAttachments={
                    newWorkDisabled ? undefined : onStageAttachments
                  }
...
function ChatHeader({
  ...
  const tabScrollWatch = `${session?.host ?? ""}:${session?.id ?? ""}|${orderedAuxiliaryTabs
    .map((tab) => tab.key)
    .join("|")}`;
  const {
  ...
  useEffect(() => {
    if (orderedAuxiliaryTabs.length > previousTabCountRef.current)
      scrollTabsToEnd();
    previousTabCountRef.current = orderedAuxiliaryTabs.length;
  }, [orderedAuxiliaryTabs.length, scrollTabsToEnd]);
  const timelineActive =
    !workspaceActiveTabKey && !reviewerActive && !shellActiveHandleId;
  ...
  const header = (
    <header className="flex h-inspector-tabs w-full shrink-0 items-stretch bg-sidebar">
      ...
      >
          ...
          >
			{session ? (
				<SessionPaneTab
					...
					session={session}
					tabAction={sessionTabAction ?? switchAgentControl}
				/>
			) : (
            <button
              ...
              role="tab"
              tabIndex={
                timelineActive || orderedAuxiliaryTabs.length === 0 ? 0 : -1
              }
              title={label}
              type="button"
            >
            ...
            <div className="relative min-w-0 flex-1 self-stretch overflow-hidden">
              <div
                ref={tabsOverflowRef}
                className="scrollbar-none flex h-full min-w-flex-min min-w-0 items-stretch overflow-x-auto"
              >
                      ...
                      <DraggableChatTab key={tab.key} value={tab.key}>
                        {tab.kind === "reviewer" ? (
                          ...
                          <ShellTerminalTab
                            ...
                            isActive={
                              tab.terminal.handleId === shellActiveHandleId
                            }
                            onClose={() =>
                              onCloseShellTerminal?.(tab.terminal.handleId)
                            }
                            onRename={
                              onRenameShellTerminal
                                ? (title) =>
                                    onRenameShellTerminal(
                                      tab.terminal.handleId,
                                      title,
                                    )
                                : undefined
                            }
                            onSelect={() =>
                              onSelectShellTerminal?.(tab.terminal.handleId)
                            }
                            shell={tab.terminal}
                          />
                        ) : (
              ...
              {showLeftFade ? (
                <div
                  aria-hidden="true"
                  className="session-tab-scroll-fade session-tab-scroll-fade--left"
                />
              ) : null}
              {showRightFade ? (
                <div
                  aria-hidden="true"
                  className="session-tab-scroll-fade"
                />
              ) : null}
            </div>
          </div>
        ...
        >
          {tabStripAction ? (
            <div data-testid="session-tab-strip-action">{tabStripAction}</div>
          ) : null}
          {!session && !sessionTabAction ? switchAgentControl : null}
          {headerActions}
        </div>
      </div>
    </header>
...
function Timeline({
  ...
  const localApiBaseUrl = useSyncExternalStore(
    subscribeApiBaseUrl,
    getApiBaseUrl,
    getApiBaseUrl,
  );
