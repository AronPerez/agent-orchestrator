...
export function SessionChatSurface({
  ...
  const snapshot =
    queriedSnapshot?.sessionId === session.id ? queriedSnapshot : undefined;
  ...
  const acceptedLocalTurnObserved = Boolean(
    ...
    snapshot?.turns.some((turn) => turn.id === pendingAcceptedTurnId),
  );
  const acceptedLocalWorkPending = Boolean(
    pendingAcceptedTurnId && !acceptedLocalTurnObserved,
  );
  const controllerBusy =
    snapshot?.controller?.state === "busy" ||
    commands.busy ||
    acceptedLocalWorkPending;
  const hasRunningTurn = Boolean(
    snapshot?.turns.some((turn) => turn.state === "running"),
  );
  const queuedTurnCount =
    snapshot?.turns.filter((turn) => turn.state === "queued").length ?? 0;
  useEffect(() => {
    if (acceptedLocalTurnObserved && pendingAcceptedTurnId) {
      acknowledgeAcceptedTurn(pendingAcceptedTurnId);
    }
  }, [
    acceptedLocalTurnObserved,
  ...
  const { paths, truncated } = useWorkspaceFilePaths(
    session,
    Boolean(snapshot),
  );
  ...
  const [switchSelectorContainer, setSwitchSelectorContainer] =
  ...
  const durableAgentSwitch =
    selectedDurableAgentSwitch &&
    !isAgentSwitchRetired(selectedDurableAgentSwitch.id)
      ? selectedDurableAgentSwitch
      : undefined;
  const admissionAgentSwitch: AgentSwitchSummary | undefined =
    !durableAgentSwitch && switchMutation.isPending && switchMutation.input
      ...
      : undefined;
  const agentSwitch =
    durableAgentSwitch ?? admissionAgentSwitch ?? observedTerminalSwitch;
  ...
  const switchPresentation = agentSwitch
    ? deriveAgentSwitchPresentation({
        ...
        terminalHandleId: targetChatControllerReady
          ? "chat-controller"
          : undefined,
      })
    : undefined;
  ...
  useEffect(() => {
    ...
    agentSwitch,
    observedSettledSwitch,
    settleAgentSwitch,
    switchPresentation,
  ]);
  const shownSwitchPresentation =
    switchPresentation?.outcome === "failure" &&
    dismissedFailureSwitchId === agentSwitch?.id
      ? undefined
      : switchPresentation?.outcome === "success"
  ...
  const switchLocksChat = Boolean(
    switchPresentation?.lockAgentTerminal &&
    !switchPresentation.allowSourceInput,
  );
  ...
  if (unavailable && !renderShellFallback) {
    return (
      <Centered>
        ...
        <p className="max-w-sm text-center text-xs leading-relaxed text-muted-foreground">
          The worktree is untouched. Open a shell from the inspector to work in
          it directly.
        </p>
      </Centered>
    );
  ...
  const activeTurnId = renderSnapshot.turns.find(
  ...
  return (
    <div className="relative h-full min-h-0">
      <ChatWorkspace
        ...
        onInterrupt={() => commands.interrupt(activeTurnId)}
        onResumeAgent={() => {
...
function ChatAgentSwitchStatus({
  ...
  const fullOverlay =
    presentation.lockAgentTerminal &&
    !presentation.allowSourceInput &&
    !auxiliaryActive;
  const warning =
    presentation.outcome === "failure" || presentation.outcome === "recovery";
    ...
    >
      ...
      >
        {success ? (
          ...
          <Loader2
            aria-hidden="true"
            className="mt-0.5 size-4 shrink-0 animate-spin text-status-working"
          />
        )}
...
function unavailableConversationSnapshot(
  session: WorkspaceSession,
