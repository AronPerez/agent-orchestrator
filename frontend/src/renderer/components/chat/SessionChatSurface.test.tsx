...
function snapshotFor(
  sessionId: string,
): ConversationSnapshot & { capabilities: string[] } {
  return {
...
vi.mock("../../hooks/useConversation", () => ({
  useConversation: (session: { id: string } | undefined) => ({
    ...conversationState,
    ...
    ChatWorkspace: ({
      ...
      return (
        <div>
          ...
          {shellTarget ? (
            <div data-testid="shell-target">{shellTarget.handleId}</div>
          ) : null}
          {switchAgentControl}
        </div>
      );
...
describe("SessionChatSurface link routing", () => {
    ...
    conversationState.snapshot = {
      ...
      turns: [
        ...
        },
        {
          id: "turn-queued",
          state: "queued",
          requestedAt: "2026-08-25T09:00:01Z",
        },
      ],
    };
    const onConversationWorkChange = vi.fn();
  ...
  it("returns to idle after the accepted turn appears in the conversation snapshot", async () => {
    ...
    await waitFor(() => {
      expect(
        conversationCommandState.acknowledgeAcceptedTurn,
      ).toHaveBeenCalledWith("turn-accepted");
      expect(onConversationWorkChange).toHaveBeenLastCalledWith({
  ...
  it("fences new work and agent switching without applying the decision-blocking agent lock", () => {
    ...
    expect(screen.getByTestId("chat-agent-input")).toHaveAttribute(
      "data-disabled",
      "false",
    );
    expect(screen.getByTestId("chat-new-work")).toHaveAttribute(
      "data-disabled",
      "true",
    );
  ...
  it.each([
    ...
    "restores durable %s presentation and locks Chat input after reload",
    async (_name, overrides, outcome, buttonDisabled) => {
      agentSwitchState.data = [
        {
      ...
      if (outcome === "in_progress") {
        ...
        ).toHaveTextContent("Starting target agent");
      } else {
        expect(
          screen.queryByRole("list", { name: "Switching…" }),
  ...
  it("ignores completed switch history when a stopped Chat controller reloads", () => {
    ...
    queryClient.setQueryData(agentSwitchesQueryKey(session), [
      historicalSwitch,
    ]);
    ...
    expect(
      screen.queryByTestId("chat-agent-switch-status"),
    ).not.toBeInTheDocument();
    expect(screen.getByTestId("chat-agent-input")).toHaveAttribute(
      "data-disabled",
      "false",
    ...
    expect(
      screen.getByRole("button", { name: "Switch agent" }),
    ).not.toHaveAttribute("data-outcome");
  });
  ...
  it("keeps failure visible until a retry settles, then ignores a later controller stop", async () => {
    ...
    const view = render(
      <Wrapper client={queryClient}>
        <SessionChatSurface
          session={{ ...session, activeAgentSwitch: activeSwitch }}
        />
      </Wrapper>,
    );
    ...
    view.rerender(
      <Wrapper client={queryClient}>
        <SessionChatSurface
          session={{ ...session, activeAgentSwitch: retrySwitch }}
        />
      </Wrapper>,
    );
    expect(screen.getByTestId("chat-agent-switch-status")).toHaveAttribute(
    ...
    view.rerender(
      <Wrapper client={queryClient}>
        <SessionChatSurface
          session={{
            ...session,
            activeAgentSwitch: completedRetry,
  ...
  it("remounts the chat workspace when switching between chat sessions", () => {
    const first = {
      ...session,
      id: "proj-orchestrator-1",
      kind: "orchestrator" as const,
    };
    const second = {
      ...session,
      id: "proj-orchestrator-2",
      kind: "orchestrator" as const,
    };
    const queryClient = new QueryClient({
      defaultOptions: {
    ...
    expect(
      screen.getByText("Rendered proj-orchestrator-1"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Rendered proj-orchestrator-2"),
    ).toBeInTheDocument();
    expect(
      screen.queryByText("Mounted proj-orchestrator-1"),
    ).not.toBeInTheDocument();
  });
});
