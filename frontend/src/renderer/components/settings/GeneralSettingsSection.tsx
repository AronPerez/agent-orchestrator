...
import {
  useSettings,
  useUpdateCloudOffering,
  useUpdateSessionInterface,
...
function SessionInterfaceRow() {
  ...
  const note =
    saveError ??
    error ??
    (!chatAvailable ? t("settings.sessionInterface.unavailable") : null);

  return (
    <div className="flex w-full flex-col">
      <SettingsRow
        className="rounded-none"
        label={t("settings.sessionInterface.label")}
      >
        <SettingsOptionMenu
...
export function GeneralSettingsSection({
  ...
  const soundNotificationsEnabled = useSoundNotificationsStore(
    (state) => state.enabled,
  );
  const setSoundNotificationsEnabled = useSoundNotificationsStore(
    (state) => state.setEnabled,
  );
  const soundNotificationsSaving = useSoundNotificationsStore(
    (state) => state.saving,
  );
  const soundNotificationsSaveError = useSoundNotificationsStore(
