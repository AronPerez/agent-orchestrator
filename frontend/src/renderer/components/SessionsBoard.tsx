...
export function SessionsBoard({ projectId }: SessionsBoardProps) {
	...
	const isDaemonReady =
		usesPreviewWorkspaceData || hasBrowserDaemon || (shell ? shell.daemonStatus.state === "ready" : true);
			...
			<div className="min-h-0 flex-1 overflow-y-auto md:overflow-hidden">
				...
				{showStartup ? (
					...
					<div className="md:h-full md:overflow-x-auto md:overflow-y-hidden">
						...
						<div className="relative flex flex-col divide-y divide-border-strong md:grid md:h-full md:min-w-[64rem] md:grid-cols-4 md:divide-x md:divide-y-0 xl:min-w-0">
							<div
								aria-hidden="true"
								className="pointer-events-none absolute inset-x-0 top-12 z-10 hidden border-t border-border-strong md:block"
							/>
							{COLUMNS.map((col) => (
								<BoardColumn
