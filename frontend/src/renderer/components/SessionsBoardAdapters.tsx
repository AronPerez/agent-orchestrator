...
export function toBoardSessionPresentation(
  ...
  const switchVisual = switchPresentation
    ? agentSwitchStatusVisual(switchPresentation)
    : undefined;
  return {
...
const DesktopSessionCard = memo(function DesktopSessionCard({
  ...
  const summaries = sessionPRDisplaySummaries(
    session,
    useSessionScmSummary(session).data,
  );
  const termination = useTerminateSessionState(session);
  const showTerminate =
    interactive && session.isTerminated !== true && onTerminate;
  const keepTerminateVisible = session.status === "merged";
  const usagePresentation = toUsagePresentation(usage, t);
  const translate: ProductUITranslator = (key, values) =>
    t(key as MessageKey, values);
...
function toUsagePresentation(
  ...
  const cost =
    formatEstimatedCost(usage.estimatedCost) ?? t("usage.unavailable");
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        ...
        >
          <RotateCcw
            className={cn("size-icon-md", isRestoring && "animate-spin")}
            aria-hidden="true"
          />
        </button>
      </TooltipTrigger>
...
function ArchiveRestoreError({ message }: { message?: string }) {
  return message ? (
    <div
      className="border-t border-border px-2 py-1.5 text-2xs text-destructive"
      role="alert"
    >
      {message}
    </div>
