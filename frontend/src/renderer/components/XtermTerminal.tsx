...
function configureScrollbarReservation(term: Terminal): void {
  ...
  if (viewport)
    viewport.scrollBarWidth = isMacPlatform()
      ? MAC_TERMINAL_SCROLLBAR_WIDTH
      : 0;
}
...
export function XtermTerminal(props: XtermTerminalProps) {
  ...
  const notifyCursorSchemeRef = useRef<
    (scheme: Theme, force?: boolean, retry?: boolean) => void
  >(() => {});
  ...
  useEffect(() => {
    ...
    const disposeFontSizeShortcut = aoBridge.terminal.onFontSizeShortcut(
      (delta) => {
    ...
    term.loadAddon(
      new WebLinksAddon(activateLink, { hover: trackHover, leave: clearHover }),
    );
    ...
    const revealScrollbar = () => {
      if (!scrollbarTrack || scrollbarTrack.dataset.scrollable !== "true")
        return;
    ...
    const scrollPositionChange = scrollbarTrack
      ...
      : null;
    const scrollbarResize = scrollbarTrack
      ? term.onResize(scheduleScrollbarUpdate)
      : null;
    const scrollToPointer = (clientY: number) => {
      if (!scrollbarTrack || !scrollbarThumb) return;
      const maxScrollLine =
        term.buffer.active.type === "normal" ? term.buffer.active.baseY : 0;
      const trackRect = scrollbarTrack.getBoundingClientRect();
      const travel =
        trackRect.height - scrollbarThumb.getBoundingClientRect().height;
      if (maxScrollLine <= 0 || travel <= 0) return;
      const thumbTop = Math.min(
        travel,
        Math.max(0, clientY - trackRect.top - scrollbarThumb.offsetHeight / 2),
      );
    ...
    const scrollbarPointerMove = (event: PointerEvent) => {
      ...
      if (maxScrollLine <= 0 || travel <= 0) return;
      const lineDelta =
        ((event.clientY - scrollbarDrag.startY) / travel) * maxScrollLine;
    ...
    const userInputListeners = new Set<
      (data: string, source: TerminalUserInputSource) => void
    >();
    ...
    const scheduleStableFit = (allowHidden = false, onSettled?: () => void) => {
      ...
      if (fitCapTimer === null)
        fitCapTimer = setTimeout(flushScheduledFit, FIT_CAP_MS);
    };
    ...
    const settleTimers = [50, 250, 600, 1200].map((ms) =>
      window.setTimeout(scheduleVisibleFit, ms),
    );
    ...
    term.attachCustomWheelEventHandler((event) => {
      ...
      if (event.deltaMode === 1 /* DOM_DELTA_LINE */) {
        ...
        lines =
          (Math.trunc(event.deltaY) || Math.sign(event.deltaY)) * term.rows;
      } else {
        const rowHeight =
          (term.options.fontSize ?? TERMINAL_FONT_SIZE_DEFAULT) *
      ...
      void (async () => {
        ...
        for (const file of files) {
          try {
            ...
            const saved = await aoBridge.terminal.saveDroppedFile({
              name: file.name,
              bytes,
            });
    ...
    const prepareForActivation = (): Promise<void> => {
      ...
      return new Promise((resolve) => {
        ...
        const finish = () => {
          ...
          if (cancelActivationPreparation === finish)
            cancelActivationPreparation = null;
          resolve();
        };
    ...
    <>
      ...
      >
        <div
          ...
          className={
            macPlatform
              ? "terminal-xterm-host terminal-xterm-host--mac"
              : "terminal-xterm-host"
          }
          style={{
        ...
        {macPlatform ? (
          ...
          >
            <div
              className="terminal-scrollbar__thumb"
      ...
      >
        ...
        >
          ...
          <DropdownMenuItem onSelect={() => runContextMenuAction("paste")}>
            {t("titlebar.paste")}
          </DropdownMenuItem>
