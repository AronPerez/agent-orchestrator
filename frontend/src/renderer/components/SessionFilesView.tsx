...
export function SessionFilesView({
	...
	const workspaceConnectionState = useWorkspaceFileConnectionState(session);
	const filesQuery = useQuery({
		...sessionWorkspaceFilesQueryOptions(session, t("files.error.loadWorkspace")),
		refetchInterval: workspaceFilesRefetchInterval(workspaceConnectionState),
	});
	useEffect(
		() => subscribeWorkspaceFileChanges(session, queryClient),
...
function ReviewFileCard({
	...
	const detailQuery = useQuery({
		...
		queryKey: ["session-workspace-file", refKey(session), file.path, section],
		enabled: expanded && !selectionOrMenuActive,
		queryFn: () => loadWorkspaceFile(session, file.path, section, t),
	});
...
async function loadWorkspaceFile(session: Ref, path: string, section: WorkspaceSectionKey | undefined, t: TFunction) {
	const { data, error } = await clientFor(session.host).GET("/api/v1/sessions/{sessionId}/workspace/file", {
		params: { path: { sessionId: session.id }, query: { path, section } },
	});
