...
export function CommandPalette() {
  ...
  const { cloneProject, createProject, initializeProjectRepository } =
    useShell();
  ...
  const [reviewStatesSnapshot, setReviewStatesSnapshot] =
    useState<Readonly<Record<string, PRReviewState[]>>>();
  const [pendingDismiss, setPendingDismiss] = useState<null | "pop" | "close">(
    null,
  );
  ...
  const currentSession =
    params.hostId && params.sessionId
      ? findSession(workspaces, { host: params.hostId, id: params.sessionId })
          ?.session
      : undefined;
  ...
  const sessionsWithOpenPRs = useMemo(
    () =>
      workspaces.flatMap((workspace) =>
        workerSessions(workspace.sessions).filter(
          (session) => openPRs(session).length > 0,
        ),
      ),
    [workspaces],
  );
    ...
    combine: (results) => {
      ...
      results.forEach((result, index) => {
        ...
        if (session && result.data && !result.isFetching && !result.isError) {
          reviewStatesBySessionId[refKey(session)] = result.data.reviews ?? [];
        }
      });
  ...
  useEffect(() => {
    ...
    setReviewStatesSnapshot((previous) => {
      let changed = false;
      const next = { ...previous };
      for (const [sessionId, reviews] of Object.entries(
        reviewQuerySummary.reviewStatesBySessionId,
      )) {
  ...
  const scoped = useMemo(
    () =>
      view.mode === "session-actions"
        ? findSession(workspaces, view.session)
        : undefined,
    [view, workspaces],
  );
      ...
      return [
        {
          id: "actions",
          label: "",
          items: filterCommands(sessionActionItems, query),
        },
      ];
    }
      ...
      switch (target.to) {
        ...
        case "/host/$hostId/project/$projectId/settings":
          useUiStore
            .getState()
            .openProjectSettings({
              host: target.params.hostId,
              id: target.params.projectId,
            });
          break;
  ...
  const blockedByRestart = useCallback(
    (project: Ref) => {
      if (!useUiStore.getState().restartingProjectIds.has(refKey(project)))
        return false;
  ...
  const openOrchestrator = useCallback(
    async (project: Ref) => {
      ...
      const workspace = workspaces.find(
        (candidate) =>
          candidate.host === project.host && candidate.id === project.id,
      );
  ...
  const resumeSession = useCallback(
    async (session: Ref) => {
      ...
      if (result.status === "not_resumable")
        return t("command.resumeNotResumable");
      ...
      try {
        switch (action.kind) {
          ...
          case "trigger-review": {
            ...
            if (triggerError)
              throw new Error(
                apiErrorMessage(triggerError, "Unable to start review"),
              );
            await queryClient.invalidateQueries({
              queryKey: ["session-reviews", refKey(action.session)],
            });
            await queryClient.invalidateQueries({
        ...
        {view.mode === "new-task" ? (
          <div onKeyDown={(event) => event.stopPropagation()}>
            ...
            <TaskComposer
              ...
              onCreated={(sessionId) =>
                void handleTaskCreated(view.project, sessionId)
              }
            />
          </div>
        ) : (
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <CommandItem
                        ...
                        disabled={
                          item.disabled ||
                          (pendingId !== null && pendingId !== item.id)
                        }
