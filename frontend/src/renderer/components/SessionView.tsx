...
export function SessionView({ sessionRef }: SessionViewProps) {
	...
	useEffect(() => setReviewFileTarget(undefined), [sessionKey]);
	const [filesFocusPath, setFilesFocusPath] = useState<string | null>(null);
	const browserPopOutPhase = browserPopOutState.sessionKey === sessionKey ? browserPopOutState.phase : "docked";
	...
	}, [hasBrowserContent, hasInspector, session, sessionKey, initializeInspectorSession]);
	...
	const handleToggleBrowserPopOut = useCallback(
		...
		setBrowserPopOutState((current) => {
				if (next) {
					if (current.sessionKey === sessionKey && current.phase !== "docked") return current;
					return {
						sessionKey,
			...
			{browserPoppedOut && session
				? createPortal(
						<div
							...
							style={
								browserPopOutState.sessionKey === sessionKey && browserPopOutState.dockRect
									? ({
