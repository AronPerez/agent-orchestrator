...
function renderWithQuery(children: ReactNode) {
  ...
  act(() => {
    for (const callback of resizeCallbacks)
      callback(
        [{ contentRect: { width: 400, height: 400 } }] as never,
        {} as ResizeObserver,
      );
  });
  return view;
...
describe("FileTree", () => {
  ...
  it("lists the root directory's entries", async () => {
    ...
    expect(
      screen.getByRole("tree", { name: "File tree" }).firstElementChild,
    ).toHaveClass("board-scrollbar");
  });
  ...
  it("loads nested directories while searching before they have been expanded", async () => {
    getMock.mockImplementation(async (_path: string, options: unknown) => {
      ...
      if (query === "src/nested") {
        return treeResponse("src/nested", [
          {
            name: "target.ts",
  ...
  it("renders the precomputed changed-only tree without calling the tree endpoint", async () => {
    const changedOnlyData: TreeNode[] = [
      { name: "notes.txt", path: "notes.txt", type: "file", status: "added" },
    ];
