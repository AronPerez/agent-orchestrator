            ...
            {isSessionRoute && isOrchestrator ? (
              <div className="inline-flex min-w-0 items-center gap-2">
                <div className="inline-flex min-w-0 items-center gap-1.5">
                  ...
                  <span
                    aria-hidden="true"
                    className="text-xs leading-none text-passive"
                  >
                    ·
                  </span>
...
const projectBoardLabelTransition = {
...
function ProjectBoardLabelButton({
  ...
  return (
    <button
      aria-label={t("shell.openProjectDashboard", { name: label })}
      className={cn(
        "min-w-0 rounded-sm text-left hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
        className,
      )}
      onClick={onOpen}
      style={style}
...
function ProjectTerminationFeedback({
  ...
  return (
    <div
      aria-label={t("shell.sessionTerminationStatus")}
      className="flex max-w-content-max items-center gap-2"
    >
      {states.map((state) =>
        state.error ? (
          <TopbarKillError
