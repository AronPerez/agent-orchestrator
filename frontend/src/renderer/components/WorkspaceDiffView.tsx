...
function emptyDiffMessage(
  compareMode: WorkspaceCompareMode | undefined,
  t: TFunction,
): string {
  return compareMode === "base"
    ? t("files.noChangesBase")
    : t("files.noChangesHead");
}
...
function useSharedScrollRowVirtualizer(
  ...
  useEffect(() => {
    if (!enabled) return;
    setScrollElement(
      scrollContainerRef.current?.closest<HTMLElement>(
  ...
  useEffect(() => {
    ...
    measure();
    const resizeTarget =
      scrollElement.querySelector<HTMLElement>(".session-files-review-list") ??
      scrollElement;
...
function DiffView({
  ...
  const { listRef, virtualizer } = useSharedScrollRowVirtualizer(
    containerRef,
    rows.length,
    shouldVirtualize,
  );
  ...
  const runs = useMemo(
    () =>
      rows.map((row, index) =>
        row.kind === "del"
          ? highlight.oldSide[index]
          : highlight.newSide[index],
      ),
    [rows, highlight],
  );
  ...
  useEffect(() => {
    const onSelectionChange = () => {
      setHasSelection(
        isSelectionActiveIn(window.getSelection(), containerRef.current),
      );
    };
  ...
  return (
    <div>
      ...
      <DiffSelectionMenu
        ...
        onOpenChange={(open) =>
          setMenuState((current) => (current ? { ...current, open } : current))
        }
        open={menuOpen}
...
const HunkBand = memo(function HunkBand({ row }: { row: DiffRow }) {
  return (
    <div className="flex select-none items-baseline gap-2 bg-surface-faint px-2.5 py-0.75 text-passive">
      ...
      {row.section ? (
        <span className="min-w-0 truncate text-passive/90">{row.section}</span>
      ) : null}
    </div>
  );
});
    ...
    <div>
      ...
      >
        <LineFeedbackButton
          ...
          onClick={() =>
            annotation.begin(
              lineAnnotationTarget(path, previousPath, row, index),
            )
          }
          t={t}
...
const DiffRowContent = memo(DiffRowContentInner, (prev, next) => {
  ...
  const prevActive = isAnnotationRow(
    prev.annotation.target,
    prev.path,
    prev.index,
  );
  const nextActive = isAnnotationRow(
          ...
          <div key={`sp${index}`}>
            <div className="grid grid-cols-2 divide-x divide-border/40">
              <SplitSide
                ...
                runs={
                  splitRow.leftIndex === null
                    ? null
                    : oldRuns[splitRow.leftIndex]
                }
                side="old"
                t={t}
              />
              <SplitSide
                ...
                runs={
                  splitRow.rightIndex === null
                    ? null
                    : newRuns[splitRow.rightIndex]
                }
                side="new"
                t={t}
              />
            ...
            {(splitRow.leftIndex !== null &&
              isAnnotationRow(annotation.target, path, splitRow.leftIndex)) ||
            (splitRow.rightIndex !== null &&
              isAnnotationRow(annotation.target, path, splitRow.rightIndex)) ? (
              <FileAnnotationComposer annotation={annotation} />
            ) : null}
          </div>
        ),
...
function SplitSide({
  ...
  if (!row || rowIndex === null || !runs)
    return <div className="bg-surface-faint/20" aria-hidden="true" />;
  const lineNo = side === "old" ? row.oldNo : row.newNo;
  const tone = row.kind === "hunk" ? "" : diffRowTone[row.kind];
  const target =
    lineNo == null
      ? null
      : lineAnnotationTarget(path, previousPath, row, rowIndex, side);
  return (
    <div
...
function LineFeedbackButton({
  ...
  const label = t("files.addLineFeedback", {
    file: target.path,
    line: target.line,
    side,
  });
...
export function FileAnnotationComposer({
  annotation,
}: {
  annotation: FileAnnotationModel;
}) {
  ...
  const side =
    target.side === "file"
      ? ""
      : t(target.side === "old" ? "files.oldSide" : "files.newSide");
  const targetLabel =
    target.side === "file"
      ? t("files.fileFeedbackTarget", { file: target.path })
      : t("files.lineFeedbackTarget", {
          file: target.path,
          line: target.line,
          side,
        });
  ...
  return (
    ...
    >
      <div className="mb-1.5 flex items-center justify-between gap-2">
        <span className="min-w-0 truncate font-mono text-caption text-passive">
          {targetLabel}
        </span>
      ...
      <textarea
        ...
        disabled={
          annotation.status === "sending" || annotation.status === "sent"
        }
      ...
      <div className="mt-2 flex items-center justify-end gap-1.5">
        <span className="mr-auto text-caption text-passive">
          {t("files.feedbackShortcut")}
        </span>
        <Button
          disabled={
            annotation.status === "sending" || annotation.status === "sent"
        ...
        <Button
          disabled={
            !annotation.draft.trim() ||
            annotation.status === "sending" ||
            annotation.status === "sent"
          }
          size="sm"
          type="submit"
...
export function ChangeBadges({
  ...
  return (
    <span className="flex shrink-0 items-center gap-1 font-mono text-caption font-medium">
      {additions > 0 ? (
        <span className="px-0.5 text-success">+{additions}</span>
      ) : null}
      {deletions > 0 ? (
        <span className="px-0.5 text-error">-{deletions}</span>
      ) : null}
    </span>
  );
}
