...
const SECTION_ROW_INTERACTIVE_CLASS =
  "transition-colors hover:bg-interactive-hover hover:text-foreground";
...
export const sessionDndId = (project: Ref) =>
  `sidebar-sessions-${refKey(project)}`;
...
function sortableRowStyle({
...
}: Pick<SortableRow, "transform" | "transition" | "isDragging"> & {
  ...
  return {
    ...
    transition:
      isDragging || dropTransitionDisabled
        ? "none"
...
const projectBlockCollision: CollisionDetection = (args) => {
  ...
  for (const container of args.droppableContainers) {
    ...
    if (!closest || distance < closest.distance)
      closest = { container, rect, distance };
  }
...
function useGrabbingCursor(active: boolean) {
  useEffect(() => {
    ...
    return () =>
      document.documentElement.classList.remove("sidebar-reordering");
  }, [active]);
}
...
function readExpandedProjectIds(): ReadonlySet<string> {
  ...
  try {
    const value: unknown = JSON.parse(
      window.localStorage.getItem(expandedProjectsStorageKey) ?? "null",
    );
    return new Set(
      Array.isArray(value)
        ? value
            ...
            .map((id) =>
              id.includes(":") ? id : refKey({ host: LOCAL_HOST, id }),
            )
        : [],
    );
  } catch {
...
function useSelection() {
  ...
  const goGlobalSettings = useCallback(
    () => openGlobalSettings(),
    [openGlobalSettings],
  );
  return {
...
export function Sidebar({
  ...
  const selectedHost =
    hostFilter !== null && hostSections.some(({ host }) => host === hostFilter)
      ? hostFilter
      : null;
  ...
  const workspaces = visibleHostSections.flatMap(
  ...
  const [
    dismissedInitialActiveProjectIds,
    setDismissedInitialActiveProjectIds,
  ] = useState<ReadonlySet<string>>(() => new Set());
  const toggleProjectDisclosure = (key: string) => {
    const routeFallbackActive =
      initialActiveSessionProjectKey === key &&
      !dismissedInitialActiveProjectIds.has(key);
    const currentlyExpanded = expandedIds.has(key) || routeFallbackActive;
    setExpandedIds((prev) => {
    ...
    setDismissedInitialActiveProjectIds((prev) => {
      if (initialActiveSessionProjectKey !== key) return prev;
      const next = new Set(prev);
      currentlyExpanded ? next.add(key) : next.delete(key);
      return next;
    });
  };
  ...
  const isNightly =
    typeof appVersion === "string" && appVersion.includes("-nightly.");
  ...
  const [sessionOrderByProject, setSessionOrderByProject] = useState<
  ...
  const projectDropTargetRef = useRef<{
    overId: string;
    placement: ProjectDropPlacement;
  } | null>(null);
  ...
  const activeDragWorkspace = useMemo(
    () =>
      orderedWorkspaces.find(
        (workspace) => refKey(workspace) === projectDragState.activeId,
      ) ?? null,
  ...
  const activeDragSessions = useMemo(
    () =>
      activeDragWorkspace
        ? applyOrder(
            sortedWorkerSessions(activeDragWorkspace.sessions).filter(
              (session) => session.isTerminated !== true,
            ),
            refKey,
  ...
  const recordSessionOrder = useCallback((project: Ref, order: string[]) => {
    setSessionOrderByProject((previous) => ({
      ...previous,
      [refKey(project)]: order,
    }));
  }, []);
  const restrictProjectOverlayToRows = useCallback<Modifier>(
    ({ transform }) => {
      ...
      return {
        ...
        x: 0,
        y: bounds
          ? Math.min(bounds.maxY, Math.max(bounds.minY, transform.y))
          : transform.y,
        scaleX: 1,
        scaleY: 1,
      };
  ...
  const onProjectDragEnd = useCallback(
    ({ active, over }: DragEndEvent) => {
      ...
      if (over) {
        ...
        const placement =
          projectDropTargetRef.current?.overId === targetId
            ? projectDropTargetRef.current.placement
            : (projectDragState.placement ??
              (projectIds.indexOf(projectId) < projectIds.indexOf(targetId)
                ? "after"
                : "before"));
        commitProjectOrder(
  ...
  const onProjectDragStart = useCallback(({ active }: DragStartEvent) => {
    ...
    const activeRow = blocks
      .find((block) => block.dataset.projectId === projectId)
    ...
    if (activeRow && blocks.length > 0) {
      ...
      projectDragBoundsRef.current = {
        ...
        maxY:
          blocks[blocks.length - 1].getBoundingClientRect().bottom - activeTop,
      };
    }
    setProjectDragState({
      activeId: projectId,
  ...
  const updateProjectDropTarget = useCallback(
    ({
      active,
      activatorEvent,
      delta,
      over,
    }: DragMoveEvent | DragOverEvent) => {
      ...
      const placement: ProjectDropPlacement =
        boundaryReference === null
          ? projectIds.indexOf(activeId) < projectIds.indexOf(overId!)
            ? "after"
            : "before"
          : boundaryReference <= over.rect.top + over.rect.height / 2
            ? "before"
            : "after";
      projectDropTargetRef.current = { overId: overId!, placement };
      const changesOrder =
        reorderAtProjectBoundary(projectIds, activeId, overId!, placement) !==
        null;
  ...
  return (
    ...
    >
              ...
              >
                {pinnedSessions.map((session) => (
                  <PinnedSessionRow
                    ...
                    session={session}
                    active={
                      selection.activeHostId === session.host &&
                  ...
                  {visibleHostSections.flatMap((section) =>
                    section.status === "failed"
                      ...
                      : [
                          ...(section.streamState === "disconnected"
                            ? [
                                <HostStreamOfflineNotice
                                  key={`stale:${section.host}`}
                                  section={section}
                                />,
                              ]
                            : []),
                          ...orderedWorkspaces
                            .filter(
                              (workspace) => workspace.host === section.host,
                            )
                            .map((workspace) => {
                              const projectKey = refKey(workspace);
                              return (
                                <ProjectItem
                                  key={projectKey}
                                  workspace={workspace}
                                  ...
                                  expanded={
                                    expandedIds.has(projectKey) ||
                                    (initialActiveSessionProjectKey ===
                                      projectKey &&
                                      !dismissedInitialActiveProjectIds.has(
                                        projectKey,
                                      ))
                                  }
                                  suppressInitialExpandAnimation={expandedIds.has(
                                    projectKey,
                                  )}
                                  selection={selection}
                                  draggingProjectId={projectDragState.activeId}
                                  dropIndicator={
                                    projectDragState.overId === projectKey
                                      ? (projectDragState.placement ??
                                        undefined)
                                      : undefined
                                  }
                                  consumeDragClick={
                                    projectDragClickGuard.consumeClick
                                  }
                                  onSessionOrderChange={recordSessionOrder}
                                  onToggle={() =>
                                    toggleProjectDisclosure(projectKey)
                                  }
      ...
      {/* Footer — Settings opens the global settings page directly.
			    ...
			    the final action. */}
      <SidebarFooter className="relative mt-auto gap-0 overflow-hidden border-t border-border-strong px-2 !py-2 transition-[padding] duration-200 ease-linear group-data-[collapsible=icon]:min-h-20 group-data-[collapsible=icon]:items-center group-data-[collapsible=icon]:border-t-0 group-data-[collapsible=icon]:px-1.5 group-data-[collapsible=icon]:!pb-0 group-data-[collapsible=icon]:!pt-1.5">
    ...
    <SidebarMenuItem className="sidebar-expanded-chrome px-2.5 py-2 group-data-[collapsible=icon]:hidden">
      <div role="alert">
        <p className="text-sm font-medium text-foreground">
          {t("hosts.sectionFailed", { host: label })}
        </p>
        <p className="mt-0.5 text-xs text-passive">
          {section.failure ?? t("hosts.unreachable")}
        </p>
        <button
          ...
          onClick={() =>
            void queryClient.invalidateQueries({
              queryKey: workspaceHostQueryKey(section.host),
            })
          }
          type="button"
        >
...
function HostStreamOfflineNotice({ section }: { section: HostSection }) {
  ...
  return (
    <SidebarMenuItem className="sidebar-expanded-chrome px-2.5 py-2 group-data-[collapsible=icon]:hidden">
      <div role="status">
        <p className="text-sm font-medium text-foreground">
          {t("hosts.liveUpdatesOffline", { host: label })}
        </p>
        <p className="mt-0.5 text-xs text-passive">
          {t("hosts.liveUpdatesOffline.hint")}
        </p>
      </div>
    </SidebarMenuItem>
  );
...
type ProjectItemProps = {
  ...
  onSessionOrderChange: (project: Ref, order: string[]) => void;
  onToggle: () => void;
  onRemoveProject: (project: Ref) => Promise<void>;
...
const ProjectItemContent = memo(function ProjectItemContent({
  ...
  const orchestratorActive =
    ...
    workspace.sessions.some(
      (session) =>
        session.id === selection.activeSessionId &&
  ...
  const projectIsDragging = draggingProjectId === refKey(workspace);
  ...
  const visibleSessions = useMemo(
    () =>
      sortedWorkerSessions(workspace.sessions).filter(
        (session) => session.isTerminated !== true,
      ),
    [workspace.sessions],
  );
  ...
  const sessions = useMemo(
    () => applyOrder(visibleSessions, refKey, sessionOrder, "start"),
    [sessionOrder, visibleSessions],
  );
  const sessionIds = useMemo(() => sessions.map(refKey), [sessions]);
  const sessionLayoutDependency = useMemo(
    () => sessionIds.join("\u0000"),
    [sessionIds],
  );
  ...
  const [dropTransitionDisabledId, setDropTransitionDisabledId] = useState<
    string | null
  >(null);
  ...
  const commitSessionOrder = useCallback(
    (next: string[] | null) => {
      ...
      onSessionOrderChange(workspace, next);
    },
    [onSessionOrderChange, workspace],
  );
  ...
  const openSession = useCallback(
    (session: Ref) => selection.goSession(session),
    [selection],
  );
  ...
  const onProjectClick = () => {
    if (consumeDragClick(refKey(workspace))) return;
    if (!expanded) {
      toggleDisclosure();
  ...
  const handleConfirmRemove = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : t("shell.couldNotRemoveProject");
    ...
    <ContextMenu>
      <ContextMenuTrigger asChild>
        <motion.li
          ...
          data-project-id={refKey(workspace)}
          data-project-ref={refKey(workspace)}
              ...
              <div>
                ...
                <SidebarMenuButton
                  ...
                  className={cn(
                    ...
                    draggingProjectId &&
                      "hover:bg-transparent hover:text-muted-foreground active:bg-transparent active:text-muted-foreground",
		...
		Always visible (not hover-gated) to avoid CSS :hover group propagation in Chromium. */}
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <OrchestratorIcon
                        aria-hidden="true"
                        strokeWidth={orchestratorActive ? 2.5 : 2}
                      />
                    </button>
                  </TooltipTrigger>
                  <TooltipContent>
                ...
                <DropdownMenu>
                  ...
                  >
                    <DropdownMenuItem
                      ...
                      onSelect={() => requestNewTask(workspace)}
                    >
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem
                      onSelect={() => selection.goSettings(workspace)}
                    >
                      <Settings aria-hidden="true" />
          ...
          {isRemoving ? (
            <div
              className="sidebar-expanded-chrome px-5 py-1 text-2xs text-muted-foreground"
              role="status"
            >
            ...
            <div
              className="sidebar-expanded-chrome px-5 py-1 text-2xs text-destructive"
              role="alert"
            >
              {removeError}
            </div>
            ...
            {expanded && sessions.length > 0 && (
              <motion.div
                ...
                initial={
                  animReady &&
                  (!suppressInitialExpandAnimation ||
                    hasInteractedWithDisclosure.current)
                    ? { height: 0 }
                    : false
                }
                animate={{ height: "auto" }}
                      ...
                      >
                        {sessions.map((session) => (
                          <SortableSessionRow
                            ...
                            consumeDragClick={
                              sessionDragClickGuard.consumeClick
                            }
                            layoutDependency={sessionLayoutDependency}
                            listIsDragging={sessionDragging}
                            dropTransitionDisabled={
                              dropTransitionDisabledId === refKey(session)
                            }
                            onOpen={openSession}
                          />
                        ))}
...
const ProjectDragPreview = memo(function ProjectDragPreview({
  workspace,
  expanded,
  selection,
  sessions,
  ...
  return (
    <div
      className="pointer-events-none w-full cursor-grabbing"
      data-project-drag-overlay=""
    >
      <div
        className={cn(
          NAV_ROW_CLASS,
          "flex w-full cursor-grabbing items-center gap-2 pr-sidebar-project-actions [&_svg]:size-icon-md",
        )}
        ...
        <div className="ml-3.5 py-1">
          {sessions.map((session) => {
            const switchPresentation =
              deriveSessionAgentSwitchPresentation(session);
            ...
            const active =
              ...
              selection.activeSessionId === session.id;
            return (
              <div className="pl-4.5" key={refKey(session)}>
                <div
                  className={cn(
...
const SortableSessionRow = memo(function SortableSessionRow({
  ...
  return (
    <SessionRow
      ...
      active={active}
      onOpen={() => {
        if (!consumeDragClick(refKey(session))) onOpen(session);
      }}
              ...
              <span className="flex min-w-0 flex-1 items-center gap-1.5">
                <span
                  className={cn(
                    ...
                    active
                      ? "text-foreground"
                      : "text-muted-foreground group-hover/session-row:text-foreground",
                  )}
                  data-session-name=""
                  onDoubleClick={(event) => {
                    event.preventDefault();
                ...
                {switchLabel ? (
                  <span
                    id={switchStatusId}
                    className="max-w-28 shrink-0 truncate text-2xs text-muted-foreground"
                  >
                    {switchLabel}
                  </span>
                ) : null}
...
const SessionMessageAge = memo(function SessionMessageAge({
  session,
}: {
  session: WorkspaceSession;
}) {
  ...
  return (
    <time
      ...
      title={t("shell.lastMessageAt", {
        time: formatTimeCompact(session.lastUserMessageAt),
      })}
    >
...
const SessionActions = memo(function SessionActions({
  ...
  const { mutate: terminateSession, isPending: isKilling } =
    useTerminateSession();
  ...
  if (status.state === "downloading") {
    ...
    return (
      <div
        ...
        className={cn(
          NAV_ROW_CLASS,
          "relative flex w-full items-center text-left [&_svg]:size-icon-md [&_svg]:shrink-0",
        )}
        role="status"
      >
        <span
          className="relative grid size-icon-lg shrink-0 place-items-center"
          aria-hidden="true"
        >
          <svg
    ...
    >
      ...
      <span
        ...
        className={cn(
          ...
          escalated ? "bg-working" : "bg-red-500",
        )}
      />
    </button>
  );
...
function UpdateStatusRail({
  ...
  const { t } = useTranslation();
  if (status.state === "available") {
    const label = t("settings.updates.available", {
  ...
  if (status.state === "downloading") {
    const label = t("settings.updates.downloading", {
  ...
  if (!collapsible) {
    return (
      <div className={cn(SECTION_ROW_CLASS, trailing && "pr-1", className)}>
        <div className="flex min-w-0 flex-1 items-center gap-2">{labelRow}</div>
        {trailing}
      </div>
    );
  }
...
function SidebarSearchButton({ onOpen }: { onOpen: () => void }) {
  ...
  const paletteBinding = effectiveShortcutBindings(
    "command-palette",
    isMac,
    overrides,
  )[0];
...
function CreateProjectButton({
  ...
  SidebarProps,
  "onCloneProject" | "onCreateProject" | "onInitializeProject"
> & { hideTrigger?: boolean }) {
  const { t } = useTranslation();
