...
function renderBoardWithClient(
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <TooltipProvider>
        <SessionsBoard
          project={projectId ? { host, id: projectId } : undefined}
        />
      </TooltipProvider>
    </QueryClientProvider>,
  );
...
describe("SessionsBoard", () => {
  it("restarts the host-qualified project when project ids collide", async () => {
    ...
    workspaceQueryMock.mockReturnValue({
      data: [
        ...
        {
          ...workspaceWithSessions([
            orchestrator("remote-one"),
            orchestrator("remote-two"),
          ]),
          host,
        },
      ],
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Restart on workbox" }),
    );

    await waitFor(() =>
      expect(remotePostMock).toHaveBeenCalledWith("/api/v1/orchestrators", {
  ...
  it("names the target host in a remote session termination prompt", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Terminate remote worker" }),
    );

    expect(await screen.findByRole("dialog")).toHaveTextContent("on workbox");
    expect(
      screen.getByRole("button", { name: "Yes, terminate session on workbox" }),
    ).toBeInTheDocument();
  });
  ...
  it("localizes dynamic card actions and pull request lifecycle labels", async () => {
    ...
    try {
      renderBoard("p1");
      expect(
        screen.getByRole("button", { name: "终止 localized worker" }),
      ).toBeInTheDocument();
  ...
  it("shows the Board identity and compact actions in the in-panel board chrome", () => {
    ...
    expect(screen.getByTestId("board-topbar-label").textContent).toContain(
      "Board",
    );
    ...
    expect(
      screen
        ...
        .closest(".center-panel-titlebar"),
    ).toHaveClass("workspace-topbar-container");
    expect(
      within(screen.getByRole("button", { name: "New task" }))
        .getByText("Task")
  ...
  it.each([
    ...
    "shows %s orchestrator activity in the in-panel board toolbar",
    (state, label, tone, pulses) => {
      boardActionsInPanelMock.mockReturnValue(true);
      workspaceQueryMock.mockReturnValue({
        data: [
          {
      ...
      expect(indicator).toHaveClass(
        pulses ? "animate-status-pulse" : "size-dot-sm",
      );
  ...
  it("labels an idle session as Idle, not Working", () => {
    ...
    const terminateButton = within(idleCard).getByRole("button", {
      name: "Terminate brand-font-pipeline",
    });
    expect(terminateButton).toHaveClass(
      "opacity-0",
    ...
    expect(
      within(idleCard).getByText("Idle").parentElement?.parentElement,
    ).toHaveClass("flex");
    expect(within(idleCard).getByText("brand-font-pipeline")).toHaveClass(
      "font-semibold",
      "line-clamp-2",
    );
  });
  ...
  it("shows coverage-aware cost with tokens on active and archived cards", async () => {
    ...
    const emptyCard = screen
      .getByText("empty worker")
      .closest('[data-testid="board-session-card"]') as HTMLElement;
    const emptyCost = within(emptyCard).getAllByText("Unavailable");
    expect(emptyCost).toHaveLength(2);
    expect(
      emptyCost.some((node) => node.getAttribute("aria-hidden") === "true"),
    ).toBe(true);
    expect(emptyCost.some((node) => node.classList.contains("sr-only"))).toBe(
      true,
    );
    const tokensOnlyCard = screen
      .getByText("tokens worker")
      .closest('[data-testid="board-session-card"]') as HTMLElement;
    expect(
      within(tokensOnlyCard).getByText("Unavailable · 800"),
    ).toHaveAttribute("aria-hidden", "true");
    expect(
      within(tokensOnlyCard).getByText("Unavailable · 800 tokens"),
    ).toHaveClass("sr-only");
    ...
    expect(within(archive).getByText("$0.02 · 1,900 tokens")).toHaveClass(
      "sr-only",
    );
  });
  ...
  it("shows the usage metric as plain text without a tab stop or tooltip", async () => {
    ...
    expect(within(card).getByText("$1.24 · 12,400 tokens")).toHaveClass(
      "sr-only",
    );
    expect(
      within(card).queryByRole("button", { name: /Estimated cost/ }),
  ...
  it("survives a usage payload from a daemon that omits processedTokens", async () => {
    ...
    const usage = await screen.findByText("Unavailable · 84.9M");
    expect(usage).toHaveAttribute("aria-hidden", "true");
    expect(screen.getByText("Unavailable · 84,920,477 tokens")).toHaveClass("sr-only");
  });
  ...
  it("pulses the shared activity indicator on an actively working session card", () => {
    ...
    renderBoard("p1");
    const card = screen
      .getByText("active-card-task")
      .closest('[data-testid="board-session-card"]') as HTMLElement;
    const working = within(card).getByText("Working")
  ...
  it("shows switch progress instead of the exited source on a card", () => {
    ...
    const card = screen
      .getByText("switching worker")
      .closest('[data-testid="board-session-card"]') as HTMLElement;
    const status = within(card).getByText("Switching to Codex")
      .parentElement as HTMLElement;
  ...
  it("uses distinct card badge tones for idle, no signal, and draft PR sessions", () => {
    ...
    renderBoard("p1");
    const idleCard = screen
      .getByText("idle-card-task")
      .closest('[data-testid="board-session-card"]') as HTMLElement;
    const noSignalCard = screen
      .getByText("no-signal-card-task")
      .closest('[data-testid="board-session-card"]') as HTMLElement;
    const draftCard = screen
      .getByText("draft-card-task")
      .closest('[data-testid="board-session-card"]') as HTMLElement;

    expect(within(idleCard).getByText("Idle").parentElement).toHaveClass(
      "text-status-idle",
    );
    expect(
      within(noSignalCard).getByText("No signal").parentElement,
    ).toHaveClass("text-status-unknown");
    expect(within(draftCard).getByText("Draft PR").parentElement).toHaveClass(
  ...
  it("places an exited live session in Needs you with an Exited badge", () => {
    ...
    const needsYouColumn = screen
      .getByText("Needs you")
      .closest("section") as HTMLElement;
    expect(needsYouColumn.firstElementChild).toHaveClass("h-12");
    expect(
      within(needsYouColumn).getByText("agent-exited-task"),
    ).toBeInTheDocument();
    expect(
      within(needsYouColumn).getByText("Exited").parentElement,
    ).toHaveClass("text-status-exited");
  });
  ...
  it("renders an idle-first work lane with a separate lower working section", () => {
    ...
    const workLane = screen.getByRole("region", {
      name: "Idle / Working sessions",
    });
    const idleRegion = within(workLane).getByRole("region", {
      name: "Idle sessions",
    });
    const workingRegion = within(workLane).getByRole("region", {
      name: "Working sessions",
    });
    const reviewRegion = screen.getByRole("region", {
      name: "In review sessions",
    });
    const workSummary = within(workLane).getByRole("group", {
      name: "Idle / Working lane summary",
    });

    expect(
      within(workSummary).getByText("Idle").querySelector("span"),
    ).toHaveClass("bg-status-idle");
    expect(
      within(workSummary).getByText("Working").querySelector("span"),
    ).toHaveClass("bg-status-working");
    ...
    expect(
      within(workLane).getByLabelText("2 idle sessions"),
    ).toHaveTextContent("2");
    expect(
      within(workLane).getByLabelText("1 working session"),
    ).toHaveTextContent("1");
    expect(
      screen.queryByRole("button", { name: /idle sessions/i }),
    ).not.toBeInTheDocument();
    ...
    expect(
      within(idleRegion).getByText("second-idle-task"),
    ).toBeInTheDocument();
    expect(within(workingRegion).getByText("active-task")).toBeInTheDocument();
    expect(
      within(reviewRegion).getByText("idle-with-pr-task"),
    ).toBeInTheDocument();
    expect(
      within(workLane).queryByText("idle-with-pr-task"),
    ).not.toBeInTheDocument();

    const idleCard = screen
      .getByText("idle-no-pr-task")
      .closest('[data-testid="board-session-card"]') as HTMLElement;
  ...
  it("uses one shared scrollbar when a short idle section sits above many working sessions", () => {
    ...
    const laneScrollers = Array.from(
      workLane.querySelectorAll<HTMLElement>(".overflow-y-auto"),
    );
    ...
    expect(
      idleRegion.querySelector(".overflow-y-auto"),
    ).not.toBeInTheDocument();
    expect(
      workingRegion.querySelector(".overflow-y-auto"),
    ).not.toBeInTheDocument();
    expect(
      within(idleRegion).getByText("single-idle-task"),
    ).toBeInTheDocument();
    expect(
      within(workingRegion).getAllByTestId("board-session-card"),
    ).toHaveLength(7);
  });
  ...
  it("lets idle sessions fill the lane when no working sessions exist", () => {
    ...
    expect(within(workLane).getByLabelText("1 idle session")).toHaveTextContent(
      "1",
    );
    expect(
      within(workLane).getByLabelText("0 working sessions"),
    ).toHaveTextContent("0");
    expect(idleRegion).toHaveClass("flex-1");
    expect(within(idleRegion).getByText("idle-task")).toBeInTheDocument();
    expect(
      within(workLane).queryByRole("region", { name: "Working sessions" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("lets working sessions fill the lane when no idle sessions exist", () => {
    ...
    expect(
      within(workLane).getByLabelText("0 idle sessions"),
    ).toHaveTextContent("0");
    expect(
      within(workLane).getByLabelText("2 working sessions"),
    ).toHaveTextContent("2");
    expect(
      within(workLane).queryByRole("region", { name: "Idle sessions" }),
    ).not.toBeInTheDocument();
    expect(workingRegion).toHaveClass("flex-1");
    expect(workingRegion).not.toHaveClass("flex-[2]", "border-t");
    expect(workLane.querySelectorAll(".overflow-y-auto")).toHaveLength(1);
    expect(
      within(workingRegion).getByText("first-working-task"),
    ).toBeInTheDocument();
    expect(
      within(workingRegion).getByText("second-working-task"),
    ).toBeInTheDocument();
  });
  ...
  it("keeps idle and working sections visible when navigating between project boards", () => {
    ...
    const p1Lane = screen.getByRole("region", {
      name: "Idle / Working sessions",
    });
    expect(
      within(p1Lane).getByRole("region", { name: "Idle sessions" }),
    ).toHaveTextContent("p1 idle");
    expect(
      within(p1Lane).getByRole("region", { name: "Working sessions" }),
    ).toHaveTextContent("p1 active");
    ...
    const p2Lane = screen.getByRole("region", {
      name: "Idle / Working sessions",
    });
    expect(screen.queryByText("p1 idle")).not.toBeInTheDocument();
    expect(
      within(p2Lane).getByRole("region", { name: "Idle sessions" }),
    ).toHaveTextContent("p2 idle");
    expect(
      within(p2Lane).getByRole("region", { name: "Working sessions" }),
    ).toHaveTextContent("p2 active");
  });
  ...
  it("shows a static archive card with a persistent restore action", async () => {
    ...
    if (!mergedPr)
      throw new Error("Archived-session fixture requires a pull request");
    archivedSession.prs = [
      {
        ...mergedPr,
        number: 41,
    ...
    expect(archiveButton.parentElement).toHaveClass(
      "absolute",
      "inset-x-0",
      "bottom-0",
      "bg-background",
    ...
    expect(
      screen.getByTestId("board").querySelector(":scope > .min-h-0.flex-1"),
    ).toHaveClass(archiveToggleOffsetClassName);
    const archive = await expandArchive();
    ...
    const terminatedCard = within(archive)
    ...
    expect(
      within(terminatedCard!).queryByRole("button", {
        name: "Open dead worker",
      }),
    ).not.toBeInTheDocument();
    ...
    expect(
      within(terminatedCard!).getByRole("img", { name: "claude-code" }),
    ).toBeInTheDocument();
    ...
    expect(within(prStatus).getByText("merged")).toHaveClass(
      "text-status-merged",
    );
    ...
    expect(
      within(terminatedCard!).getByRole("button", {
        name: "Copy branch ao/dead-worker",
      }),
    ).toBeInTheDocument();
    const divider = terminatedCard!.querySelector(
      "div[aria-hidden='true'].h-px.bg-border",
    );
    expect(divider).not.toBeNull();
    expect(
      divider!.compareDocumentPosition(prStatus) &
        Node.DOCUMENT_POSITION_FOLLOWING,
    ).not.toBe(0);
    expect(
      screen.getByText("ao/dead-worker").compareDocumentPosition(divider!) &
        Node.DOCUMENT_POSITION_FOLLOWING,
    ).not.toBe(0);
    expect(
      screen.getByRole("button", { name: "Restore dead worker" }),
    ).toBeInTheDocument();

    expect(
      screen.queryByRole("group", { name: "Archive layout" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("keeps archive cards mounted after collapse so reopen does not remount them", async () => {
    ...
    expect(
      screen.queryByRole("list", { name: "Archived sessions" }),
    ).not.toBeInTheDocument();
  ...
  it("renders archived sessions as a grid even when rows were previously saved", async () => {
    ...
    expect(restore.closest("[role='listitem']")).toContainElement(
      screen.getByText("Terminated"),
    );
    expect(
      screen.queryByRole("button", { name: "Open dead worker" }),
    ).not.toBeInTheDocument();
  });
      ...
      expect(notificationShowMock).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          body: expect.stringContaining(
            "started a new conversation from the saved prompt",
          ),
        }),
      ),
    );
  ...
  it("keeps restore actions visible and disables siblings while one session is restoring", async () => {
    let finishRestore:
      | ((value: { data: Record<string, never> }) => void)
      | undefined;
    ...
    workspaceQueryMock.mockReturnValue({
      data: [
        workspaceWithSessions([
          terminatedSession(),
          terminatedSession({ id: "s-other", title: "other worker" }),
        ]),
      ],
      isError: false,
      isSuccess: true,
    ...
    const restoringButton = screen.getByRole("button", {
      name: "Restore dead worker",
    });
    const otherButton = screen.getByRole("button", {
      name: "Restore other worker",
    });
  ...
  it("opens the restore-unavailable dialog when a session is not resumable", async () => {
    postMock.mockResolvedValueOnce({
      error: { code: "SESSION_NOT_RESUMABLE" },
    });
    workspaceQueryMock.mockReturnValue({
      data: [workspaceWithSessions([terminatedSession()])],
      isError: false,
    ...
    expect(
      await screen.findByText("Session can no longer be restored"),
    ).toBeInTheDocument();
  });

  it("shows an archive row error when restore fails", async () => {
    postMock.mockResolvedValueOnce({
      error: { code: "RESTORE_FAILED", message: "boom" },
    });
    workspaceQueryMock.mockReturnValue({
      data: [workspaceWithSessions([terminatedSession()])],
      isError: false,
    ...
    expect(
      await screen.findByText("Unable to restore session"),
    ).toBeInTheDocument();
  ...
  it("shows a merged-only lane and opens its card without showing restore", async () => {
    ...
    const mergeLane = screen.getByRole("region", {
      name: "Ready to merge / Merged sessions",
    });
    const mergedRegion = within(mergeLane).getByRole("region", {
      name: "Merged sessions",
    });
    const mergeSummary = within(mergeLane).getByRole("group", {
      name: "Ready to merge / Merged lane summary",
    });
    expect(
      within(mergeSummary).getByText("Ready to merge").querySelector("span"),
    ).toHaveClass("bg-status-ready");
    expect(
      within(mergeSummary).getByText("Merged").querySelector("span"),
    ).toHaveClass("bg-status-merged");
    expect(
      within(mergeLane).getByLabelText("0 ready to merge sessions"),
    ).toHaveTextContent("0");
    expect(
      within(mergeLane).getByLabelText("1 merged session"),
    ).toHaveTextContent("1");
    expect(
      within(mergeLane).queryByRole("region", {
        name: "Ready to merge sessions",
      }),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.queryByRole("button", { name: "Restore merged worker" }),
    ).not.toBeInTheDocument();
  ...
  it("splits ready and merged sessions into upper and lower regions", () => {
    ...
    const readyRegion = within(mergeLane).getByRole("region", {
      name: "Ready to merge sessions",
    });
    const mergedRegion = within(mergeLane).getByRole("region", {
      name: "Merged sessions",
    ...
    expect(
      within(mergeLane).getByLabelText("1 ready to merge session"),
    ).toHaveTextContent("1");
    expect(
      within(mergeLane).getByLabelText("1 merged session"),
    ).toHaveTextContent("1");
  ...
  it("uses the shared minimal scrollbar styling for every Kanban lane", () => {
    ...
    const laneScrollers = screen
      ...
      .flatMap((column) =>
        Array.from(column.querySelectorAll<HTMLElement>(".overflow-y-auto")),
      );
  ...
  it("archives a terminated merged runtime without duplicating it in the merged lane", async () => {
    workspaceQueryMock.mockReturnValue({
      data: [
        workspaceWithSessions([
          ...
          terminatedSession({
            id: "s-archived-merged",
            title: "archived merged worker",
            status: "merged",
          }),
        ]),
      ],
    ...
    const mergedRegion = screen.getByRole("region", {
      name: "Merged sessions",
    });
    expect(
      within(mergedRegion).getByText("live merged worker"),
    ).toBeInTheDocument();
    expect(
      within(mergedRegion).queryByText("archived merged worker"),
    ).not.toBeInTheDocument();
    ...
    expect(
      within(archivedMergedCard!).queryByRole("button", {
        name: "Open archived merged worker",
      }),
    ).not.toBeInTheDocument();
    expect(
      within(archivedMergedCard!).queryByRole("button", {
        name: "Terminate archived merged worker",
      }),
    ).not.toBeInTheDocument();
    expect(
      within(archivedMergedCard!).getByText("Merged").parentElement,
    ).toHaveClass("text-status-merged");
    expect(
      within(archive).getByRole("button", {
        name: "Restore archived merged worker",
      }),
    ).toBeInTheDocument();
  });
  ...
  it("asks for confirmation when terminating an ordinary live session from its card", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Terminate idle worker" }),
    );

    expect(navigateMock).not.toHaveBeenCalled();
    expect(
      screen.getByRole("dialog", { name: "Terminate idle worker?" }),
    ).toBeInTheDocument();
  });
  ...
  it("terminates a live merged session from its card without opening the session", async () => {
    ...
    await userEvent.click(
      within(dialog).getByRole("button", { name: "Yes, terminate session" }),
    );
  ...
  it("keeps only the targeted card disabled while its termination is pending", async () => {
    let finishKill!: (value: {
      data: { ok: boolean; sessionId: string };
      error: undefined;
    }) => void;
    postMock.mockReturnValueOnce(
      new Promise((resolve) => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Terminate worker one" }),
    );
    await userEvent.click(
    ...
    expect(
      screen.getByRole("button", { name: "Killing worker one" }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: "Killing worker one" }),
    ).toHaveClass("opacity-100");
    expect(
      screen.getByRole("button", { name: "Terminate worker two" }),
    ).toBeEnabled();
  ...
  it("keeps the merged-card confirmation dismissed and surfaces termination failures", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Terminate merged worker" }),
    );
    await userEvent.click(
      within(screen.getByRole("dialog")).getByRole("button", {
        name: "Yes, terminate session",
...
describe("SessionsBoard find bar", () => {
  ...
  it("filters the archive with the same query", async () => {
    ...
    expect(
      within(archive).queryByText("beta archived"),
    ).not.toBeInTheDocument();
  });
