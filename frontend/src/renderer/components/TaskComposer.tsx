...
export function TaskComposer({
	...
	const modelCatalogQuery = useQuery(agentModelsQueryOptions(selectedAgent, project ?? { host: LOCAL_HOST, id: "" }));
	const revalidationQuery = useQuery({
		queryKey: [
			"agent-model-revalidation",
			selectedAgent,
			project ? refKey(project) : "",
			modelCatalogQuery.data?.validatedAt ?? "",
		],
		queryFn: () => revalidateAgentModels(selectedAgent, project ?? { host: LOCAL_HOST, id: "" }),
	...
	useEffect(() => {
		if (revalidationQuery.data) {
			queryClient.setQueryData(
				agentModelsQueryKey(selectedAgent, project ?? { host: LOCAL_HOST, id: "" }),
				revalidationQuery.data,
			);
		}
	}, [project, queryClient, revalidationQuery.data, selectedAgent]);
	const modelWarning =
