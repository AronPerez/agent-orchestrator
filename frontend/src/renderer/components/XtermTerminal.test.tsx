...
const state = vi.hoisted(() => ({
  ...
  searchAddon: null as null | {
    ...
    resultListeners: Set<
      (results: { resultCount: number; resultIndex: number }) => void
    >;
  },
...
vi.mock("@xterm/addon-search", () => ({
  SearchAddon: class FakeSearchAddon {
    ...
    onDidChangeResults(
      listener: (results: { resultCount: number; resultIndex: number }) => void,
    ) {
...
describe("XtermTerminal", () => {
  ...
  it("fades the macOS scrollbar after scrolling goes idle", () => {
    ...
    act(() =>
      state.lastTerminal!.scrollListeners.forEach((listener) => listener()),
    );
  ...
  it("copies selected terminal text on the terminal copy shortcut", () => {
    ...
    expect(window.ao!.clipboard.writeText).toHaveBeenCalledWith(
      "copied selection",
    );
  });
  ...
  it.each([
    ...
    "uses %s plus/minus for terminal font size on %s",
    (_label, platform, ctrlKey, metaKey) => {
      setNavigatorPlatform(platform);
      const onChangeFontSize = vi.fn();
      render(
        <XtermTerminal onChangeFontSize={onChangeFontSize} theme="dark" />,
      );
  ...
  it("opens a themed context menu on right-click and disables Copy without a selection", async () => {
    ...
    expect(fireEvent.contextMenu(host, { clientX: 120, clientY: 88 })).toBe(
      false,
    );
  ...
  it.each([
    ...
    "opens search with %s and consumes the terminal shortcut",
    async (_label, platform, ctrlKey, metaKey) => {
      setNavigatorPlatform(platform);
      render(<XtermTerminal theme="dark" />);
      const event = {
        altKey: false,
      ...
      expect(
        await screen.findByRole("searchbox", { name: "Search terminal" }),
      ).toHaveFocus();
    },
  );
  ...
  it("searches incrementally, reports matches, and navigates in both directions", async () => {
    ...
    act(() =>
      state.searchAddon!.resultListeners.forEach((listener) =>
        listener({ resultCount: 3, resultIndex: 1 }),
      ),
    );
  ...
  it("marks an invalid regular expression without calling xterm search", async () => {
    ...
    expect(screen.getByText("—")).toHaveAccessibleName(
      "Invalid regular expression",
    );
    expect(state.searchAddon!.findNext).not.toHaveBeenCalledWith(
      "[",
      expect.anything(),
    );
  });
  ...
  it("clears search highlights and returns focus to xterm on Escape", async () => {
    ...
    expect(
      screen.queryByRole("searchbox", { name: "Search terminal" }),
  ...
  it("toggles terminal fullscreen from the right-click menu", async () => {
    ...
    fireEvent.contextMenu(
      container.querySelector(".terminal-pane-frame")!.firstElementChild!,
    );
    ...
    expect(
      pane.contains(exitFullscreenItem.closest<HTMLElement>("[role='menu']")),
    ).toBe(true);
    Object.defineProperty(document, "fullscreenElement", {
      configurable: true,
      value: null,
  ...
  it("runs context menu copy and select all against the xterm instance", async () => {
    ...
    await waitFor(() =>
      expect(window.ao!.clipboard.writeText).toHaveBeenCalledWith("menu copy"),
    );
  ...
  it("pastes from the context menu through the terminal paste path", async () => {
    ...
    await waitFor(() =>
      expect(onInput).toHaveBeenCalledWith("menu\rpaste", "paste"),
    );
  ...
  it("honors bracketed paste mode from the context menu", async () => {
    const onInput = vi.fn();
    window.ao!.clipboard.readText = vi
      .fn()
      .mockResolvedValue("bracketed\npaste");
    const { container } = render(
      <XtermTerminal
        theme="dark"
        onReady={(terminal) => terminal.onUserInput(onInput)}
    ...
    await waitFor(() =>
      expect(onInput).toHaveBeenCalledWith(
        "\x1b[200~bracketed\rpaste\x1b[201~",
        "paste",
      ),
    );
  });
  ...
  it("does not show a copied toast when clipboard write fails", async () => {
    window.ao!.clipboard.writeText = vi
      .fn()
  ...
  it("forwards keyboard input from explicit key events", () => {
    ...
    state.lastTerminal!.keyListeners.forEach((listener) =>
      listener({ key: "a" }),
    );
  ...
  it("forwards only validated OSC color replies from raw xterm data", () => {
    const onInput = vi.fn();
    render(
      <XtermTerminal
        theme="dark"
  ...
  it("translates wheel motion into SGR wheel reports for zellij scrollback", () => {
    ...
    const suppressed = state.lastTerminal!.wheelHandler!({
      deltaY: -50,
    } as WheelEvent);

    expect(suppressed).toBe(false);
    expect(onInput).toHaveBeenCalledWith(
      "\x1b[<64;1;1M\x1b[<64;1;1M\x1b[<64;1;1M",
      "wheel",
    );
  });
  ...
  it("handles line- and page-mode wheels (Linux/Windows mice), not just pixel deltas", () => {
    ...
    expect(
      state.lastTerminal!.wheelHandler!({
        deltaY: -1,
        deltaMode: 1,
      } as WheelEvent),
    ).toBe(false);
  ...
  it("scrolls down on positive wheel delta and leaves zoom (ctrl/meta) wheel alone", () => {
    ...
    expect(
      state.lastTerminal!.wheelHandler!({ deltaY: 20 } as WheelEvent),
    ).toBe(false);
  ...
  it("scrolls xterm's own viewport for normal-buffer panes with mouse tracking off (codex, plain shell)", () => {
    ...
    expect(
      state.lastTerminal!.wheelHandler!({ deltaY: -50 } as WheelEvent),
    ).toBe(false);
  ...
  it.each([
    ...
    "opens %s web links in the system browser on %s+Click",
    (kind, _modifier, event) => {
      ...
      const handler =
        kind === "plain" ? state.linkHandler! : oscHandler.activate;
  ...
  it("forces plain drag selection without raw xterm data forwarding", () => {
    ...
    expect(state.lastTerminal!.options.macOptionClickForcesSelection).toBe(
      true,
    );
    expect(
      state.lastTerminal!._core._selectionService.enable,
    ).toHaveBeenCalled();
    expect(
      state.lastTerminal!._core.element.classList.remove,
    ).toHaveBeenCalledWith("enable-mouse-events");
    expect(
  ...
  it("attaches a real dropped file's saved path, and still lets the drop bubble to the window", async () => {
    const saveDroppedFile = vi
      .fn()
      .mockResolvedValue("/tmp/ao-dropped/notes.txt");
    ...
    fireEvent.drop(host, {
      dataTransfer: dropTransfer({ isDirectory: false }, [file]),
    });
    await waitFor(() =>
  ...
  it("lets a dropped folder bubble to the window instead of attaching it as a file", () => {
    ...
    fireEvent.drop(host, {
      dataTransfer: dropTransfer({ isDirectory: true }, [folder]),
    });
