...
const clientForMock = vi.hoisted(() => vi.fn(() => ({ GET: reviewGetMock })));
...
vi.mock("../hooks/useSessionInterfaceTransition", () => ({
  ...
  interfaceTransitionIsCancellable: (transition?: { phase?: string }) =>
    Boolean(
      transition &&
      ["requested", "preflighting", "draining"].includes(
        transition.phase ?? "",
      ),
    ),
...
vi.mock("./chat/SessionChatSurface", () => ({
  SessionChatSurface: ({
    ...
    >
      ...
      <button
        type="button"
        onClick={() => onConversationWorkChange?.({ ...chatSurfaceWorkState })}
      >
        report chat work
      </button>
    </div>
...
vi.mock("./CenterPane", () => ({
  CenterPane: ({
    ...
    onSelectReviewerTerminal?: (target: {
      handleId: string;
      harness: string;
    }) => void;
    ...
    <div>
      ...
      <div data-testid="reviewer-harness">
        {reviewerTerminal?.harness ?? ""}
      </div>
      {reviewerTerminal ? (
        <button
          type="button"
          onClick={() => onSelectReviewerTerminal?.(reviewerTerminal)}
        >
          select reviewer tab
        </button>
      ) : null}
      <div data-testid="shell-tabs">
        {shellTerminals.map((s) => s.title).join(",")}
      </div>
      {shellTerminals.map((s) => (
        <button
          key={s.handleId}
          type="button"
          onClick={() => onSelectShellTerminal?.(s.handleId)}
        >
          select {s.title}
        </button>
      ))}
      {shellTerminals.map((s) => (
        <button
          key={`close-${s.handleId}`}
          type="button"
          onClick={() => onCloseShellTerminal?.(s.handleId)}
        >
          close {s.title}
        </button>
      ))}
...
vi.mock("./SessionFileWorkspace", () => ({
  SessionFileWorkspace: ({ path }: { path: string }) => (
    <div data-testid="session-file-workspace">{path}</div>
  ),
}));
        ...
        {filesAvailable ? (
          <>
            ...
            <button
              type="button"
              onClick={() =>
                onOpenReviewFile?.({ path: "src/panel.tsx", line: 42 })
              }
            >
              view review file
            </button>
            <button
              type="button"
              onClick={() => onOpenReviewFile?.({ path: "notes.txt" })}
            >
...
vi.mock("../hooks/useShellTerminals", () => ({
  useShellTerminals: () => ({
    data: shellTerminalsState.data.map((shell) => ({
      ...shell,
      host: "local",
    })),
    isLoading: false,
...
function inspectorOpen(sessionId: string): boolean {
  return (
    useUiStore.getState().inspectorSessions[`local:${sessionId}`]?.isOpen ??
    true
  );
}
...
function browserUnseen(sessionId: string): boolean {
  return Boolean(
    useUiStore.getState().inspectorSessions[`local:${sessionId}`]
      ?.browserUnseen,
  );
}
...
describe("SessionView", () => {
  beforeEach(() => {
    ...
    for (const session of workspaces.flatMap(
    ...
    reviewGetMock.mockImplementation(async (path: string) => {
      ...
      return {
        data: { reviewerHandleId: "", reviews: [], runs: [] },
        error: undefined,
      };
    });
  });
  ...
  it("publishes which terminal the session pane is showing", () => {
    ...
    expect(
      useUiStore.getState().visibleTerminalKindBySession["local:sess-1"],
    ).toBe("worker");

    fireEvent.click(
      screen.getByRole("button", { name: "select sess-1-shell" }),
    );
    expect(
      useUiStore.getState().visibleTerminalKindBySession["local:sess-1"],
    ).toBe("shell");
  ...
  it("keeps a session-scoped shell reachable from a Chat session", () => {
    ...
    act(() =>
      useUiStore
        .getState()
        .setActiveShellTerminal({ host: "local", id: "chat-shell" }),
    );
    expect(screen.getByTestId("chat-surface")).toBeInTheDocument();
  ...
  it("shows only the session on screen in the tab strip", () => {
    ...
    expect(screen.getByTestId("session-tab")).not.toHaveTextContent(
      "do the other thing",
    );
    expect(
  ...
  it("shows a shell opened from chat and returns to the chat agent tab", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "open shell from chat" }),
    );
    expect(screen.getByText("chat surface")).toBeInTheDocument();
  ...
  it.each([
    ...
    "switches an idle %s directly with drain",
    (_label, sessionId, mode, targetMode, buttonName) => {
  ...
  it.each([
    ...
    "switches an idle Chat %s directly to Terminal UI with drain",
    (_label, sessionId) => {
      ...
      fireEvent.click(
        screen.getByRole("button", { name: "Switch to terminal UI" }),
      );
  ...
  it("keeps the policy dialog closed when an idle direct switch fails", async () => {
    ...
    interfaceTransitionMock.start.mockRejectedValueOnce(
  ...
  it("keeps the policy dialog open when an explicit busy Chat choice fails", async () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "Switch to Terminal UI?" }),
    ).toBeInTheDocument();
    await act(async () => {
      fireEvent.click(
        screen.getByRole("button", { name: /^Stop now and switch/ }),
      );
    });
  ...
  it.each([
    ...
    "opens the switch policy dialog for %s",
    (
      _label,
      sessionId,
      mode,
  ...
  it.each([
    ...
    "asks for policy before a busy Chat session switches for %s",
    (_label, sessionId, status, activityState) => {
      interfaceTransitionState.status = { supported: true, targetMode: "tui" };
      const session = workerSession(sessionId);
      session.mode = "chat";
      session.status = status;
  ...
  it.each([
    [
      "a busy controller",
      { controllerBusy: true, hasRunningTurn: false, queuedTurnCount: 0 },
    ],
    [
      "a running turn",
      { controllerBusy: false, hasRunningTurn: true, queuedTurnCount: 0 },
    ],
    [
      "accepted queued work",
      { controllerBusy: false, hasRunningTurn: false, queuedTurnCount: 1 },
    ],
  ] as const)(
    "asks for policy when Chat reports %s but the session projection is idle",
    (_label, work) => {
      interfaceTransitionState.status = { supported: true, targetMode: "tui" };
      const session = workerSession("sess-1");
      session.mode = "chat";
  ...
  it.each([
    ...
    "maps explicit busy Chat consent through the %s action",
    (buttonName, policy) => {
      interfaceTransitionState.status = { supported: true, targetMode: "tui" };
      const session = workerSession("sess-1");
      session.mode = "chat";
  ...
  it("disables Chat input while an accepted Chat-to-Terminal drain starts, runs, or settles", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /^Finish work, then switch/ }),
    );
    expect(interfaceTransitionMock.start).toHaveBeenCalledWith({
      targetMode: "tui",
      policy: "drain",
  ...
  it("does not resurrect an acknowledged recovery notice after the session view remounts", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Dismiss interface switch message" }),
    );
    await waitFor(() =>
      expect(interfaceTransitionMock.acknowledgeNotice).toHaveBeenCalledWith(
  ...
  it.each([
    ...
    "hides the interface switch button for %s sessions when Chat UI is unsupported",
    (_label, sessionId) => {
      interfaceTransitionState.status = {
        supported: false,
        targetMode: "chat",
      ...
      expect(
        screen.queryByRole("button", { name: "Switch to chat UI" }),
      ).not.toBeInTheDocument();
    },
  );
  ...
  it("walks backward through auxiliary terminals before returning to the permanent terminal", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "select second shell" }),
    );
    ...
    shellTerminalsState.data = shellTerminalsState.data.filter(
      (shell) => shell.handleId !== "sh-b",
    );
    view.rerender(<SessionView sessionRef={sessionRef("sess-1")} />);
  ...
  it("uses the stored reviewer harness for the reviewer tab icon when no latest run is current", async () => {
    ...
    reviewGetMock.mockResolvedValueOnce({
      data: {
        reviewerHandleId: "review-sess-1",
  ...
  it("returns to the session terminal when the reviewer handle is cleared", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "select reviewer tab" }),
    );
    expect(screen.getByTestId("terminal-target")).toHaveTextContent("reviewer");

    act(() => {
      view.client.setQueryData(["session-reviews", "local:sess-1"], {
        reviewerHandleId: "",
        reviews: [],
      });
    });
    ...
    expect(
      screen.queryByRole("button", { name: "select reviewer tab" }),
    ).not.toBeInTheDocument();
  });
  ...
  it.each(["tui", "chat"] as const)(
    ...
    async (mode) => {
      ...
      const view = render(<SessionView sessionRef={sessionRef("sess-1")} />);
      const reviewerButtonName =
        mode === "chat" ? "Reviewer" : "select reviewer tab";
      await screen.findByRole("button", { name: reviewerButtonName });
      fireEvent.click(screen.getByRole("button", { name: reviewerButtonName }));
      expect(screen.getByTestId("terminal-target")).toHaveTextContent(
        "reviewer",
      );
      ...
      expect(
        screen.queryByRole("button", { name: reviewerButtonName }),
      ).not.toBeInTheDocument();
  ...
  it("opens the inspector with the shared sidebar spring tokens", () => {
    ...
    expect(screen.getByTestId("panel-inspector")).toHaveAttribute(
      "data-state",
      "expanded",
    );
  ...
  it("opens the Summary inspector alongside the terminal by default", () => {
    ...
    expect(screen.getByTestId("inspector-resize-handle")).not.toHaveClass(
      "hidden",
    );
  ...
  it("keeps inspector chrome visible from the first render of the opening transition", () => {
    ...
    const openingRenders = inspectorVisibilityRenders.slice(
  ...
  it("keeps the inspector toggle and trailing notification pinned while the panel changes state", () => {
    ...
    const notification = within(actions).getByRole("button", {
      name: "Notifications",
    });
    const buttons = within(actions).getAllByRole("button");
    expect(buttons.indexOf(notification)).toBeGreaterThan(
    ...
    expect(screen.getByRole("button", { name: "Notifications" })).toBe(
      notification,
    );
    expect(screen.getByRole("button", { name: "Open inspector panel" })).toBe(
      toggle,
    );
  ...
  it("restores and clamps the persisted inspector width in pixels", () => {
    ...
    render(<SessionView sessionRef={sessionRef("sess-1")} />);
    expect(
      document.documentElement.style.getPropertyValue("--ao-inspector-w"),
    ).toBe("340px");
  });
    ...
    await waitFor(() => {
      ...
      expect(
        screen
          ...
          .style.getPropertyValue("--session-inspector-max-width"),
      ).toBe("min(68%, max(300px, calc(100% - 440px)))");
    });

    act(() => useUiStore.getState().setInspectorView("local:sess-1", "files"));
    await waitFor(() => {
      expect(screen.getByTestId("panel-group")).toHaveAttribute(
        "data-workspace-mode",
        "files",
  ...
  it("mounts the inspector in sync when navigating from an orchestrator session", () => {
    const { rerender } = render(
      <SessionView sessionRef={sessionRef("sess-orch")} />,
    );
  ...
  it("renders no inspector panel or handle for orchestrator sessions", () => {
    ...
    expect(screen.queryByTestId("panel-inspector")).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("inspector-resize-handle"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("inspector-collapsed-rail"),
    ).not.toBeInTheDocument();

    // The shortcut is inactive without an inspector.
    fireEvent.keyDown(window, { key: "B", metaKey: true, shiftKey: true });
    expect(
      useUiStore.getState().inspectorSessions["local:sess-orch"],
    ).toBeUndefined();
  });
  ...
  it("smoothly morphs the browser over the whole app window and back to its dock", async () => {
    ...
    try {
      ...
      expect(overlay).toHaveStyle({
        "--browser-popout-dock-left": "780px",
        "--browser-popout-dock-width": "500px",
      });
      expect(overlay).toHaveClass("browser-popout-overlay--mac-windowed");
      expect(
        screen.getByRole("button", { name: "browser center" }),
      ).toBeInTheDocument();
      expect(screen.getByText("terminal center")).toBeInTheDocument();
      await waitFor(() =>
      ...
      fireEvent.transitionEnd(
        overlay?.querySelector(".browser-popout-frame") as Element,
        {
          propertyName: "width",
        },
      );
      ...
      await waitFor(() =>
        expect(
          screen.queryByRole("button", { name: "browser center" }),
        ).not.toBeInTheDocument(),
      );
      expect(screen.getByText("terminal center")).toBeInTheDocument();
  ...
  it("does not reserve the traffic-light band during native macOS fullscreen", () => {
    ...
    expect(document.querySelector(".browser-popout-overlay")).not.toHaveClass(
  ...
  it("opens the files view in the inspector rail first", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "files center" }),
    ).not.toBeInTheDocument();
    expect(screen.getByText("terminal center")).toBeInTheDocument();
  });

  it("does not mount daemon file controls for a cloud session", () => {
    const cloudSession: WorkspaceSession = {
      ...workerSession("sess-1"),
      host: "cloud:org-1",
      id: "cloud-session-1",
      workspaceId: "cloud-project-1",
      cloud: { orgId: "org-1" },
    };
    workspaceQueryState.data = [
      {
    ...
    render(
      <SessionView
        sessionRef={{ host: "cloud:org-1", id: "cloud-session-1" }}
      />,
    );

    expect(
      screen.queryByRole("button", { name: "open files" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "view review file" }),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("opens docked files as center tabs while preserving the agent surface", () => {
    ...
    expect(screen.getByTestId("session-file-workspace")).toHaveTextContent(
      "src/App.tsx",
    );
    expect(screen.getByText("terminal center")).toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole("button", { name: "select agent tab" }));
    expect(
      screen.queryByTestId("session-file-workspace"),
    ).not.toBeInTheDocument();
    expect(screen.getByRole("tab", { name: "App.tsx" })).toHaveAttribute(
      "aria-selected",
      "false",
  ...
  it("opens a review file target in a center tab and keeps the Files inspector visible", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("tab", { name: "panel.tsx" })).toHaveAttribute(
        "aria-selected",
        "true",
      );
      expect(screen.getByTestId("session-file-workspace")).toHaveTextContent(
    ...
    expect(useUiStore.getState().inspectorSessions["local:sess-1"]?.view).toBe(
      "files",
    );
    expect(
  ...
  it("resolves a review basename against host-qualified workspace files", async () => {
    reviewGetMock.mockImplementation(async (path: string) => {
      if (path === "/api/v1/sessions/{sessionId}/workspace/files") {
        return {
          data: {
    ...
    expect(clientForMock).toHaveBeenCalledWith("local");
  });

  it("resolves a chat basename against host-qualified workspace files", async () => {
    workerSession("sess-1").mode = "chat";
    reviewGetMock.mockImplementation(async (path: string) => {
      if (path === "/api/v1/sessions/{sessionId}/workspace/files") {
        return {
  ...
  it("maximizes files over the whole app window and returns to the rail", () => {
    ...
    expect(
      screen.getByRole("button", { name: "files center" }),
    ).toBeInTheDocument();
  ...
  it("does not reserve the traffic-light band for maximized files during native macOS fullscreen", () => {
    ...
    expect(document.querySelector(".files-popout-overlay")).not.toHaveClass(
  ...
  it("keeps Summary selected when preview content arrives with the async workspace response", () => {
    ...
    const { rerender } = render(
      <SessionView sessionRef={sessionRef("sess-2")} />,
    );
