...
export function SessionInspector({
  ...
  const availableViewDefs = VIEW_DEFS.filter(
    (entry) =>
      (reviewsAvailable || entry.id !== "reviews") &&
      (filesAvailable || entry.id !== "files"),
  );
  const view: InspectorView = availableViewDefs.some(
    (entry) => entry.id === requestedView,
  )
    ? requestedView
    : "summary";
  useEffect(() => {
