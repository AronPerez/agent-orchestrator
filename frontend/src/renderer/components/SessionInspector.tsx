		...
		<SessionInspectorSummaryView
			...
			pullRequestCards={
				<div className="flex flex-col gap-1.5">
					{hasPRs ? (
						prSummaries.map((pr) => (
							<PRSummaryCard key={pr.url || pr.htmlUrl || pr.number} pr={pr} session={session} />
						))
					) : (
...
function PRSummaryCard({ pr, session }: { pr: SessionPRSummary; session: WorkspaceSession }) {
	const { t } = useTranslation();
	const queryClient = useQueryClient();
...
function ReviewsSection({
	...
	const saveReviewer = useMutation({
		...
		onSuccess: () => {
			void queryClient.invalidateQueries({ queryKey: ["session-reviews", sessionKey] });
			void queryClient.invalidateQueries({ queryKey: workspaceQueryKey });
		},
	});
