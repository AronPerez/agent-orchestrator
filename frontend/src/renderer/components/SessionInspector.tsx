...
function SummaryView({
  ...
  return (
    <SessionInspectorSummaryView
      ...
      completion={
        showCompletion ? <CompletionControls session={session} /> : undefined
      }
      pullRequestCards={
        hasPRs ? (
          prSummaries.map((pr) => (
...
function ResumeAgentControl({ session }: { session: WorkspaceSession }) {
  ...
  if (session.isTerminated === true || session.activity?.state !== "exited")
    return null;
...
function CompletionControls({ session }: { session: WorkspaceSession }) {
  ...
  const policy = useMutation({
    ...
    onMutate: async (terminateOnPrMerge) => {
      ...
      const previous =
        queryClient.getQueryData<HostSection[]>(queryKey);
  ...
  const confirmTermination = () => {
    const workspaces = flattenHostSections(
      queryClient.getQueryData<HostSection[]>(
        workspaceHostQueryKey(session.host),
      ),
    );
        ...
        <>
          <div className="flex items-center justify-between gap-3 py-1">
            <label
              ...
              htmlFor={`merge-policy-${session.id}`}
            >
...
function ActivityTimeline({
  ...
  for (const pr of prs.filter((pr) => pr.state === "draft")) {
    history.push({
      tone: "neutral",
      content: (
        <PRTimelineLink pr={pr} verb={appI18n.t("inspector.timeline.draft")} />
      ),
  ...
  for (const pr of prs.filter((pr) => pr.state !== "draft")) {
    history.push({
      tone: "neutral",
      content: (
        <PRTimelineLink pr={pr} verb={appI18n.t("inspector.timeline.opened")} />
      ),
  ...
  for (const pr of prs.filter((pr) => pr.state === "merged")) {
    history.push({
      tone: "good",
      content: (
        <PRTimelineLink pr={pr} verb={appI18n.t("inspector.timeline.merged")} />
      ),
      timestamp: prStateTime(pr),
    });
  }
...
function ReviewsSection({
  ...
  const [autoInjectReview, setAutoInjectReview] = useState(
...
function MergedReviewsSection({
  ...
  const groups: InspectorReviewGroup[] = rows.map(
    ([number, { ao, github }]) => {
      const aoRuns = ao
      ...
      const unresolved = (github?.review?.unresolvedBy ?? []).reduce(
        ...
        github: github
          ? {
              entries: entries.map((entry) => ({
                body: entry.body,
                id:
                  entry.reviewUrl || `${entry.reviewerId}:${entry.submittedAt}`,
                isBot: entry.isBot,
              ...
              unresolved,
              unresolvedBy: (github.review?.unresolvedBy ?? []).map(
                (reviewer) => ({
  ...
  const states: PRReviewState[] = sortedPRs(session).map((pr, index) => {
    ...
    const latestRun =
      pr.review === "approved" || pr.review === "changes_requested"
        ? {
            ...
            targetSha,
            verdict:
              pr.review === "approved" ? "approved" : "changes_requested",
          }
        : undefined;
    ...
    if (pr.number === MOCK_RUNNING_PR) {
      return {
        latestRun: run({
          status: "running",
          id: `demo-review-run-${pr.number}-live`,
        }),
        prNumber: pr.number,
        prUrl: pr.url,
        status: "running",
...
function ReviewPanel({
  ...
  const harness =
    latest?.harness || config?.reviewers?.[0]?.harness || "claude-code";
  ...
  const reviewRunning = openReviewStates.some(
    (reviewState) => reviewState.status === "running",
  );
  ...
  const runDisabled =
    ...
    openReviewStates.every(
      (reviewState) => reviewState.status === "ineligible",
    );
  const primaryReviewActionLabel = reviewRunning
    ...
    : runAction;
  const killDisabled =
    isKilling || isTriggering || isSwitchingReviewer || !hasReviewerSession;
...
function openReviewStatesFor(
  ...
  return reviewStates.filter((reviewState) =>
    openPRURLs.has(reviewState.prUrl),
  );
}
...
function reviewSessionRunAction(
  reviewStates: PRReviewState[],
  isTriggering: boolean,
): string {
  if (
  ...
  if (
    reviewStates.some(
      (reviewState) =>
        reviewState.status === "changes_requested" || reviewState.latestRun,
    )
  ) {
