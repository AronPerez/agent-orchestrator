...
const FileTreeScrollElement = forwardRef<
  HTMLDivElement,
  HTMLAttributes<HTMLDivElement>
>(function FileTreeScrollElement({ className, ...props }, ref) {
  return (
    <div ref={ref} className={cn("board-scrollbar", className)} {...props} />
  );
});
...
function withChildrenAt(
  ...
  return nodes.map((node) => {
    ...
    if (dir === node.path || dir.startsWith(`${node.path}/`)) {
      return {
        ...node,
        children: withChildrenAt(node.children ?? [], dir, children),
      };
    }
    return node;
  });
...
export function FileTree({
  ...
  const rootQuery = useQuery({
    ...sessionWorkspaceTreeQueryOptions(session, ""),
    enabled: !changedOnly,
  });
        ...
        entries.map(async (entry) => {
          ...
          const result = await queryClient.fetchQuery(
            sessionWorkspaceTreeQueryOptions(
              session,
              node.path,
              t("files.error.loadWorkspaceTree"),
            ),
          );
  ...
  const loadChildren = useCallback(
    async (dir: string) => {
      ...
      try {
        ...
        setLazyData((current) =>
          withChildrenAt(current, dir, result.entries.map(entryToNode)),
        );
      } catch {
    ...
    [queryClient, session.host, session.id, t],
  );
  ...
  const handleToggle = useCallback(
    (id: string) => {
      ...
      if (node?.isOpen && node.data.type === "dir")
        void loadChildren(node.data.path);
    },
  ...
  const isEmpty =
    data.length === 0 &&
    (changedOnly || (rootQuery.isFetched && !rootQuery.isError));
  ...
  return (
    ...
    >
      ...
      {rootQuery.isError && !changedOnly ? (
        <p className="p-3 text-xs text-error">
          {rootQuery.error.message || t("files.error.loadWorkspaceTree")}
        </p>
      ) : null}
      {isEmpty ? (
        <p className="p-3 text-xs text-muted-foreground">
          {t("files.explorer.empty")}
        </p>
      ) : null}
...
function FileTreeRowContainer<T>({
  node,
  attrs,
  innerRef,
  children,
    ...
    >
      {isDir ? (
        <ChevronRight
          aria-hidden="true"
          className={cn(
            "size-3 shrink-0 text-passive transition-transform",
      ...
      {isDir ? (
        <WorkspaceEntryIcon
          kind="dir"
          name={entry.name}
          testId={`folder-icon-${entry.name}`}
        />
      ) : (
        <WorkspaceEntryIcon
          kind="file"
          name={entry.name}
          testId={`file-icon-${entry.name}`}
        />
      )}
      ...
      {!isDir && entry.status && entry.status !== "unmodified" ? (
        <span
          className={cn(
            "shrink-0 font-mono text-caption font-medium",
