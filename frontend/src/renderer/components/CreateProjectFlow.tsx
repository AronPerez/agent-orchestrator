...
import {
  alreadyRegisteredProjectId,
  daemonErrorMessage,
} from "../lib/daemon-error";
...
export function CreateProjectFlow({
  ...
  const [cloneSelection, setCloneSelection] =
    useState<CloneRepositorySelection | null>(null);
  ...
  // Upstream's three-way source picker (clone / local folder / workspace) in
  // front of the fork's host-aware folder step. Cloning is local-only, so it
  // never reaches openFolderStep's remote branch.
  const selectSource = (source: ProjectSource) => {
    ...
    openFolderStep(
      source === "workspace" ? "workspace" : "single_repo",
  ...
  const createProject = async (selection: CreateProjectAgentSelection) => {
    ...
    try {
      ...
      if (
        !cloneSelection &&
        selectedKind === "single_repo" &&
        isRepositorySetupRecoveryCode(code)
      ) {
  ...
  return (
    <div className="flex w-full flex-col items-center gap-1.5">
      <Tabs
        value={offering}
        onValueChange={(value) =>
          onOfferingChange(value === "cloud" ? "cloud" : "local")
        }
      >
      ...
      <p
        className="text-caption leading-body text-secondary text-center"
        role="status"
      >
        {offering === "cloud"
          ? t("createProject.kindCloudHint")
          : t("createProject.kindLocalHint")}
      </p>
    </div>
  );
}
...
function CloudSignInPanel({
  ...
  return (
    <div className="flex w-full max-w-(--size-import-modal-max) flex-col items-center gap-4 rounded-welcome-panel border border-[var(--color-border-import-modal)] bg-[var(--color-bg-import-modal)] p-(--size-import-modal-padding) text-center shadow-[var(--shadow-import-modal)]">
      <Cloud
        className="size-6 text-[var(--color-text-import-title)]"
        aria-hidden="true"
      />
...
function CloudProjectCard({
  ...
  const urlError =
    submitted && !isHttpsRepositoryUrl(repositoryUrl)
      ? t("createProject.cloudInvalidUrl")
      : null;
  const nameError =
    submitted && displayName.trim() === ""
      ? t("createProject.cloudDisplayNameRequired")
      : null;
  const branchError =
    submitted && defaultBranch.trim() === ""
      ? t("createProject.cloudDefaultBranchRequired")
      : null;
  const orgFailure = orgError
  ...
  const title = (
    <h2 className="import-title text-balance">
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input
              ...
              autoComplete="off"
              aria-describedby={
                urlError ? "cloudRepositoryUrlError" : undefined
              }
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input
                ...
                autoComplete="off"
                aria-describedby={
                  nameError ? "cloudDisplayNameError" : undefined
                }
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input
                ...
                autoComplete="off"
                aria-describedby={
                  branchError ? "cloudDefaultBranchError" : undefined
                }
        ...
        <div className="flex items-center justify-end gap-3">
          {org === undefined && orgFailure === null ? (
            <p
              className="mr-auto text-pretty text-[12px] leading-5 text-[var(--color-text-import-muted)]"
              role="status"
            >
          ...
          <Button
            ...
            disabled={isCreating || org === undefined}
          >
            {isCreating
              ? t("createProject.creating")
              : t("createProject.cloudCreate")}
          </Button>
        </div>
      </form>
    </div>
  ...
  return (
    <>
      ...
      <ProjectSourcePickerView
        ...
        folderIcon={
          <FolderOpen className="size-[14px] shrink-0" aria-hidden="true" />
        }
...
function CreateProjectFolderDialog({
  ...
  const failedRepos =
    scan?.repos.filter(
      (repo) =>
        (repo.status === "error" || !repo.hasRemote) && !repo.needsGitInit,
    ) ?? [];
