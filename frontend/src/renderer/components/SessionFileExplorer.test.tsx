...
vi.mock("./FileTree", () => ({
  FileTree: ({
    ...
    <div>
      ...
      <button
        onClick={() => onSelectPath({ path: "src/App.tsx", type: "file" })}
        type="button"
      >
        select src/App.tsx
      </button>
...
vi.mock("./FileContentPane", () => ({
  FileContentPane: ({ path }: { path: string | null }) => (
    <div data-testid="content-pane">{path ?? "none"}</div>
  ),
}));
...
describe("SessionFileExplorer", () => {
  ...
  it("passes the filter input down to the tree and shows the selected file in the content pane", async () => {
    renderWithQuery(
      <SessionFileExplorer
        session={{ host: "local", id: "sess-explorer-1" }}
      />,
    );
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "select src/App.tsx" }),
    );
  ...
  it("returns to the tree when the back button is pressed, docked", async () => {
    renderWithQuery(
      <SessionFileExplorer
        session={{ host: "local", id: "sess-explorer-back" }}
      />,
    );
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Back to file tree" }),
    );
    expect(screen.queryByTestId("content-pane")).not.toBeInTheDocument();
  ...
  it("opens docked files in the coordinated center workspace and keeps the tree visible", async () => {
    ...
    renderWithQuery(
      <SessionFileExplorer
        ...
        session={{ host: "local", id: "sess-explorer-center" }}
      />,
    );
  ...
  it("keeps the tree and content side by side when maximized", async () => {
    ...
    const { container } = renderWithQuery(
      <SessionFileExplorer
        isMaximized
        session={{ host: "local", id: "sess-explorer-maximized" }}
      />,
    );
    ...
    expect(
      screen.queryByRole("button", { name: "Back to file tree" }),
  ...
  it("toggles the changed-only setting in the ui store and reflects it in the tree", async () => {
    ...
    renderWithQuery(
      <SessionFileExplorer session={{ host: "local", id: sessionId }} />,
    );
  ...
  it("toggles between unified and split diff layout", async () => {
    renderWithQuery(
      <SessionFileExplorer
        session={{ host: "local", id: "sess-explorer-3" }}
      />,
    );
    ...
    expect(
      screen.getByRole("button", { name: "Unified diff view" }),
  ...
  it("lets the caller toggle between rail and maximized layouts", async () => {
    ...
    renderWithQuery(
      <SessionFileExplorer
        ...
        session={{ host: "local", id: "sess-explorer-4" }}
      />,
    );

    await userEvent.click(
      screen.getByRole("button", { name: "Maximize files" }),
    );
