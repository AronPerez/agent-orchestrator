...
function diffLine(text: string) {
  return (_content: string, element: Element | null): boolean =>
    element != null &&
    /whitespace-pre/.test(element.className) &&
...
describe("ReviewDiffBody", () => {
  ...
  it("renders a real diff without git-header noise and with markers in the gutter", async () => {
    ...
    expect(
      screen.queryByText("diff --git a/src/App.tsx b/src/App.tsx"),
  ...
  it("highlights only the changed tokens within a replaced line", async () => {
    ...
    await screen.findByText(diffLine("const value = 1;"));
    expect(
      container.querySelector('[class*="bg-success/35"]')?.textContent,
    ).toBe("1");
    expect(container.querySelector('[class*="bg-error/35"]')?.textContent).toBe(
      "0",
    );
  });
  ...
  it("renders both sides of a changed image instead of the binary placeholder", async () => {
    ...
    const before = await screen.findByAltText(
      "Before version of docs/logo.png",
    );
    const after = await screen.findByAltText("After version of docs/logo.png");
    expect(before).toHaveAttribute(
      "src",
      expect.stringContaining(
        "/api/v1/sessions/sess-1/workspace/file/blob?path=docs%2Flogo.png&side=before",
      ),
    );
    expect(after).toHaveAttribute(
      "src",
      expect.stringContaining(
        "/api/v1/sessions/sess-1/workspace/file/blob?path=docs%2Flogo.png&side=after",
      ),
    );
    expect(
  ...
  it("sends a selected diff range to the session agent via the context menu", async () => {
    ...
    const notCanceled = fireEvent.contextMenu(addedRow, {
      clientX: 5,
      clientY: 5,
    });
    ...
    await userEvent.click(
      await screen.findByRole("menuitem", { name: "Explain" }),
    );
  ...
  describe("large diff virtualization", () => {
    ...
    it("opens a large diff without hanging, mounting far fewer DOM rows than the diff has", async () => {
      ...
      render(
        <div data-files-scroll-root="" style={{ overflow: "auto" }}>
          <ReviewDiffBody
            ...
            detail={baseDetail({
              path: "big.txt",
              diff: bigModifiedDiff(lineCount),
            })}
            detailLoadedAt={1}
            filePath="big.txt"
            onActiveSelectionChange={vi.fn()}
      ...
      await waitFor(() =>
        expect(
          screen.getByText(
            diffLine("new line 0 with some different content entirely"),
          ),
        ).toBeInTheDocument(),
      );
      ...
      expect(
        screen.queryByText(
          diffLine(
            `new line ${lineCount - 1} with some different content entirely`,
          ),
        ),
      ).not.toBeInTheDocument();
    });
