...
describe("global board first launch", () => {
	it("runs the lightweight requirements preflight while loading the board", async () => {
		...
		await waitFor(() => {
			expect(getMock.mock.calls.some((call) => requestedUrl(call) === "/api/v1/projects")).toBe(true);
			expect(getMock.mock.calls.some((call) => requestedUrl(call) === "/api/v1/sessions")).toBe(true);
		});
		expect(await screen.findByText("Add code to Agent Orchestrator")).toBeInTheDocument();
		expect(getMock.mock.calls.some((call) => requestedUrl(call) === "/api/v1/system/requirements")).toBe(true);
	});
	...
	it("shows an error instead of first-run welcome when the local host fails", async () => {
		getMock.mockResolvedValue({ data: undefined, error: new Error("local daemon dropped") });
		renderBoard(<SessionsBoard />);
