...
vi.mock("../../lib/spawn-orchestrator", () => ({
  isChatPreflightError: (error: unknown) =>
    ...
    (error as Error & { code?: string }).code === "CHAT_DRIVER_UNAVAILABLE",
...
vi.mock("../../components/TerminalPane", () => ({
  TerminalPane: (props: {
    ...
    onTerminalStateChange?: (state: "attached" | "exited") => void;
  }) => {
...
function requestedUrl(args: string[]): string {
  return args[args.length - 1]!;
}

function requestedHost(args: string[]): string {
  return args.length > 1 ? args[0]! : "local";
}
...
function respondWith(
  ...
  getMock.mockImplementation(async (...args: string[]) => {
    const host = requestedHost(args);
    const url = requestedUrl(args);
              ...
              : [
                  {
                    id: "gh",
                    label: "gh",
                    satisfied: githubCliSatisfied,
                    required: false,
                    detail: githubCliSatisfied ? "/usr/bin/gh" : "Not found",
                  },
                ]),
          ],
        },
...
describe("global board first launch", () => {
  it("runs the lightweight requirements preflight while loading the board", async () => {
    ...
    await waitFor(() => {
      expect(
        getMock.mock.calls.some(([url]) => url === "/api/v1/projects"),
      ).toBe(true);
      expect(
        getMock.mock.calls.some(([url]) => url === "/api/v1/sessions"),
      ).toBe(true);
    });
    expect(await screen.findByText("Add a project")).toBeInTheDocument();
    expect(
      getMock.mock.calls.some(([url]) => url === "/api/v1/system/requirements"),
    ).toBe(true);
  });
  ...
  it("shows the import chooser instead of empty columns when no projects exist", async () => {
    ...
    expect(await screen.findByText("Add a project")).toBeInTheDocument();
    expect(
      screen.getByText("Choose where your code is located"),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Clone from Git" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Import a workspace folder" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Import an existing project" }),
    ).toBeInTheDocument();
  ...
  it("automatically opens GitHub sign-in without requesting focus when gh is signed out", async () => {
    ...
    expect(
      await screen.findByText("Connect GitHub for pull requests"),
    ).toBeInTheDocument();
    ...
    expect(screen.getByTestId("terminal-pane")).toHaveAttribute(
      "data-focus-requested",
      "false",
    );
    act(() =>
      terminalPanePropsMock.mock.lastCall?.[0].onTerminalStateChange?.(
        "attached",
      ),
    );
    expect(
  ...
  it("adopts a daemon-owned login after renderer state is lost", async () => {
    ...
    getMock.mockImplementation(async (url: string) =>
      url === "/api/v1/shell-terminals"
        ? {
            data: {
              shellTerminals: [
                {
                  handleId: "surviving-login",
                  title: "Connect GitHub",
                  workingDir: "/tmp/auth",
                  createdAt: "2026-07-04T10:00:00Z",
                },
    ...
    await waitFor(() =>
      expect(terminalPanePropsMock).toHaveBeenCalledWith(
  ...
  it("does not spawn when daemon terminal reconciliation fails", async () => {
    ...
    ).toHaveTextContent("Terminal list unavailable");
    expect(postMock).not.toHaveBeenCalled();
  });
  ...
  it("respects a dismissed automatic login after the notice remounts", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByTestId("github-auth-terminal"),
      ).not.toBeInTheDocument(),
    );
    view.unmount();
    render(
  ...
  it.each(["Close", "Try again"])(
    "retains the login handle when %s cannot close its PTY",
    async (action) => {
      respondWith([], [], false);
      renderBoard(<SessionsBoard />);
      await screen.findByTestId("github-auth-terminal");
  ...
  it("offers recovery instead of treating an exited login terminal as active", async () => {
    ...
    expect(
      await screen.findByText("Sign-in stopped before GitHub was connected"),
    ).toBeInTheDocument();
  ...
  it("keeps sign-in available when GitHub CLI readiness is unknown", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Get GitHub CLI" }),
    ).not.toBeInTheDocument();
    expect(postMock).not.toHaveBeenCalledWith(
      "/api/v1/system/github-auth/terminal",
    );
  ...
  it("opens the native folder picker from the Project card", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "Import an existing project" }),
    );
  ...
  it("opens the native folder picker from the Workspace card", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "Import a workspace folder" }),
    );
    expect(chooseDirectoryMock).toHaveBeenCalledTimes(1);
    expect(chooseDirectoryMock).toHaveBeenCalledWith(
  ...
  it("keeps populated columns visible after the daemon reports a startup failure", async () => {
    ...
    lastShell = {
      daemonStatus: {
        state: "stopped",
        code: "exited",
      } as ShellContextValue["daemonStatus"],
      workspaceStartupState: "loading",
      cloneProject: cloneProjectMock,
      createProject: createProjectMock,
      initializeProjectRepository: initializeProjectRepositoryMock,
    ...
    expect(await screen.findByText("fix the bug")).toBeInTheDocument();
    expect(
      screen.queryByTestId("daemon-startup-loader"),
    ).not.toBeInTheDocument();
    expect(columnCount()).toBe(4);
  });
});
...
describe("project board with no sessions", () => {
  it("shows the task invitation instead of empty columns", async () => {
    ...
    expect(
      screen.getAllByRole("button", { name: "Spawn Orchestrator" }).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByRole("button", { name: "New task" }).length,
    ).toBeGreaterThan(0);
    expect(screen.queryByText("Add a project")).not.toBeInTheDocument();
    expect(columnCount()).toBe(0);
  });
  ...
  it("surfaces the daemon error when spawning the orchestrator fails", async () => {
    ...
    spawnOrchestratorMock.mockRejectedValue(
      new Error("branch is already checked out in another worktree"),
    );
    renderBoard(<SessionsBoard projectId="proj-1" />);

    await screen.findByText("No worker sessions yet");
    const [spawnButton] = screen.getAllByRole("button", {
  ...
  it("offers an explicit Terminal UI fallback when Chat preflight fails", async () => {
    ...
    const preflightError = Object.assign(
      new Error("Claude Code is unavailable"),
      {
    ...
    spawnOrchestratorMock
      ...
      .mockResolvedValueOnce("proj-1-orchestrator");
    renderBoard(<SessionsBoard projectId="proj-1" />);
  ...
  it("shows the project creation startup error after navigating to the project board", async () => {
    ...
    expect(
      screen.getByText(/branch is already checked out/),
    ).toBeInTheDocument();
  });
  ...
  it("clears the project creation startup error when retrying orchestrator spawn", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByText(/Project added, but orchestrator did not start/),
      ).not.toBeInTheDocument(),
    );
    expect(
      useUiStore.getState().orchestratorStartupErrors["proj-1"],
    ).toBeUndefined();
  });
  ...
  it("clears a stale spawn error when switching projects", async () => {
    ...
    await screen.findByText("No worker sessions yet");
    expect(
      screen.queryByText(/branch is already checked out/),
    ).not.toBeInTheDocument();
  });
  ...
  it("keeps the columns once the project has a session", async () => {
    ...
    expect(await screen.findByText("fix the bug")).toBeInTheDocument();
    expect(
      screen.queryByText("No worker sessions yet"),
    ).not.toBeInTheDocument();
    expect(columnCount()).toBe(4);
  });
});
...
describe.each([false, true])(
  "shared project board actions, in-panel header=%s",
  (inPanel) => {
    ...
    function updateWorkers(sessions: WorkspaceSummary["sessions"]) {
      act(() =>
        lastQueryClient!.setQueryData<WorkspaceSummary[]>(
          workspaceQueryKey,
          (workspaces) =>
    ...
    it("keeps quiet header copies only while the center invitation is visible", async () => {
      ...
      const headerTask = screen
        .getAllByRole("button", { name: "New task" })
        .find((button) => !empty.contains(button))!;
      const headerOrchestrator = screen
        .getAllByRole("button", { name: "Spawn Orchestrator" })
        .find((button) => !empty.contains(button))!;
      expect(headerTask).toHaveClass(
        "topbar-control--secondary",
        "topbar-control--labeled",
      );
      expect(headerOrchestrator).toHaveClass(
        "topbar-control--secondary",
        "topbar-control--labeled",
      );
      ...
      expect(
        within(empty).getByRole("button", { name: "Spawn Orchestrator" }),
      ).toHaveClass("topbar-control--primary");
      expect(
        within(empty).getByRole("button", { name: "New task" }),
      ).toHaveClass("topbar-control--accent");
      ...
      for (const session of [
        worker,
        { ...worker, status: "terminated" as const, isTerminated: true },
      ]) {
        updateWorkers([session]);
        await waitFor(() =>
          expect(
            screen.queryByTestId("project-board-empty"),
          ).not.toBeInTheDocument(),
        );
        expect(screen.getByRole("button", { name: "New task" })).toHaveClass(
          "topbar-control--accent",
        );
        expect(
    ...
    it("shares pending state and Terminal UI recovery after either copy starts a request", async () => {
      ...
      spawnOrchestratorMock.mockImplementationOnce(
        () =>
      ...
      await waitFor(() =>
        expect(spawnOrchestratorMock).toHaveBeenCalledTimes(1),
      );
      ...
      act(() =>
        rejectSpawn(
          Object.assign(new Error("Chat driver unavailable"), {
            code: "CHAT_DRIVER_UNAVAILABLE",
          }),
        ),
      );
      expect(
        await screen.findByText("Chat driver unavailable"),
      ).toBeInTheDocument();
      ...
      await waitFor(() =>
        expect(spawnOrchestratorMock).toHaveBeenCalledTimes(2),
      );
      expect(spawnOrchestratorMock).toHaveBeenLastCalledWith(
        "proj-1",
        "board",
        false,
        "tui",
      ...
      await waitFor(() =>
        expect(navigateMock).toHaveBeenCalledWith({
          ...
          params: { projectId: "proj-1", sessionId: "orch-retry" },
        }),
      );
    });

    it.each(["success", "failure"])(
      "keeps a late %s scoped to its original project",
      async (outcome) => {
        respondWith(
          [project, { ...project, id: "proj-2", name: "other-app" }],
          [],
        );
        ...
        await screen.findByTestId("project-board-empty");
        await userEvent.click(
          screen.getAllByRole("button", { name: "Spawn Orchestrator" })[0],
        );
        await waitFor(() =>
          expect(spawnOrchestratorMock).toHaveBeenCalledTimes(1),
        );
        view.rerender(
          <QueryClientProvider client={lastQueryClient!}>
            <ShellProvider value={lastShell!}>{board("proj-2")}</ShellProvider>
          </QueryClientProvider>,
        );
        await waitFor(() => {
          for (const button of screen.getAllByRole("button", {
        ...
        act(() =>
          outcome === "success"
            ? resolveSpawn("old-project-orchestrator")
            : rejectSpawn(new Error("Old project failed")),
        );
        await waitFor(() => expect(lastQueryClient!.isMutating()).toBe(0));
        expect(navigateMock).not.toHaveBeenCalled();
        expect(
          screen.queryByText("Old project failed"),
        ).not.toBeInTheDocument();
      },
    );

    it.each(["loading", "error", "missing"])(
      "does not demote header actions for a %s project",
      async (state) => {
        if (state === "loading")
          getMock.mockImplementation(() => new Promise(() => undefined));
        else if (state === "error")
          getMock.mockResolvedValue({ error: new Error("Offline") });
        else respondWith([], []);
        renderBoard(board());
        if (state === "missing")
          await waitFor(() =>
            expect(
              lastQueryClient!.getQueryState(workspaceQueryKey)?.status,
            ).toBe("success"),
          );
        expect(
          screen.queryByTestId("project-board-empty"),
