...
export type SessionInterfaceTransition =
  components["schemas"]["SessionInterfaceTransition"];
...
const startInterfaceTransitionMutationKey = [
  "start-session-interface-transition",
] as const;
const cancelInterfaceTransitionMutationKey = [
  "cancel-session-interface-transition",
] as const;
...
function useInterfaceTransitionMutations<TInput>(
...
>(
  mutations: InterfaceTransitionMutationState<TInput>[],
  ...
  for (const mutation of mutations) {
    if (!mutation.input || refKey(mutation.input.targetSession) !== sessionKey)
      continue;
...
function clearInterfaceTransitionMutationState(
  ...
  for (const mutation of mutationCache.findAll({ mutationKey })) {
    ...
    if (
      input &&
      refKey(input.targetSession) === sessionKey &&
...
function transitionClient(session: Ref) {
  if (isCloudHost(session.host)) {
    throw new Error("Interface switching is unavailable for cloud sessions.");
  }
  return clientFor(session.host);
}

export function interfaceTransitionIsActive(
  transition?: SessionInterfaceTransition,
): boolean {
  return Boolean(transition && activePhases.has(transition.phase));
}

export function interfaceTransitionIsCancellable(
  transition?: SessionInterfaceTransition,
): boolean {
...
export function useSessionInterfaceTransition(session: Ref | undefined) {
  ...
  const query = useQuery({
    ...
    enabled: Boolean(
      session && !isCloudHost(session.host) && isHostReady(session.host),
    ),
    queryFn: async () => {
      const { data, error } = await transitionClient(session!).GET(
  ...
  const start = useMutation({
    ...
    mutationFn: async ({
      ...
      const { data, error } = await transitionClient(targetSession).POST(
        "/api/v1/sessions/{sessionId}/interface-transition",
        {
    ...
    onSuccess: (response, variables) => {
      const targetSession = variables.targetSession;
      ...
      if (response?.transition) {
        queryClient.setQueryData<SessionInterfaceTransitionStatus>(
          sessionInterfaceTransitionQueryKey(targetSession),
          {
            supported: true,
      ...
      return queryClient
        .invalidateQueries({
          queryKey: sessionInterfaceTransitionQueryKey(targetSession),
        })
        .catch(() => undefined);
    },
  });
  ...
  const cancel = useMutation({
    ...
    mutationFn: async ({
      ...
      const { error } = await transitionClient(targetSession).DELETE(
        "/api/v1/sessions/{sessionId}/interface-transition",
        { params: { path: { sessionId: targetSession.id } } },
      );
      if (error) throw error;
  ...
  const acknowledgeNotice = useMutation({
    ...
    mutationFn: async ({
      ...
      const { data, error } = await transitionClient(targetSession).PUT(
  ...
  const transitionKey =
    sessionKey && transitionID
      ? JSON.stringify([sessionKey, transitionID])
      : "";
  ...
  return {
    ...
    acknowledgeNotice: (transitionId: string) => {
      if (!session) return Promise.reject(new Error("No session is selected."));
      return acknowledgeNotice.mutateAsync({
