...
type TabsListener = (
  state: import("../../main/browser-view-host").BrowserTabsState,
) => void;
type DevToolsListener = (
  state: import("../../main/browser-view-host").BrowserDevToolsState,
) => void;
type ActivityListener = (
  state: import("../../main/browser-view-host").BrowserAgentActivityState,
) => void;
...
function setupBridge() {
  ...
  const bridge = {
    ...
    navigate: vi.fn(async ({ viewId }: { viewId: string }) =>
      bridge.stateFor(viewId),
    ),
    ...
    selectTab: vi.fn(
      async ({ viewId, tabId }: { viewId: string; tabId: string }) => ({
        viewId,
        activeTabId: tabId,
        tabs: [
          {
...
describe("useBrowserView", () => {
  ...
  it("raises the shell above the live native view while a browser overlay is open", async () => {
    ...
    await waitFor(() =>
      expect(bridge.setOverlayOpen).toHaveBeenCalledWith(true),
    );
  });
  ...
  it("opens a browser overlay without waiting for a snapshot", async () => {
    ...
    act(() => result.current.slotRef(slot));
    await waitFor(() =>
      expect(result.current.navState.url).toBe("http://localhost:3000/"),
    );
  ...
  it("keeps the native page live while native composition raises a browser overlay", async () => {
    ...
    menu.remove();
    await waitFor(() =>
      expect(bridge.setOverlayOpen).toHaveBeenLastCalledWith(false),
    );
  });
  ...
  it("re-applies the preview on remount without a native browser, whose view state does not survive", async () => {
    ...
    try {
      ...
      await waitFor(() =>
        expect(first.result.current.navState.url).toBe(
          "http://localhost:5217/",
        ),
      );
      first.unmount();
      ...
      await waitFor(() =>
        expect(second.result.current.navState.url).toBe(
          "http://localhost:5217/",
        ),
      );
      second.unmount();
...
describe("normalizeWebPreviewURL", () => {
  it("adds a scheme (http for localhost, https otherwise) and rejects unframable targets", () => {
    expect(normalizeWebPreviewURL("localhost:5173")).toBe(
      "http://localhost:5173/",
    );
    expect(normalizeWebPreviewURL("127.0.0.1:8080/app")).toBe(
      "http://127.0.0.1:8080/app",
    );
    expect(normalizeWebPreviewURL("http://localhost:3000/")).toBe(
      "http://localhost:3000/",
    );
...
describe("useBrowserView (web fallback, no window.ao)", () => {
  ...
  it("auto-loads the ao preview URL into the iframe", async () => {
    ...
    await waitFor(() =>
      expect(result.current.iframeSrc).toBe("http://localhost:4173/"),
    );
  });
