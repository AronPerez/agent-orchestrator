...
const { deleteMock, getMock, postMock, putMock, clientForMock } = vi.hoisted(
  () => ({
    deleteMock: vi.fn(),
    getMock: vi.fn(),
    postMock: vi.fn(),
...
beforeEach(() => {
  ...
  postMock.mockReset();
  putMock.mockReset();
  clientForMock.mockReset().mockReturnValue({
    GET: getMock,
    POST: postMock,
    PUT: putMock,
    DELETE: deleteMock,
...
describe("host-qualified interface transitions", () => {
  it("keeps equal session ids on different hosts isolated", async () => {
    ...
    const first = renderHook(
      () => useSessionInterfaceTransition({ host: "node-a", id: "same" }),
      { wrapper: HookWrapper },
    );
    const second = renderHook(
      () => useSessionInterfaceTransition({ host: "node-b", id: "same" }),
      { wrapper: HookWrapper },
    );

    await act(async () => {
      await Promise.all([
        first.result.current.start({ targetMode: "tui", policy: "drain" }),
        second.result.current.start({ targetMode: "tui", policy: "drain" }),
      ]);
    });

    expect(clientForMock.mock.calls.map(([host]) => host)).toEqual(
...
describe("session-scoped interface transition mutations", () => {
  it("keeps a deferred start attached to its initiating session after navigation", async () => {
    ...
    const { result, rerender } = renderHook(
      ...
      {
        initialProps: { sessionId: localRef("session-a") },
        wrapper: HookWrapper,
      },
    );
    ...
    rerender({ sessionId: localRef("session-b") });
  ...
  it("keeps start pending until the accepted transition refetch is visible", async () => {
    ...
    const { result } = renderHook(
      () => useSessionInterfaceTransition(localRef("session-a")),
      {
        wrapper: HookWrapper,
      },
    );
  ...
  it("keeps each session busy while overlapping starts are pending", async () => {
    ...
    rerender({ sessionId: localRef("session-a") });
    expect(result.current.starting).toBe(true);
  ...
  it("keeps a deferred notice acknowledgement attached to its initiating session", async () => {
    ...
    const response = deferred<{
      data: {
        ok: boolean;
        sessionId: string;
        transition: typeof acknowledgedA;
      };
      error: undefined;
    }>();
    ...
    await waitFor(() =>
      expect(result.current.transition?.id).toBe("transition-a"),
    );
...
describe("interface switch readiness", () => {
  ...
  it.each(["NATIVE_SESSION_MISSING", "NATIVE_SESSION_UNVERIFIED"])(
    ...
    async (reasonCode) => {
      ...
      const { result } = renderHook(
        () => useSessionInterfaceTransition({ host: "local", id: "session-1" }),
        {
          wrapper,
        },
      );
  ...
  it("acknowledges the exact transition and replaces the cached notice with the durable response", async () => {
    ...
    await waitFor(() =>
      expect(result.current.transition?.id).toBe("transition-1"),
    );
    await act(async () => {
    ...
    await waitFor(() =>
      expect(result.current.transition?.noticeAcknowledgedAt).toBe(
