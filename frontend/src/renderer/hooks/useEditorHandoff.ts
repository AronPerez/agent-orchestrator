...
export const editorHandoffQueryKey = (host: HostId, sessionId: string) =>
  ["editor-handoff", host, sessionId] as const;
...
function shouldAwaitWorkspace({
  sessionCreatedAt,
  sessionTerminated,
}: EditorHandoffReadiness): boolean {
...
function waitForWorkspaceRetry(signal: AbortSignal): Promise<void> {
  return new Promise((resolve, reject) => {
    if (signal.aborted) {
      reject(
        signal.reason ??
          new DOMException("Workspace readiness check cancelled", "AbortError"),
      );
      return;
    }
    const onAbort = () => {
...
export function useEditorHandoffState(
  host: HostId,
  sessionId: string,
  readiness: EditorHandoffReadiness = {},
) {
  ...
  return useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      for (let retries = 0; ; retries += 1) {
        const state = await aoBridge.editorHandoff.getState({
          host,
          sessionId,
        });
        if (
          ...
          !awaitWorkspace ||
          retries >= WORKSPACE_READINESS_MAX_RETRIES
        ) {
          return state;
        }
...
export function useOpenSessionTarget() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      projectId,
      targetId,
    }: OpenSessionTargetMutationInput) => {
      ...
      try {
        return await aoBridge.editorHandoff.open({
          host,
          sessionId,
          ...(targetId ? { targetId } : {}),
        });
    ...
    onSuccess: (result, input) => {
      if (result.kind === "editor" && isEditorId(result.id)) {
        queryClient.setQueryData<EditorHandoffState>(
          editorHandoffQueryKey(input.host, input.sessionId),
          (state) =>
            state
              ? {
                  ...state,
                  preferredEditorId:
                    result.id as typeof state.preferredEditorId,
                }
              : state,
        );
      }
    ...
    onError: (_error, input) => {
      ...
      void queryClient.invalidateQueries({
        queryKey: editorHandoffQueryKey(input.host, input.sessionId),
      });
      void captureRendererEvent("ao.renderer.open_in_editor_failed", {
        project_id: input.projectId,
      });
    },
  });
