...
import { isCloudHost, refKey, type Ref } from "../lib/hosts";
...
export function conversationQueryKey(session?: Ref) {
  return session
    ? ([...conversationQueryRoot, refKey(session)] as const)
    : conversationQueryRoot;
}

export function conversationModelsQueryKey(session?: Ref) {
  return session
    ? (["conversation-models", refKey(session)] as const)
    : (["conversation-models"] as const);
}
...
type ConversationDispatchTrackingBySession = Record<
  string,
  ConversationDispatchTracking
>;
...
function claimConversationDispatch(
  ...
  if (current[targetSessionKey]) return false;
...
function cloudOrgId(session: Ref): string | undefined {
  return isCloudHost(session.host)
    ? decodeURIComponent(session.host.slice("cloud:".length))
    : undefined;
}

function daemonConversationClient(session: Ref) {
  if (isCloudHost(session.host)) {
    throw new Error(
      "This conversation action is unavailable for cloud sessions.",
    );
  }
  return clientFor(session.host);
}
...
export function useConversation(
  ...
  const query = useInfiniteQuery({
    ...
    enabled: Boolean(session && !isCloudHost(session.host)),
...
export function useConversationCommands(session: Ref | undefined) {
  ...
  const trackedDispatch = sessionKey
    ? trackedDispatches[sessionKey]
    : undefined;
  const invalidateSession = useCallback(
    async (targetSession: Ref) => {
      await queryClient.invalidateQueries({
        queryKey: conversationQueryKey(targetSession),
      });
    },
    [queryClient],
  );
  ...
  const send = useMutation({
    onMutate: (variables: ConversationSendMutationInput) => {
      queryClient.setQueryData<ConversationDispatchTrackingBySession>(
        conversationDispatchTrackingQueryKey,
        (current = {}) => {
          const targetKey = refKey(variables.targetSession);
          const tracked = current[targetKey];
          if (tracked && tracked.requestId !== variables.clientMessageId)
            return current;
          return {
            ...current,
            [targetKey]: {
    ...
    mutationFn: async ({
      ...
      const orgId = cloudOrgId(targetSession);
      if (orgId) {
        if (input.attachments?.length || input.resources?.length) {
          throw new Error("Cloud chat does not support attachments yet.");
        }
      ...
      const { data, error } = await daemonConversationClient(
        targetSession,
    ...
    onSuccess: async (data, variables) => {
      ...
      await invalidateSession(variables.targetSession).catch(() => {});
    },
    onError: (_error, variables) => {
      releaseConversationDispatch(
        queryClient,
        refKey(variables.targetSession),
        variables.clientMessageId,
      );
    },
  });
  ...
  const interrupt = useMutation({
    mutationFn: async ({
      targetSession,
      turnId,
    }: ConversationSessionMutationInput) => {
      const orgId = cloudOrgId(targetSession);
      if (orgId) {
        if (!turnId) throw new Error("A turn is required to stop cloud work.");
        await cloudClient.cancelTurn(orgId, targetSession.id, turnId);
        return;
      }
    ...
    onError: (_error, variables) => invalidateSession(variables.targetSession),
  });

  const resume = useMutation({
    mutationFn: async () => {
      const { data, error, response } = await daemonConversationClient(
        session!,
      ).POST("/api/v1/sessions/{sessionId}/resume-agent", {
  ...
  const compact = useMutation({
    mutationFn: async () => {
      const { data, error } = await daemonConversationClient(session!).POST(
  ...
  const chooseSettings = useMutation({
    mutationFn: async (settings: TurnSettings) => {
      const { error } = await daemonConversationClient(session!).PATCH(
  ...
  const retryTurn = useMutation({
    mutationFn: async ({
      targetSession,
      sourceTurnId,
    }: ConversationRetryMutationInput) => {
      const { data, error } = await daemonConversationClient(
        targetSession,
      ).POST("/api/v1/sessions/{sessionId}/conversation/turns/{turnId}/retry", {
        params: {
  ...
  const editMessage = useMutation({
    mutationFn: async ({
      ...
      const { data, error } = await daemonConversationClient(
        targetSession,
      ).POST("/api/v1/sessions/{sessionId}/conversation/turns/{turnId}/edit", {
        params: {
          path: { sessionId: targetSession.id, turnId: sourceTurnId },
        },
    ...
    (turnId: string) => {
      ...
      queryClient.setQueryData<ConversationDispatchTrackingBySession>(
        ...
        (current = {}) => {
          ...
          if (tracked?.state !== "accepted" || tracked.turnId !== turnId)
            return current;
          const next = { ...current };
  ...
  const sendTargetsCurrentSession =
    Boolean(sessionKey && send.variables) &&
    refKey(send.variables!.targetSession) === sessionKey;
  const interruptTargetsCurrentSession =
    Boolean(sessionKey && interrupt.variables) &&
    refKey(interrupt.variables!.targetSession) === sessionKey;
  const retryTargetsCurrentSession =
    Boolean(sessionKey && retryTurn.variables) &&
    refKey(retryTurn.variables!.targetSession) === sessionKey;
  const editTargetsCurrentSession =
    Boolean(sessionKey && editMessage.variables) &&
    refKey(editMessage.variables!.targetSession) === sessionKey;
  ...
  return {
    send: (input: string | ConversationSendInput) => {
      ...
      if (
        ...
        return Promise.reject(
          new Error(
            "Conversation work is already being sent for this session.",
          ),
        );
      }
    ...
    pendingAcceptedTurnId:
      trackedDispatch?.state === "accepted"
        ? trackedDispatch.turnId
        : undefined,
    ...
    interrupt: (turnId?: string) => {
      if (!session) return;
      interrupt.mutate({ targetSession: session, turnId });
    },
...
export function useConversationConfigOptions(
  ...
  const mutation = useMutation({
    ...
    mutationFn: async ({
      ...
      await queryClient.cancelQueries({ queryKey });
      const { data, error } = await daemonConversationClient(session!).PATCH(
