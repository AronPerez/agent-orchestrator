...
export type UiState = {
  ...
  setProjectProvisioning: (project: Ref, provisioning: boolean) => void;
  ...
  showGlobalToast: (
    title: string,
    body?: string,
    style?: GlobalToast["tone"] | GlobalToast["placement"],
  ) => void;
...
export const useUiStore = create<UiState>((set, get) => ({
  ...
  openProjectSettings: (project) =>
    set({ settingsModal: { scope: "project", project } }),
  ...
  setProjectProvisioning: (project, provisioning) =>
    set((state) => {
      const provisioningProjectIds = new Set(state.provisioningProjectIds);
      const projectKey = refKey(project);
      if (provisioning) provisioningProjectIds.add(projectKey);
      else provisioningProjectIds.delete(projectKey);
  ...
  showGlobalToast: (title, body, style) =>
    set((state) => {
      ...
      const placement =
        style === "top-center" || style === "bottom-right"
          ? style
          : "bottom-right";
      const toast = { title, body, tone, placement, nonce };
      return {
        globalToast: toast,
        globalToasts: [...state.globalToasts, toast],
        globalToastSequence: nonce,
      };
    }),
  dismissGlobalToast: (nonce) =>
    set((state) => ({
      ...
      globalToast:
        state.globalToast?.nonce === nonce ? null : state.globalToast,
    })),
  clearGlobalToast: () =>
    set({ globalToast: null, globalToasts: [], globalToastSequence: 0 }),
  requestNewTask: (project) => {
    if (get().provisioningProjectIds.has(refKey(project))) {
      get().showGlobalToast(
