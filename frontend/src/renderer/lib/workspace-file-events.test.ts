...
describe("subscribeWorkspaceFileChanges", () => {
	...
	it("keeps one retry pending when another connect trigger arrives", () => {
		...
		const unsubscribe = subscribeWorkspaceFileChanges({ host: "local", id: "sess-retry" }, fakeQueryClient());
	...
	it("reports degraded after three completed connection failures", () => {
		...
		const unsubscribe = subscribeWorkspaceFileChanges(session, fakeQueryClient());
	...
	it("degrades after repeated native reconnect failures and recovers on open", () => {
		...
		for (let failure = 0; failure < 3; failure += 1) {
			source.onerror?.();
			expect(getWorkspaceFileConnectionState(session)).toBe(failure < 2 ? "connecting" : "degraded");
		}
