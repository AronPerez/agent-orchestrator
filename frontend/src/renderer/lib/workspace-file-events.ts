...
export function getWorkspaceFileConnectionState(
  session: Ref,
): WorkspaceFileConnectionState {
  return connectionStates.get(refKey(session)) ?? "connecting";
}

export function subscribeWorkspaceFileConnectionState(
  session: Ref,
  listener: () => void,
): () => void {
  ...
  return () => {
    ...
    if (!streams.has(sessionKey) && !connectionStateListeners.has(sessionKey))
      connectionStates.delete(sessionKey);
  };
}

function setWorkspaceFileConnectionState(
  sessionKey: string,
  next: WorkspaceFileConnectionState,
): void {
...
export function subscribeWorkspaceFileChanges(
  ...
  return () => {
    ...
    if (!connectionStateListeners.has(sessionKey))
      connectionStates.delete(sessionKey);
  };
}

function createWorkspaceStream(
  session: Ref,
  queryClient: QueryClient,
): WorkspaceStream {
  const sessionKey = refKey(session);
  ...
  const invalidate = () => {
    ...
    stream.debounce = setTimeout(() => {
      void queryClient.invalidateQueries({
        queryKey: ["session-workspace-files", sessionKey],
      });
      void queryClient.invalidateQueries({
  ...
  const handleTerminalFailure = (generation: number) => {
    ...
    stream.source = undefined;
    stream.failures += 1;
    setWorkspaceFileConnectionState(
      sessionKey,
      stream.failures >= 3 ? "degraded" : "connecting",
    );
