...
import {
  editorHandoffQueryKey,
  editorHandoffQueryRoot,
} from "../hooks/useEditorHandoff";
      ...
      const refreshWorkspaces = (host?: HostId, event?: Event) => {
        ...
        if (event && "data" in event) {
          try {
            ...
            if (
              ...
              pendingInterfaceTransitionSessions.add(
                refKey({ host: host ?? LOCAL_HOST, id: decoded.sessionId }),
              );
            }
            if (
              ...
              pendingEditorHandoffSessions.add(
                refKey({ host: host ?? LOCAL_HOST, id: decoded.sessionId }),
              );
            }
            if (
              ...
              pendingConversationSessions.add(
                refKey({ host: host ?? LOCAL_HOST, id: decoded.sessionId }),
              );
        ...
        debounce = setTimeout(() => {
          ...
          if (allEditorHandoffsInvalidationPending) {
            void queryClient.invalidateQueries({
              queryKey: editorHandoffQueryRoot,
            });
            allEditorHandoffsInvalidationPending = false;
            pendingEditorHandoffSessions.clear();
          } else {
            for (const sessionKey of pendingEditorHandoffSessions) {
              const session = parseRefKey(sessionKey);
              void queryClient.invalidateQueries({
                queryKey: editorHandoffQueryKey(session.host, session.id),
              });
            }
            pendingEditorHandoffSessions.clear();
          }
          ...
          for (const sessionKey of pendingInterfaceTransitionSessions) {
            void queryClient.invalidateQueries({
              queryKey: ["session-interface-transition", sessionKey],
            });
          }
