...
} from "./main/window-composition";
import { AgentBrowserRuntime } from "./main/agent-browser-runtime";
import {
  sameBrowserRuntimeIdentity,
  type BrowserRuntimeIdentity,
} from "./main/browser-runtime-identity";
import {
  connectSupervisor,
  type SupervisorLinkHandle,
...
function appendDaemonOutput(text: string): void {
  ...
  const nextStatus = refreshSlowDaemonStartupDetails(
    daemonStatus,
    daemonOutput,
  );
...
function runFilePath(): string | null {
  ...
  if (isDev)
    return path.join(os.homedir(), ".ao", DEV_STATE_SUBDIR, "running.json");
...
async function resolveSessionWorkspaceForDesktop(
  host: string,
  sessionId: string,
): Promise<string> {
  let target: string;
...
const editorHandoff = createEditorHandoff({
  ...
  remoteHost: async (host: string) => {
    ...
    const view = registry.views().find((candidate) => candidate.url === host);
    const entry = (await readRemotes(remotesFilePath()).catch(() => [])).find(
      (candidate) => candidate.url === host,
    );
    return {
      label: view?.label ?? entry?.label ?? host,
      ...(entry?.sshDestination
        ? { sshDestination: entry.sshDestination }
        : {}),
    };
  },
  readPreference: async () =>
    (await readEditorSettings(editorStateDir())).preferredEditorId,
...
async function readDaemonProbe(
  port: number,
  ...
  const timer = setTimeout(() => controller.abort(), DAEMON_PROBE_TIMEOUT_MS);
  try {
    const response = await net.fetch(`http://127.0.0.1:${port}/${endpoint}`, {
...
function daemonIdentityError(
  launch: DaemonLaunchSpec,
  probe: DaemonProbe,
  appBuildIdentity: string,
): string | null {
  if (launch.source === "dev") {
    const cwdMatches = probe.workingDirectory
      ? samePath(probe.workingDirectory, launch.cwd)
      : false;
    const startupCwdMatches = probe.startupWorkingDirectory
      ? samePath(probe.startupWorkingDirectory, launch.cwd)
      : false;
    const executableMatches = probe.executablePath
      ? pathInside(probe.executablePath, launch.cwd)
      : false;
    if (
  ...
  if (launch.source === "bundled") {
    ...
    return bundledDaemonIdentityError(
      probe,
      launch.command,
...
function establishBrowserRuntimeLink(): void {
  ...
  if (!address) {
    console.warn(
      "AO: browser runtime link skipped; daemon did not publish an address",
    );
    return;
  }
  ...
  if (
    browserRuntimeLink &&
    browserRuntimeLinkIdentity &&
    sameBrowserRuntimeIdentity(browserRuntimeLinkIdentity, identity)
  ) {
    return;
  }
...
async function inspectExistingDaemon(launch: DaemonLaunchSpec): Promise<{
  status: DaemonStatus;
  ...
  const status = await resolveDaemonFromRunFile({
    ...
    identityError: (probe) =>
      daemonIdentityError(launch, probe, APP_BUILD_IDENTITY),
  });
...
async function gracefullyReplaceDaemonForBrowser(
  ...
  const response = await fetch(`http://127.0.0.1:${status.port}/shutdown`, {
    method: "POST",
  });
  if (!response.ok)
    throw new Error(`daemon shutdown returned HTTP ${response.status}`);
...
async function refreshDaemonStatus(): Promise<DaemonStatus> {
  ...
  if (existing) {
    if (
      browserDaemonOwnershipDecision(appRunId, existing).action === "replace"
    ) {
      return startDaemon();
    }
...
function resolvedDaemonPort(): number {
  return isDev && !process.env.AO_PORT
    ? DEV_DAEMON_PORT
    : expectedDaemonPort(process.env);
}
...
async function startDaemonInner(startEpoch: number): Promise<DaemonStatus> {
  ...
  if (!launch) {
    setDaemonStatus({
      state: "stopped",
      message:
        "AO_DAEMON_COMMAND is not configured; renderer uses loopback REST when available.",
      code: "not_configured",
    });
    return daemonStatus;
  }
  ...
  if (runFilePath_) {
    try {
      runFilePid =
        parseRunFile(await readFile(runFilePath_, "utf8"))?.pid ?? null;
    } catch {
  ...
  if (keep) {
    ...
    try {
      ...
      console.warn(
        `AO: keep-daemon log redirect failed; daemon will run with stdio disabled: ${logPath}`,
      );
    ...
    runFileTimer = setInterval(() => {
      readFile(handshakePath, "utf8")
        .then((contents) => {
          ...
          if (
            info &&
            info.startedAtMs >= spawnedAtMs - RUN_FILE_FRESHNESS_SKEW_MS
          ) {
...
async function restartDaemon(): Promise<DaemonStatus> {
  ...
  const exited = new Promise<boolean>((resolve) => {
    ...
    const timer = setTimeout(
      () => finish(false),
      DAEMON_RESTART_STOP_TIMEOUT_MS,
    );
  ...
  if (!(await exited)) {
    ...
    setDaemonStatus({
      state: "error",
      message:
        "AO daemon is still stopping. It will restart automatically when shutdown completes.",
      details: daemonOutput.trim() || undefined,
      code: "not_ready",
    });
    return daemonStatus;
...
ipcMain.handle("editorHandoff:getState", (event, input) => {
  if (event.sender !== getShellWebContents())
    throw new Error("Untrusted editor handoff request.");
  const shaped =
    input && typeof input === "object"
      ? (input as { host?: unknown; sessionId?: unknown })
      : {};
...
ipcMain.handle("editorHandoff:open", (event, input) => {
  ...
  return editorHandoff.open({
    host: typeof shaped.host === "string" ? shaped.host : "local",
    sessionId: typeof shaped.sessionId === "string" ? shaped.sessionId : "",
    ...(typeof shaped.targetId === "string"
      ? { targetId: shaped.targetId as OpenTargetId }
      : {}),
...
ipcMain.handle(
  "theme:set",
  (_event, preference: "light" | "dark" | "system") => {
    if (
      preference === "light" ||
      preference === "dark" ||
      preference === "system"
...
ipcMain.on(SET_TERMINAL_FOCUSED_CHANNEL, (event, focused: unknown) => {
  if (event.sender !== getShellWebContents() || typeof focused !== "boolean")
    return;
...
ipcMain.handle("menu:action", (_event, action: string) => {
  ...
  const wc =
    (focused && focused !== shell
      ? focused
      : browserViewHost?.getLastFocusedPanelContents()) ?? shell;
  if (!wc) return;
  switch (action) {
    case "edit.undo":
      return wc.undo();
...
ipcMain.handle("app:checkAncestorRepo", async (_event, path: string) => {
  await ensureShellEnv();
  return ancestorRepositorySetupWarning(path, {
    env: daemonEnv(),
    homeDir: os.homedir(),
  });
});
...
ipcMain.handle(
  ...
  async (_event, input: { name: string; bytes: Uint8Array }) => {
    ...
    const base =
      path.basename(input.name || "").replace(/[^\w.-]+/g, "_") || "dropped";
...
ipcMain.handle(
  "appState:setMigration",
  async (_event, migration: MigrationState) => {
    const runFile = runFilePath();
    if (!runFile) return;
...
ipcMain.handle(
  "updateSettings:set",
  async (_event, settings: UpdateSettings) => {
    const runFile = runFilePath();
    if (!runFile) return;
...
ipcMain.handle(
  "uiSettings:set",
  async (_event, settings: Partial<UiSettings>): Promise<UiSettings> => {
    const runFile = runFilePath();
...
ipcMain.handle(
  ...
  async (
    ...
    keybindingOverrides = await writeKeybindingOverrides(
...
ipcMain.handle("keybindings:setRecording", (event, active: unknown): void => {
  if (event.sender !== getShellWebContents() || typeof active !== "boolean")
    return;
...
ipcMain.on(TRAY_SET_ATTENTION_STATE_CHANNEL, (event, state) =>
...
function notifyRenderersOfCloudSession(
  account: import("./shared/cloud-account").CloudAccount | null,
): void {
...
async function writeAppStateOnLaunch(): Promise<void> {
  ...
  if (!runFile) {
    throw new Error(
      "cannot resolve ~/.ao run-file path; skipping app-state marker",
    );
  }
  const stateDir = path.dirname(runFile);
...
app.whenReady().then(async () => {
  ...
  if (keybindingRunFile) {
    keybindingOverrides = await readKeybindingOverrides(
      path.dirname(keybindingRunFile),
    );
  }
...
process.on("exit", () => {
  if (
    daemonProcess &&
    !supervisorLink?.connected &&
    !keepDaemonAlive(process.env)
  ) {
    killDaemon(daemonProcess);
  }
});
