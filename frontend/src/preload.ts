...
ipcRenderer.on(
  TELEMETRY_POLICY_CHANGED_CHANNEL,
  (_event, view: TelemetryPolicyView) => {
...
ipcRenderer.on(
  ...
  (_event, request: unknown) => {
    ...
    void Promise.allSettled(
      [...rendererQueuePurgeListeners].map((listener) =>
...
function isRendererQueuePurgeRequest(
  value: unknown,
): value is RendererTelemetryQueuePurgeRequest {
...
const api = {
  app: {
    ...
    chooseDirectory: (
      input?: string | { title?: string; defaultPath?: string },
    ) =>
      ipcRenderer.invoke("app:chooseDirectory", input) as Promise<
        string | null
      >,
    checkGitRepository: (remoteUrl: string) =>
      ipcRenderer.invoke(
        "app:checkGitRepository",
        remoteUrl,
      ) as Promise<boolean>,
    ...
    scanImportFolder: (input: { path: string; mode: ImportFolderMode }) =>
      ipcRenderer.invoke(
        ...
        input,
      ) as Promise<ImportFolderScan>,
    checkAncestorRepo: (path: string) =>
      ipcRenderer.invoke("app:checkAncestorRepo", path) as Promise<
        string | undefined
      >,
    getRepositoryBranch: (path: string) =>
      ipcRenderer.invoke("app:getRepositoryBranch", path) as Promise<
        string | undefined
      >,
    getGitHubLogin: (repoPath?: string) =>
      ipcRenderer.invoke("app:getGitHubLogin", repoPath) as Promise<string>,
    getCachedGitHubOwners: () =>
      ipcRenderer.invoke("app:getCachedGitHubOwners") as Promise<
        Array<{ login: string; avatarUrl: string }>
      >,
    refreshGitHubOwners: () =>
      ipcRenderer.invoke("app:refreshGitHubOwners") as Promise<
        Array<{ login: string; avatarUrl: string }>
      >,
    checkGitHubRepositoryAvailability: (input: {
      owner: string;
      name: string;
    }) =>
      ipcRenderer.invoke(
    ...
    onOpenFolderPath: (listener: (path: string) => void) => {
      ...
      return () => {
        if (activeOpenFolderPathListener === listener)
          activeOpenFolderPathListener = null;
      };
    },
  ...
  terminal: {
    ...
    onFontSizeShortcut: (listener: (delta: -1 | 1) => void) => {
      const wrapped = (_event: Electron.IpcRendererEvent, delta: -1 | 1) =>
        listener(delta);
  ...
  menu: {
    action: (action: string) =>
      ipcRenderer.invoke("menu:action", action) as Promise<void>,
  ...
  editorHandoff: {
    getState: (input: EditorHandoffStateInput) =>
      ipcRenderer.invoke(
        "editorHandoff:getState",
        input,
      ) as Promise<EditorHandoffState>,
    open: (input: OpenSessionTargetInput) =>
      ipcRenderer.invoke(
        "editorHandoff:open",
        input,
      ) as Promise<OpenSessionTargetResult>,
  },
  telemetry: {
    getBootstrap: async () => {
      const bootstrap = (await ipcRenderer.invoke(
        "telemetry:getBootstrap",
      )) as TelemetryBootstrap | null;
      if (bootstrap && currentTelemetryPolicy)
        currentTelemetryPolicy = {
          ...currentTelemetryPolicy,
          eventsEnabled: bootstrap.eventsEnabled,
          consentGeneration: bootstrap.consentGeneration,
        };
      return bootstrap;
    },
    getPolicy: async () => {
      const view = (await ipcRenderer.invoke(
        "telemetry:getPolicy",
      )) as TelemetryPolicyView;
      currentTelemetryPolicy = view;
      for (const listener of telemetryPolicyListeners) listener(view);
      return view;
    },
    setEventsEnabled: (eventsEnabled: boolean) =>
      ipcRenderer.invoke("telemetry:setEventsEnabled", {
        eventsEnabled,
        expectedGeneration: currentTelemetryPolicy?.consentGeneration ?? "",
      }) as Promise<TelemetryPolicyView>,
    onPolicy: (listener: (view: TelemetryPolicyView) => void) => {
      telemetryPolicyListeners.add(listener);
      if (currentTelemetryPolicy) listener(currentTelemetryPolicy);
      return () => telemetryPolicyListeners.delete(listener);
    },
    onClearQueues: (listener: () => void | Promise<void>) => {
      rendererQueuePurgeListeners.add(listener);
      return () => rendererQueuePurgeListeners.delete(listener);
    },
    capture: (input: RendererTelemetryCaptureInput) => {
      if (!currentTelemetryPolicy) return Promise.resolve(false);
      return ipcRenderer.invoke("telemetry:capture", {
        ...input,
        consentGeneration: currentTelemetryPolicy.consentGeneration,
      }) as Promise<boolean>;
    },
    signalAgentSwitchVisibility: (signal: AgentSwitchVisibilitySignalBody) => {
      if (!currentTelemetryPolicy) return false;
      ipcRenderer.send(AGENT_SWITCH_VISIBILITY_IPC_CHANNEL, {
        consentGeneration: currentTelemetryPolicy.consentGeneration,
        signal,
      });
      return true;
  ...
  browser: {
    ...
    setOverlayOpen: (open: boolean) =>
      ipcRenderer.send("browser:overlay", open),
    ...
    historySuggestions: (input: { viewId: string; query: string }) =>
      ipcRenderer.invoke("browser:history:suggest", input) as Promise<
        BrowserHistorySuggestion[]
      >,
    clear: (viewId: string) =>
      ipcRenderer.invoke("browser:clear", viewId) as Promise<BrowserNavState>,
    goBack: (viewId: string) =>
      ipcRenderer.invoke("browser:goBack", viewId) as Promise<BrowserNavState>,
    ...
    reload: (viewId: string) =>
      ipcRenderer.invoke("browser:reload", viewId) as Promise<BrowserNavState>,
    stop: (viewId: string) =>
      ipcRenderer.invoke("browser:stop", viewId) as Promise<BrowserNavState>,
    captureScreenshot: (viewId: string) =>
      ipcRenderer.invoke("browser:captureScreenshot", viewId) as Promise<void>,
    downloads: {
      list: () =>
        ipcRenderer.invoke(
          "browser:downloads:list",
        ) as Promise<BrowserDownloadsState>,
    ...
    getTabs: (viewId: string) =>
      ipcRenderer.invoke(
        "browser:getTabs",
        viewId,
      ) as Promise<BrowserTabsState>,
    ...
    getProfile: (viewId: string) =>
      ipcRenderer.invoke(
        "browser:profile:get",
        viewId,
      ) as Promise<BrowserProfileViewState>,
    ...
    notifyPanelUsed: (viewId: string) =>
      ipcRenderer.send("browser:panelUsed", viewId),
    notifyPanelBlur: (viewId: string) =>
      ipcRenderer.send("browser:panelBlur", viewId),
    onFocusLocation: (listener: (viewId: string) => void) => {
      const wrapped = (_event: Electron.IpcRendererEvent, viewId: string) =>
        listener(viewId);
    ...
    devtools: (input: BrowserDevToolsInput) =>
      ipcRenderer.invoke(
        "browser:devtools",
        input,
      ) as Promise<BrowserDevToolsState>,
    ...
    onAgentActivity: (listener: (state: BrowserAgentActivityState) => void) => {
      const wrapped = (
        _event: Electron.IpcRendererEvent,
        state: BrowserAgentActivityState,
      ) => listener(state);
    ...
    onAnnotationSubmit: (
      listener: (payload: BrowserAnnotationSubmitPayload) => void,
    ) => {
      const wrapped = (
        _event: Electron.IpcRendererEvent,
    ...
    onAnnotationCancel: (
      listener: (payload: BrowserAnnotationCancelPayload) => void,
    ) => {
      const wrapped = (
        _event: Electron.IpcRendererEvent,
        payload: BrowserAnnotationCancelPayload,
      ) => listener(payload);
  ...
  browserProfiles: {
    list: () =>
      ...
      ) as Promise<BrowserProfileListState>,
    create: (name: string) =>
      ipcRenderer.invoke("browserProfiles:create", {
        name,
      }) as Promise<BrowserProfile>,
    ...
    clear: (id: string) =>
      ipcRenderer.invoke("browserProfiles:clear", { id }) as Promise<void>,
    delete: (id: string) =>
      ipcRenderer.invoke("browserProfiles:delete", { id }) as Promise<void>,
    discoverImportSources: () =>
      ipcRenderer.invoke(
        "browserProfiles:import:discover",
      ) as Promise<BrowserImportDiscovery>,
    import: (input: BrowserImportRequest) =>
      ipcRenderer.invoke(
        "browserProfiles:import:start",
        input,
      ) as Promise<BrowserImportResult>,
  ...
  notifications: {
    ...
    setBadge: (count: number) =>
      ipcRenderer.invoke("notifications:setBadge", count) as Promise<void>,
    devBounce: () =>
      ipcRenderer.invoke("notifications:devBounce") as Promise<void>,
  ...
  tray: {
    setAttentionState: (state: TrayAttentionState) =>
      ipcRenderer.send(TRAY_SET_ATTENTION_STATE_CHANNEL, state),
    onOpenSession: (listener: (target: TrayOpenSessionTarget) => void) => {
      const wrapped = (
        _event: Electron.IpcRendererEvent,
        target: TrayOpenSessionTarget,
      ) => listener(target);
  ...
  appState: {
    getMigration: () =>
      ipcRenderer.invoke("appState:getMigration") as Promise<MigrationState>,
    setMigration: (migration: MigrationState) =>
      ipcRenderer.invoke("appState:setMigration", migration) as Promise<void>,
  },
  updateSettings: {
    get: () =>
      ipcRenderer.invoke("updateSettings:get") as Promise<UpdateSettings>,
    set: (settings: UpdateSettings) =>
      ipcRenderer.invoke("updateSettings:set", settings) as Promise<void>,
  },
  uiSettings: {
    get: () => ipcRenderer.invoke("uiSettings:get") as Promise<UiSettings>,
    set: (settings: Partial<UiSettings>) =>
      ipcRenderer.invoke("uiSettings:set", settings) as Promise<UiSettings>,
  },
  keybindings: {
    get: () =>
      ipcRenderer.invoke("keybindings:get") as Promise<KeybindingOverrides>,
    set: (overrides: KeybindingOverrides) =>
      ipcRenderer.invoke(
        "keybindings:set",
        overrides,
      ) as Promise<KeybindingOverrides>,
    setRecording: (active: boolean) =>
      ipcRenderer.invoke("keybindings:setRecording", active) as Promise<void>,
  },
  updates: {
    getStatus: () =>
      ipcRenderer.invoke("updates:getStatus") as Promise<UpdateStatus>,
    check: (options?: UpdateCheckOptions) =>
      ipcRenderer.invoke("updates:check", options) as Promise<void>,
    returnHome: (requestId?: string) =>
      ipcRenderer.invoke("updates:returnHome", requestId) as Promise<void>,
    download: (requestId?: string) =>
      ipcRenderer.invoke("updates:download", requestId) as Promise<void>,
    install: (confirmedVersion?: string) =>
      ipcRenderer.invoke(
        "updates:install",
        confirmedVersion,
  ...
  featureBuilds: {
    list: () =>
      ipcRenderer.invoke("featureBuilds:list") as Promise<FeatureBuild[]>,
    getActive: () =>
  ...
  remotes: {
    ...
    remove: (url: string) =>
      ipcRenderer.invoke("remotes:remove", url) as Promise<void>,
    probe: (url: string) =>
      ipcRenderer.invoke("remotes:probe", url) as Promise<RemoteHealth>,
    ...
    disconnect: (url: string) =>
      ipcRenderer.invoke("remotes:disconnect", url) as Promise<void>,
    connected: () =>
      ipcRenderer.invoke("remotes:connected") as Promise<ConnectedHostView[]>,
  },
  cloud: {
    getSession: () =>
      ipcRenderer.invoke("cloud:getSession") as Promise<CloudAccount | null>,
  ...
  cloudCp: {
    request: (init: CloudCpProxyRequestInit) =>
      ipcRenderer.invoke(
        "cloudCp:request",
        init,
      ) as Promise<CloudCpProxyResponse>,
    openStream: (init: CloudCpProxyRequestInit) =>
      ipcRenderer.invoke("cloudCp:openStream", init) as Promise<{
        streamId: string;
      }>,
    ...
    onStreamEvent: (
      streamId: string,
      listener: (event: CloudCpStreamEvent) => void,
    ) => {
      const channel = `cloudCp:stream:${streamId}`;
      const wrapped = (
        _event: Electron.IpcRendererEvent,
        event: CloudCpStreamEvent,
      ) => listener(event);
