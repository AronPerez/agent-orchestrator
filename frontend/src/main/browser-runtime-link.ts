...
export type BrowserRuntimeDial = () => Promise<{
  socket: net.Socket;
  head?: Buffer;
}>;
...
export function connectBrowserRuntime(
  ...
  const sendCancelledResultOnTarget = (
    command: BrowserRuntimeCommand,
    target: net.Socket,
  ) => {
    ...
    const frame = `${JSON.stringify({
      ...
      ok: false,
      error: {
        code: "BROWSER_COMMAND_CANCELED",
        message: "Browser runtime link closed",
      },
    })}\n`;
    try {
      target.write(frame);
  ...
  const send = async (
    ...
    if (socket !== target || connectionEpoch !== epoch || target.destroyed)
      return;
    const frame = `${JSON.stringify(message)}\n`;
    if (Buffer.byteLength(frame, "utf8") > MAX_RESULT_BYTES) {
      throw Object.assign(
        new Error(`Browser result exceeds ${MAX_RESULT_BYTES} bytes`),
        {
  ...
  const respond = async (
    ...
    try {
      ...
      controller.signal.throwIfAborted();
      await send(
        { type: "result", requestId: command.requestId, ok: true, result },
        target,
        epoch,
      );
    } catch (error) {
  ...
  const consumeLine = (line: string, target: net.Socket, epoch: number) => {
    ...
    try {
      message = JSON.parse(line) as
        BrowserRuntimeCommand | BrowserRuntimeCancel;
    } catch {
      return;
    }
    ...
    const next = previous.then(() =>
      respond(command, target, epoch, controller),
    );
    commandChains.set(command.sessionId, next);
    void next.finally(() => {
      if (commandChains.get(command.sessionId) === next)
        commandChains.delete(command.sessionId);
    });
  };
  ...
  function attach(
    ...
    const onReady = () => {
      ...
      void send(
        { type: "hello", version: PROTOCOL_VERSION, token: options.token },
        next,
        epoch,
      ).catch((error) => {
    ...
    next.on("error", (error) =>
      log(`browser-runtime-link: error: ${error.message}`),
    );
    let connectionTornDown = false;
    const tearDownConnection = () => {
      if (connectionTornDown || socket !== next || connectionEpoch !== epoch)
        return;
  ...
  function connect() {
    ...
    if (!options.dial) {
      if (address === null)
        throw new Error("connectBrowserRuntime requires an address or a dial");
      const next =
