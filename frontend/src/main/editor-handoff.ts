...
const EDITOR_CANDIDATES: EditorCandidate[] = [
  {
    ...
    macApps: ["Cursor"],
    winInstallDirs: [
      [LOCAL_APPDATA_PROGRAMS, "Cursor", "resources", "app", "bin"],
      [PROGRAM_FILES, "Cursor", "bin"],
    ],
  },
  {
    id: "vscode",
    ...
    winInstallDirs: [
      [LOCAL_APPDATA_PROGRAMS, "Microsoft VS Code", "bin"],
  ...
  { id: "neovim", name: "Neovim", commands: ["nvim"], requiresTerminal: true },
  {
    ...
    macApps: ["Windsurf"],
    winInstallDirs: [
      [LOCAL_APPDATA_PROGRAMS, "Windsurf", "resources", "app", "bin"],
      [PROGRAM_FILES, "Windsurf", "bin"],
    ],
  },
  {
    id: "zed",
    ...
    commands: ["zed"],
    macApps: ["Zed"],
    winInstallDirs: [[LOCAL_APPDATA_PROGRAMS, "Zed", "bin"]],
  },
  {
    id: "trae",
    name: "Trae",
    ...
    macApps: ["Trae"],
    winInstallDirs: [
      [LOCAL_APPDATA_PROGRAMS, "Trae", "resources", "app", "bin"],
      [PROGRAM_FILES, "Trae", "bin"],
    ],
  },
  {
    id: "kiro",
    ...
    commands: ["kiro"],
    macApps: ["Kiro"],
    winInstallDirs: [[LOCAL_APPDATA_PROGRAMS, "Kiro", "bin"]],
  },
  {
    id: "positron",
    name: "Positron",
    ...
    macApps: ["Positron"],
    winInstallDirs: [
      [LOCAL_APPDATA_PROGRAMS, "Positron", "resources", "app", "bin"],
    ],
  },
  {
    id: "vscodium",
    ...
    macApps: ["VSCodium"],
    winInstallDirs: [
      [LOCAL_APPDATA_PROGRAMS, "VSCodium", "resources", "app", "bin"],
      [PROGRAM_FILES, "VSCodium", "bin"],
    ],
  },
  {
    id: "vscode-insiders",
    ...
    winInstallDirs: [
      [LOCAL_APPDATA_PROGRAMS, "Microsoft VS Code Insiders", "bin"],
      [PROGRAM_FILES, "Microsoft VS Code Insiders", "bin"],
    ],
  },
  {
    id: "sublime",
    ...
    winInstallDirs: [
      [PROGRAM_FILES, "Sublime Text"],
      [PROGRAM_FILES, "Sublime Text 3"],
      [LOCAL_APPDATA_PROGRAMS, "Sublime Text"],
    ],
  },
  {
    id: "intellij",
    ...
    winInstallDirs: [
      [PROGRAM_FILES, "JetBrains", "IntelliJ IDEA", "bin"],
      [LOCAL_APPDATA_PROGRAMS, "JetBrains", "IntelliJ IDEA", "bin"],
    ],
  },
  {
    id: "webstorm",
    ...
    macApps: ["WebStorm"],
    winInstallDirs: [
      [PROGRAM_FILES, "JetBrains", "WebStorm", "bin"],
      [LOCAL_APPDATA_PROGRAMS, "JetBrains", "WebStorm", "bin"],
    ],
  },
  {
    id: "pycharm",
    ...
    winInstallDirs: [
      [PROGRAM_FILES, "JetBrains", "PyCharm", "bin"],
      [LOCAL_APPDATA_PROGRAMS, "JetBrains", "PyCharm", "bin"],
    ],
  },
  {
    id: "goland",
    ...
    macApps: ["GoLand"],
    winInstallDirs: [
      [PROGRAM_FILES, "JetBrains", "GoLand", "bin"],
      [LOCAL_APPDATA_PROGRAMS, "JetBrains", "GoLand", "bin"],
    ],
  },
  {
    id: "phpstorm",
    ...
    macApps: ["PhpStorm"],
    winInstallDirs: [
      [PROGRAM_FILES, "JetBrains", "PhpStorm", "bin"],
      [LOCAL_APPDATA_PROGRAMS, "JetBrains", "PhpStorm", "bin"],
    ],
  },
  {
    id: "rubymine",
    ...
    macApps: ["RubyMine"],
    winInstallDirs: [
      [PROGRAM_FILES, "JetBrains", "RubyMine", "bin"],
      [LOCAL_APPDATA_PROGRAMS, "JetBrains", "RubyMine", "bin"],
    ],
  },
  {
    id: "clion",
    ...
    macApps: ["CLion"],
    winInstallDirs: [
      [PROGRAM_FILES, "JetBrains", "CLion", "bin"],
      [LOCAL_APPDATA_PROGRAMS, "JetBrains", "CLion", "bin"],
    ],
  },
  {
    id: "rider",
    ...
    macApps: ["Rider"],
    winInstallDirs: [
      [PROGRAM_FILES, "JetBrains", "Rider", "bin"],
      [LOCAL_APPDATA_PROGRAMS, "JetBrains", "Rider", "bin"],
    ],
  },
  {
    id: "android-studio",
    ...
    winInstallDirs: [[PROGRAM_FILES, "Android", "Android Studio", "bin"]],
  },
  {
    id: "fleet",
    name: "Fleet",
    commands: ["fleet"],
    macApps: ["Fleet"],
    winInstallDirs: [[PROGRAM_FILES, "JetBrains", "Fleet", "bin"]],
  },
];

const WIN_INSTALL_ROOTS: Record<
  string,
...
function defaultIsExecutable(
  ...
  try {
    ...
    return (
      stat.isFile() &&
      (platform === "win32" || (stat.mode & constants.X_OK) !== 0)
    );
  } catch {
    return false;
  }
...
function executableNames(
  ...
  if (platform !== "win32" || pathAPI(platform).extname(command))
    return [command];
...
function commandSearchDirs(
  ...
  const fromPath = (env.PATH || "")
    ...
    .filter(Boolean);
  if (platform === "darwin")
    return [...fromPath, "/opt/homebrew/bin", "/usr/local/bin", "/usr/bin"];
...
function resolveTerminalEditor(
  ...
  if (deps.platform === "win32") {
    return {
      ...
      argsForWorkspace: () => {
        ...
        const commandLine = [editorCommand, "."]
          .map(windowsCommandArg)
          .join(" ");
...
function resolveEditor(
  ...
  for (const command of candidate.commands) {
    ...
    if (resolved) {
      if (candidate.requiresTerminal)
        return resolveTerminalEditor(resolved, deps, isExecutable);
      return {
        command: resolved,
...
const REMOTE_CAPABLE_EDITORS = new Set<EditorId>([
  "vscode",
  "vscode-insiders",
  "vscodium",
  "cursor",
...
function supportsRemoteOpen(
  id: OpenTargetId,
  command: ResolvedCommand,
): boolean {
  return (
    REMOTE_CAPABLE_EDITORS.has(id as EditorId) && command.remoteCapable === true
  );
}
...
export function createEditorHandoff(deps: EditorHandoffDeps): EditorHandoff {
  const isExecutable =
    deps.isExecutable ??
    ((candidatePath) => defaultIsExecutable(candidatePath, deps.platform));
  ...
  return {
    async getState({ host, sessionId }) {
      const { editors, targets } = resolveAll();
      ...
      if (remote) {
        const remoteTargets = editors
          .filter(({ target, command }) =>
            supportsRemoteOpen(target.id, command),
          )
          .map(({ target }) => target);
        const remoteState = {
          hostLabel: remote.label,
          sshConfigured: Boolean(remote.sshDestination),
        };
    ...
    async open(input) {
      ...
      if (
        input.targetId &&
        input.targetId !== "file-manager" &&
        input.targetId !== "terminal" &&
        !isEditorId(input.targetId)
      ) {
      ...
      if (remote) {
        if (!remote.sshDestination) {
          throw new Error(
            `Add an SSH destination for ${remote.label} to open its workspaces here.`,
          );
        }
        const capable = editors.find(
          ({ target: candidate, command }) =>
            candidate.id === targetId &&
            supportsRemoteOpen(candidate.id, command),
        );
        if (!capable)
          throw new Error(
            `That open target is not available on ${remote.label}.`,
          );
        const workspacePath = await deps.resolveWorkspace(
          input.host,
          sessionId,
        );
        ...
        try {
          ...
          await deps.launch(
            capable.command.command,
            ["--folder-uri", folderUri],
            deps.homeDir,
          );
        } catch (error) {
          deps.logError?.(
            `failed to open remote session target ${capable.target.id}`,
            error,
          );
          throw new Error(
            `Could not open ${capable.target.name}. Check that it is installed and try again.`,
          );
        }
      ...
      try {
        if (target.kind === "file_manager") {
          ...
          const resolved =
            target.kind === "terminal"
              ? terminal?.command
              : editors.find(({ target: editor }) => editor.id === target.id)
                  ?.command;
          if (!resolved) throw new Error("target command was not resolved");
          await deps.launch(
            resolved.command,
            resolved.argsForWorkspace(workspacePath),
            workspacePath,
          );
        }
      } catch (error) {
        ...
        throw new Error(
          `Could not open ${target.name}. Check that it is installed and try again.`,
        );
      }
      if (target.kind === "editor")
        await deps.writePreference(target.id as EditorId);
      return target;
    },
  };
}
