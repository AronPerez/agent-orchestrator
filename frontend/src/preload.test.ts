...
const openFolderPathBufferListener =
  electronMocks.listeners.get("app:openFolderPath");
const telemetryPolicyBroadcastListener = electronMocks.listeners.get(
  "telemetry:policyChanged",
);
const rendererQueuePurgeRequestListener = electronMocks.listeners.get(
  "telemetry:clearRendererQueues",
);
...
describe("preload repository branch bridge", () => {
  it("invokes the main-process branch probe over IPC", async () => {
    ...
    await expect(
      exposedBridge().app.getRepositoryBranch("/repo/project"),
    ).resolves.toBe("main");

    expect(electronMocks.invoke).toHaveBeenCalledWith(
      "app:getRepositoryBranch",
      "/repo/project",
    );
  });
});
...
describe("preload telemetry generation bridge", () => {
  it("tags captures with the latest broadcast generation without a renderer reload", async () => {
    telemetryPolicyBroadcastListener?.(
      {},
      {
        eventsEnabled: false,
        consentGeneration: "generation-off",
    ...
    await exposedBridge().telemetry.capture({
      kind: "message",
      message: "first",
    });
    telemetryPolicyBroadcastListener?.(
      ...
      {
        eventsEnabled: true,
        consentGeneration: "generation-on",
        updatedAt: "2026-08-28T10:15:31.000Z",
        acknowledged: true,
        state: "applied",
        environmentVeto: false,
        durabilitySupported: true,
  ...
  it("attaches the latest policy generation to visibility signals without accepting renderer generation", () => {
    telemetryPolicyBroadcastListener?.(
      ...
      {
        eventsEnabled: true,
        consentGeneration: "generation-a",
        updatedAt: "2026-08-28T10:15:30.000Z",
        acknowledged: true,
        state: "applied",
        environmentVeto: false,
    ...
    expect(
      exposedBridge().telemetry.signalAgentSwitchVisibility({
        kind: "focus",
        value: true,
      }),
    ).toBe(true);
    telemetryPolicyBroadcastListener?.(
      ...
      {
        eventsEnabled: true,
        consentGeneration: "generation-b",
        updatedAt: "2026-08-28T10:15:31.000Z",
        acknowledged: true,
        state: "applied",
        environmentVeto: false,
    ...
    expect(electronMocks.send).toHaveBeenNthCalledWith(
      ...
      {
        consentGeneration: "generation-b",
        signal: { kind: "online", value: false },
      },
    );
  });
  ...
  it("acknowledges renderer queue purge only after every registered cleanup succeeds", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(electronMocks.send).toHaveBeenCalledWith(
        "telemetry:rendererQueuesCleared",
        { requestId: "purge-1", ok: true },
      );
    });
    disposeFirst();
    disposeSecond();
  ...
  it("reports renderer queue purge failure instead of over-acknowledging opt-out", async () => {
    const dispose = exposedBridge().telemetry.onClearQueues(
      vi.fn().mockRejectedValue(new Error("purge failed")),
    );
...
describe("preload new-session shortcut bridge", () => {
  it("delivers the IPC event and removes the exact wrapped listener", () => {
    ...
    dispose();
    expect(electronMocks.off).toHaveBeenCalledWith(
      NEW_SESSION_SHORTCUT_CHANNEL,
      wrapped,
    );
  });
...
describe("preload keyboard-shortcuts help bridge", () => {
  it("delivers the IPC event and removes the exact wrapped listener", () => {
    ...
    const wrapped = electronMocks.listeners.get(
      KEYBOARD_SHORTCUTS_HELP_CHANNEL,
    );
    expect(wrapped).toBeTypeOf("function");
...
describe("preload application shortcut bridges", () => {
  ...
  it("reports whether the active Chat draft has an unsafe unload boundary", () => {
    const copy = {
      ...
      detail: "Copiez le brouillon",
      stay: "Rester",
      leave: "Quitter",
    };
    exposedBridge().app.setChatDraftRisk(
      ["persistence-failed", "pending-attachments"],
      copy,
    );
  ...
  it.each([
    [
      NEW_SHELL_TERMINAL_SHORTCUT_CHANNEL,
      (listener: () => void) =>
        exposedBridge().app.onNewShellTerminalShortcut(listener),
    ],
    [
      CLOSE_SHELL_TERMINAL_SHORTCUT_CHANNEL,
      (listener: () => void) =>
        exposedBridge().app.onCloseShellTerminalShortcut(listener),
    ],
    [
      OPEN_SETTINGS_SHORTCUT_CHANNEL,
      (listener: () => void) =>
        exposedBridge().app.onOpenSettingsShortcut(listener),
    ],
    [
      PREVIOUS_SESSION_SHORTCUT_CHANNEL,
      (listener: () => void) =>
        exposedBridge().app.onPreviousSessionShortcut(listener),
    ],
    [
      NEXT_SESSION_SHORTCUT_CHANNEL,
      (listener: () => void) =>
        exposedBridge().app.onNextSessionShortcut(listener),
    ],
    [
      PREVIOUS_TAB_SHORTCUT_CHANNEL,
      (listener: () => void) =>
        exposedBridge().app.onPreviousTabShortcut(listener),
    ],
    [
      NEXT_TAB_SHORTCUT_CHANNEL,
      (listener: () => void) => exposedBridge().app.onNextTabShortcut(listener),
    ],
    [
      FOCUS_TERMINAL_SHORTCUT_CHANNEL,
      (listener: () => void) =>
        exposedBridge().app.onFocusTerminalShortcut(listener),
    ],
...
describe("preload uiSettings bridge", () => {
  it("invokes get and set over IPC", async () => {
    ...
    await expect(exposedBridge().uiSettings.get()).resolves.toEqual({
      locale: "en",
    });
    await expect(
      exposedBridge().uiSettings.set({ locale: "zh-CN" }),
    ).resolves.toEqual({ locale: "zh-CN" });

    expect(electronMocks.invoke).toHaveBeenNthCalledWith(1, "uiSettings:get");
    expect(electronMocks.invoke).toHaveBeenNthCalledWith(2, "uiSettings:set", {
      locale: "zh-CN",
    });
  });
});
...
describe("preload browser profile bridge", () => {
  it("routes profile state, native menu, and CRUD calls over IPC", async () => {
    ...
    await bridge.browser.historySuggestions({
      viewId: "1:worker-1",
      query: "git",
    });
    ...
    expect(electronMocks.invoke).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({ viewId: "1:worker-1" }),
    );
    expect(electronMocks.invoke).toHaveBeenNthCalledWith(
      3,
      "browser:profile:select",
      expect.objectContaining({ viewId: "1:worker-1", profileId: null }),
    );
    expect(electronMocks.invoke).toHaveBeenNthCalledWith(
      4,
      "browser:history:suggest",
      { viewId: "1:worker-1", query: "git" },
    );
    expect(electronMocks.invoke).toHaveBeenNthCalledWith(
      5,
    ...
    expect(electronMocks.invoke).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({ destination: { mode: "merge", name: "Work" } }),
    );
  });
  ...
  it("validates profile-management event payloads and removes wrapped listeners", () => {
    ...
    stateWrapped?.(
      {},
      { viewId: "1:worker-1", profileId: null, temporary: true },
    );
    ...
    manageDispose();
    expect(electronMocks.off).toHaveBeenCalledWith(
      "browser:profileManage",
      manageWrapped,
    );

    const progressListener = vi.fn();
    const progressDispose =
      bridge.browserProfiles.onImportProgress(progressListener);
    const progressWrapped = electronMocks.listeners.get(
      "browserProfiles:import:progress",
    );
    progressWrapped?.(
      {},
      { requestId: "request", phase: "reading", completed: 1, total: 2 },
    );
