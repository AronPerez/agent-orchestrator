...
Triage bugs into well-structured GitHub issues on the AO repo.

> **AO is Go + Electron.** The backend is a Go daemon (`backend/`)
> exposing a loopback HTTP API on `127.0.0.1:3001`; the frontend is an Electron +
> React supervisor (`frontend/`). There is **no** pm2/Node runtime here — the
> daemon owns lifecycle and terminal sessions run under a runtime adapter
> (**tmux** on macOS/Linux, **ConPTY** on Windows; see
> `backend/internal/adapters/runtime/`). Triage against _this_ stack, not the old
> TypeScript agent-orchestrator.

## ⚠️ Which `ao` are you running?

**`AO` ships no `ao` on your PATH.** A bare `ao` very likely resolves to a
...
in AO (and miss ones that do).

Before any diagnostics:

```bash
which -a ao                      # see every ao on PATH — expect surprises
ao status 2>/dev/null            # if this shows port 3000, it is NOT AO
```

Use a AO binary explicitly:
...
this skill, `ao` means _your verified AO binary_ (`/tmp/ao` or the bundled
...
- **Target repo:** Always file on **`AronPerez/agent-orchestrator`** (the product repo, not
  a fork). AO is the product, not a thin fork of upstream.
...
AO binary (`/tmp/ao` here) for every `ao` call:
...
tmux ls                                # tmux runtime sessions backing terminals
...
- Runtime adapter (tmux): `backend/internal/adapters/runtime/`
...
check changelogs. Root cause is sometimes in a dependency, not AO.
...
```bash
gh issue list --repo AronPerez/agent-orchestrator --state all --search "<symptom>"
gh issue list --repo AronPerez/agent-orchestrator --state all --search "<component-name>"
gh issue list --repo AronPerez/agent-orchestrator --state all --search "<error-message>"
gh pr list --repo AronPerez/agent-orchestrator --state all --search "<keywords>"
```

### Duplicate found → comment on existing issue

```bash
gh issue comment <number> --repo AronPerez/agent-orchestrator --body "$(cat <<'EOF'
## New Report
...
- [ ] Reproduced against AO (:3001 / Go code path), not another AO install
...
# Create asset branch
gh api -X POST repos/AronPerez/agent-orchestrator/git/refs \
...
gh api -X PUT "repos/AronPerez/agent-orchestrator/contents/.issue-assets/${SLUG}/name.png" \
  -f message="chore: upload screenshot" \
  -f content="$IMG_B64" \
  -f branch="issue-assets-${SLUG}"
# Use: ![screenshot](https://raw.githubusercontent.com/AronPerez/agent-orchestrator/issue-assets-<slug>/.issue-assets/<file>)
```

### 5c. Create the issue

```bash
gh issue create --repo AronPerez/agent-orchestrator --title "<title>" --body "$(cat <<'EOF'
## Bug
<summary>
...
```bash
gh label list --repo AronPerez/agent-orchestrator          # source of truth — apply only these
gh issue edit <number> --repo AronPerez/agent-orchestrator --add-label "bug"
```
...
AO is a Go repo — fixes go through a local branch, build, and `gh pr create`.
...
- **Trivial, verifiable fix** (you can build and test it yourself):
  ...
  gh pr create --repo AronPerez/agent-orchestrator --fill \
...
- **Non-trivial fix** (broad change, needs iteration, or you can't fully verify):
  spawn a AO worker session to do the work in its own worktree instead of
  pushing a guess:
  ...
  Build with 'cd backend && go build ./... && go test ./...' before opening a PR against AronPerez/agent-orchestrator."
  ```
...
- Terminal bugs → tmux runtime adapter vs the terminal mux vs the Electron xterm
...
```bash
gh api repos/AronPerez/agent-orchestrator/git/trees/main?recursive=1 --jq '.tree[].path'    # list files
gh api repos/AronPerez/agent-orchestrator/contents/{path} --jq '.content' | python3 -c "import base64,sys; sys.stdout.buffer.write(base64.b64decode(sys.stdin.read()))"  # read file
gh search code "term" --repo AronPerez/agent-orchestrator --json path --jq '.[].path'        # search code
gh api "repos/AronPerez/agent-orchestrator/commits?path={path}&per_page=10" --jq '.[] | "\(.sha[0:8]) \(.commit.message | split("\n")[0])"'  # file history
```

### C. Build / Version Diagnostics

AO is built from source, not published to npm. Pin the binary under test
...
- **Linkify all issue/PR refs:** `[#123](https://github.com/AronPerez/agent-orchestrator/issues/123)`, `[PR #456](url)`. Never bare `#123`.

## Pitfalls

- **Wrong `ao` binary.** A bare `ao` may be a different AO install (old npm build on
  :3000). Always pin a AO binary and confirm `ao status` shows port **3001**.
- **Verify the bug reproduces against AO (:3001 / Go code path) before
...
- **Only apply labels that exist** (`gh label list --repo AronPerez/agent-orchestrator`).
