...
describe("parsePairingPayload", () => {
  ...
  it("defaults secure to false when absent", () => {
    const parsed = parsePairingPayload(
      JSON.stringify({ v: 1, host: "192.168.1.5", port: 3011 }),
    );
  ...
  it.each(["yes", 1, null])("treats malformed secure (%j) as false", (bad) => {
    const parsed = parsePairingPayload(
      JSON.stringify({ v: 1, host: "192.168.1.5", port: 3011, secure: bad }),
    );
    expect(parsed?.secure).toBe(false);
  });
  ...
  it("still rejects a v other than 1", () => {
    expect(
      parsePairingPayload(JSON.stringify({ v: 2, host: "h", port: 1 })),
    ).toBeNull();
  });

  it("still rejects an empty host", () => {
    expect(
      parsePairingPayload(JSON.stringify({ v: 1, host: "", port: 1 })),
    ).toBeNull();
  });

  it("still accepts numeric and string ports, returned as a string", () => {
    expect(
      parsePairingPayload(JSON.stringify({ v: 1, host: "h", port: 3011 }))
        ?.port,
    ).toBe("3011");
    expect(
      parsePairingPayload(JSON.stringify({ v: 1, host: "h", port: "3011" }))
  ...
  it("still defaults a missing password to an empty string", () => {
    expect(
      parsePairingPayload(JSON.stringify({ v: 1, host: "h", port: 1 }))
        ?.password,
    ).toBe("");
  });
});
...
describe("applyPairingPayload", () => {
  ...
  it("does not carry a saved node label onto a scanned machine", () => {
    const stored = cfg({ label: "Old laptop" });
    const parsed = {
      host: "new-host",
      port: "3011",
      password: "",
    ...
    expect(applyPairingPayload(stored, parsed).label).toBe("");
  });
});
