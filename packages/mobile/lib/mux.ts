...
function base64ToBytes(b64: string): Uint8Array {
  ...
  for (let i = 0; i < clean.length; i += 4) {
    ...
    const c =
      i + 2 < clean.length ? (B64_LOOKUP[clean.charCodeAt(i + 2)] ?? 0) : -1;
    const d =
      i + 3 < clean.length ? (B64_LOOKUP[clean.charCodeAt(i + 3)] ?? 0) : -1;
    out.push((a << 2) | (b >> 4));
...
export class MuxClient {
  ...
  private open() {
    ...
    try {
      if (Platform.OS === "web") {
        // Browsers cannot set headers on new WebSocket(), so the connection
        // token rides in the subprotocol list instead: the daemon
        // authenticates the "ao.bearer.<password>" entry and negotiates/
        // echoes the "ao.auth" marker (browsers fail the handshake if they
        // request subprotocols and get no echo). Without this the daemon's
        // LAN listener 401s the tokenless upgrade, and the reconnect loop
        // below trips its per-IP lockout — 429ing the page's REST calls too.
        // The upgrade carries the page's real Origin (not overridable), the
        // same one REST fetches send, so CORS passes identically.
        ws = new WebSocket(
          muxUrl(this.cfg),
          this.cfg.password
            ? ["ao.auth", `ao.bearer.${this.cfg.password}`]
            : undefined,
        );
      } else {
        ...
        ws = new WS(muxUrl(this.cfg), undefined, {
    ...
    if (msg.ch === "sessions" && msg.type === "snapshot") {
      ...
      switch (msg.type) {
        case "data":
          ...
          this.handlers.onTerminalData?.(
            id,
            base64ToBytes(String(msg.data ?? "")),
          );
          break;
        case "opened":
        ...
        case "error":
          this.handlers.onTerminalError?.(
            id,
            msg.error ?? msg.message ?? "terminal error",
          );
          break;
        case "resize":
  ...
  sendInput(id: string, data: string, projectId?: string) {
    ...
    this.send({
      ...
      id,
      type: "data",
      data: bytesToBase64(utf8Encode(data)),
      projectId,
    });
  }
